...
def base_pairs(structure: Structure, dictionary: ComponentDictionary, options: BasePairOptions) -> list[BasePair]: ...
def cation_pi(structure: Structure, options: CationPiOptions) -> list[CationPi]: ...
def chain_interface(structure: Structure, first: str, second: str, cutoff: float, backend: object) -> list[int]: ...
def half_sphere_exposure(structure: Structure, radius: float, backend: object) -> list[HalfSphereExposure]: ...
def native_contact_fraction(reference: Structure, target: Structure, cutoff: float, tolerance: float, backend: object) -> NativeContacts: ...
def pi_stacking(structure: Structure, options: PiStackingOptions) -> list[PiStacking]: ...
def residue_contact_map(structure: Structure, cutoff: float, minimum_separation: int, backend: object) -> ContactMap: ...
def salt_bridges(structure: Structure, maximum_distance: float, backend: object) -> list[SaltBridge]: ...
def secondary_structure(structure: Structure, options: DsspOptions) -> list[SseRecord]: ...
def surface_contacts(structure: Structure, radii: NDArray[float32], options: SurfaceContactOptions) -> ContactTable: ...
def water_bridges(structure: Structure, options: WaterBridgeOptions) -> list[WaterBridge]: ...
def gaussian_network_model(structure: Structure, sites: Selection, options: GnmOptions, *, periodic: bool = ...) -> GaussianNetworkModel: ...
...
def coordination_numbers(structure: Structure, left: Selection, right: Selection, shell: tuple[float, float], *, backend: object = ..., periodic: bool = ...) -> list[int]: ...
