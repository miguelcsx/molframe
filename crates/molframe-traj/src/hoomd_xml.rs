...
fn interactions<const N: usize>(text: &str) -> Result<Vec<HoomdInteraction<N>>, HoomdXmlError> {
    ...
    // A row is a kind followed by `N` particle indices. `N + 1` cannot be a
    // const argument to `as_chunks`, so the width stays a runtime value; the
    // divisibility check above makes every chunk exactly one row.
    fields
        .chunks(width)
        .map(|row| {
