...
/// Parses the sole trajectory-bearing particle group in canonical molframe units.
///
...
fn read_cells(
    ...
    let mut values = read_float(&dataset)?;
    scale_values(&mut values, options.units.length_to_angstrom)?;
    let (boxes, _) = values.as_chunks::<9>();
    boxes
        .iter()
        .map(|chunk| matrix(chunk))
...
fn cell_at(
    cells: Option<&[[[f64; 3]; 3]]>,
    index: usize,
) -> Result<Option<molframe_core::structure::UnitCell>, H5mdError> {
