...
fn cross_similarity(
    ...
    let mut difference =
        (molframe_geom::distance(reference[first.reference], reference[second.reference])
            - molframe_geom::distance(mobile[first.mobile], mobile[second.mobile]))
        .abs();
    difference += (molframe_geom::distance(
        reference[first.reference + window - 1],
        reference[second.reference + window - 1],
    ) - molframe_geom::distance(
