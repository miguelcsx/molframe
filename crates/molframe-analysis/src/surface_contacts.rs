...
use molframe_surface::{ExcludedSurfacePoint, SasaError, surface_points_excluding_pairs};
...
pub fn surface_contacts(
    ...
    let parts = reduce_pairs_within_unsorted(
        ...
        |close, pair| {
            ...
            let distance = f64_to_f32(molframe_geom::distance(a, b));
    ...
    for (first, second, distance) in close {
        ...
        if support(
            ...
            contacts.push(Contact {
                first: molframe_core::index::AtomIndex::new(usize_to_u32(first)),
                second: molframe_core::index::AtomIndex::new(usize_to_u32(second)),
                distance,
            });
        }
    }
