...
use molframe_core::annotation::AtomAnnotation;
...
pub(crate) fn component_kind(
    ...
    match column.get(atom) {
        Some((code, Presence::Present)) => molframe_chem::ComponentKind::from_code(code),
        _ => None,
    }
}
...
pub(crate) fn polymer_role(
    ...
    match column.get(atom) {
        Some((code, Presence::Present)) => molframe_chem::PolymerAtomRole::from_code(code),
        _ => None,
    }
}
