...
use molframe_core::{ExecutionContext, structure::Structure};
...
pub fn native_contact_fraction(
    ...
    let all = molframe_core::selection::AtomSelection::All(reference.atom_count());
    ...
    let counts = reduce_pairs_within_unsorted(
        ...
        |totals, pair| {
            ...
            if molframe_geom::distance_squared(a, b) <= kept_cutoff_squared {
                totals.1 += 1;
            }
        },
    )?;
