...
use molframe_chem::{ComponentProvider, PolymerAtomRole, PolymerRoleProfile};
use molframe_core::index::ResidueIndex;
use molframe_core::structure::{AtomRef, ResidueRef, Structure};
use molframe_core::{AtomAnnotation, Diagnostic, Presence};
...
fn measure_residue(
    ...
    component: &molframe_chem::Component,
    ...
    let base_plane_deviation =
        molframe_geom::plane_deviation_with_options(&base_points, policy.plane_fit)
    ...
    let glycosidic_bond_length = role_position(structure, residue, PolymerAtomRole::NUCLEIC_C1)?
        ...
        .map(|(first, second)| molframe_geom::distance(first, second));
...
fn sugar_pseudorotation(points: [Option<[f32; 3]>; 5]) -> Option<Pucker> {
    ...
    let torsions = [
        molframe_geom::dihedral(o4, c1, c2, c3),
        molframe_geom::dihedral(c1, c2, c3, c4),
        molframe_geom::dihedral(c2, c3, c4, o4),
        molframe_geom::dihedral(c3, c4, o4, c1),
        molframe_geom::dihedral(c4, o4, c1, c2),
    ];
    let values: Option<Vec<_>> = torsions
        .into_iter()
        .map(|value| value.map(molframe_geom::degrees))
        .collect();
