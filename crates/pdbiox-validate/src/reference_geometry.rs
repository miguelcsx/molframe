...
fn assess_component(
    ...
    for [first_index, centre_index, third_index] in angles {
        ...
        let (Some(observed_angle), Some(expected_angle)) = (
            pdbiox_geom::angle(first.1, centre.1, third.1),
            pdbiox_geom::angle(ideal[first_index], ideal[centre_index], ideal[third_index]),
        ) else {
            continue;
        };
        report.assessed += 1;
        let observed_degrees = pdbiox_geom::degrees(observed_angle);
        let expected_degrees = pdbiox_geom::degrees(expected_angle);
...
fn assess_bond(
    ...
    let observed = pdbiox_geom::distance(first.1, second.1);
    let expected = pdbiox_geom::distance(ideal_first, ideal_second);
