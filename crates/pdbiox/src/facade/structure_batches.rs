...
pub enum StructureBatchReader {
    /// `BinaryCIF` source.
    #[cfg(feature = "bcif")]
    BinaryCif(pdbiox_bcif::BcifBatchSource<WindowedSourceFile>),
    /// Text PDB source.
    #[cfg(feature = "pdb")]
    Pdb(pdbiox_pdb::PdbBatchSource<WindowedSourceFile>),
    /// Text mmCIF source.
    #[cfg(feature = "mmcif")]
    Mmcif(pdbiox_cif::MmcifBatchSource<WindowedSourceFile>),
    /// Text `ModelCIF` source selected explicitly by its constructor.
    #[cfg(feature = "modelcif")]
    ModelCif(pdbiox_modelcif::ModelCifBatchSource<WindowedSourceFile>),
}
...
pub fn open_structure_batches(
    ...
    match format {
        #[cfg(feature = "pdb")]
        Format::Pdb => Ok(StructureBatchReader::Pdb(pdbiox_pdb::PdbBatchSource::new(
            source,
            options.clone(),
            DatasetId::new(0),
            ChunkId::new(0),
        ...
        Format::Mmcif => Ok(StructureBatchReader::Mmcif(
            pdbiox_cif::MmcifBatchSource::new(
                source,
                options.clone(),
                DatasetId::new(0),
                ChunkId::new(0),
        ...
        Format::BinaryCif => Ok(StructureBatchReader::BinaryCif(
            pdbiox_bcif::BcifBatchSource::new(
                source,
                options.clone(),
                DatasetId::new(0),
                ChunkId::new(0),
