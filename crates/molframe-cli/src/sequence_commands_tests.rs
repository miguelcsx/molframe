...
fn format_conversion_delegates_to_native_parsers_and_writers() {
    let document =
        molframe::seq::read_sequence(">a\nACD\n>b\nA-D\n", molframe::seq::SequenceFormat::Fasta)
            .unwrap_or_else(|error| panic!("parse failed: {error}"));
    let rendered = molframe::seq::write_sequence(&document, molframe::seq::SequenceFormat::Clustal)
...
fn fastq_output_refuses_to_invent_quality_scores() {
    let document = molframe::seq::read_sequence(">a\nACD\n", molframe::seq::SequenceFormat::Fasta)
        .unwrap_or_else(|error| panic!("parse failed: {error}"));
    let error = molframe::seq::write_sequence(&document, molframe::seq::SequenceFormat::Fastq)
...
fn alignment_projection_preserves_native_correspondence() {
    let alignment = match molframe::seq::global(b"AC", b"A", molframe::seq::Scoring::simple()) {
