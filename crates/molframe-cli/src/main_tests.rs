...
fn traj_alias_has_the_same_declarative_command() {
    let cli = Cli::try_parse_from(["molframe", "traj", "info", "run.xtc"])
...
fn output_format_is_optional_for_context_sensitive_defaulting() {
    let cli = Cli::try_parse_from(["molframe", "policy"]).expect("policy should parse");
    assert_eq!(cli.global.format, None);
}

#[test]
fn diff_requires_an_explicit_coordinate_tolerance() {
    assert!(Cli::try_parse_from(["molframe", "diff", "left.cif", "right.cif"]).is_err());
...
fn advanced_workflows_have_declarative_entrypoints() {
    ...
    assert!(Cli::try_parse_from(["molframe", "sequence", "extract", "entry.cif"]).is_ok());
    assert!(
        Cli::try_parse_from([
            "molframe",
            "sequence",
    ...
    assert!(
        Cli::try_parse_from([
            "molframe", "sequence", "convert", "-", "--from", "fasta", "--to", "clustal"
        ])
        .is_ok()
    );
    assert!(
    ...
    assert!(Cli::try_parse_from(["molframe", "batch", "info", "*.cif", "--workers", "4",]).is_ok());
    assert!(Cli::try_parse_from(["molframe", "man", "--outdir", "man"]).is_ok());
}
...
fn global_policy_overrides_are_typed_and_strict() {
    ...
    assert!(
        Cli::try_parse_from(["molframe", "info", "entry.cif", "--assembly", "crystal:nan"])
            .is_err()
    );
    assert!(
        Cli::try_parse_from([
