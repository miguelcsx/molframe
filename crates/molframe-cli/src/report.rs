...
use molframe::{Diagnostic, ParseMode, Rendered};
...
pub struct Context {
    ...
    pub policy: &'static molframe::AnalysisPolicy,
...
impl Context {
    ...
    fn with_embedded_provenance(self, text: &str) -> String {
        ...
        match self.format {
            ...
            OutputKind::Csv | OutputKind::Tsv => {
                format!("{text}\n# molframe-provenance={provenance}")
            }
            _ => text.to_owned(),
        }
    }
    ...
    pub(super) fn provenance_json(self) -> String {
        ...
        record
            .text("molframe_version", env!("CARGO_PKG_VERSION"))
            .text(
                "profile",
                &self
                    .policy
        ...
        record.text(
            ...
            match self.missing_element_policy {
                molframe::MissingElementPolicy::PreserveUnknown => "preserve-unknown",
                molframe::MissingElementPolicy::InferFromAtomName => "infer-from-atom-name",
            },
        );
        record.text(
            "ambiguous_residue_boundary_policy",
            match self.residue_boundary_policy {
                molframe::AmbiguousResidueBoundaryPolicy::Reject => "reject",
    ...
    pub fn provenance_metadata(self) -> std::collections::BTreeMap<String, String> {
        let mut metadata = std::collections::BTreeMap::from([
            (
                "molframe.version".to_owned(),
                env!("CARGO_PKG_VERSION").to_owned(),
            ),
            (
                "molframe.policy_fingerprint".to_owned(),
                self.policy.fingerprint().to_string(),
            ),
            ("molframe.policy".to_owned(), self.policy.to_string()),
            (
                "molframe.missing_element_policy".to_owned(),
                match self.missing_element_policy {
                    molframe::MissingElementPolicy::PreserveUnknown => "preserve-unknown",
                    molframe::MissingElementPolicy::InferFromAtomName => "infer-from-atom-name",
                }
            ...
            (
                "molframe.ambiguous_residue_boundary_policy".to_owned(),
                match self.residue_boundary_policy {
                    molframe::AmbiguousResidueBoundaryPolicy::Reject => "reject",
                    molframe::AmbiguousResidueBoundaryPolicy::InferFromFileOrder => {
                        "infer-from-file-order"
        ...
        if let Some(version) = self.ccd_version {
            metadata.insert("molframe.ccd_version".to_owned(), version.to_owned());
        }
        metadata
    }
}
