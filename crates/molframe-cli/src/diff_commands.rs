...
use molframe::{CountDifference, StructureDifference, ValueDifference};
...
pub(crate) fn diff(
    ...
    let result = molframe::structure_difference(
        &left,
        &right,
...
fn push_cell_json(
    output: &mut Vec<String>,
    difference: Option<&ValueDifference<Option<molframe::UnitCell>>>,
) {
    if let Some(difference) = difference {
        let mut object = Json::new();
...
fn cell_field(object: &mut Json, name: &str, cell: Option<molframe::UnitCell>) {
    match cell {
