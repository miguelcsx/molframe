...
pub fn rmsd_to_reference(
    ...
    if frames
        .iter()
        .any(|frame| frame.positions.len() != target.positions.len())
    {
        return Err(EnsembleGeometryError::DimensionMismatch);
    }
    frames
        .iter()
        .map(|frame| match alignment {
            FrameAlignment::None => rmsd(&frame.positions, &target.positions)
                .map_err(|_| EnsembleGeometryError::DimensionMismatch),
            FrameAlignment::Rigid => superpose(&frame.positions, &target.positions)
                .map(|fit| fit.rmsd)
...
pub fn pairwise_fitted_rmsd(
    ...
    let mut matrix = allocate_square(frames.len(), memory_limit)?;
    for left in 0..frames.len() {
        for right in (left + 1)..frames.len() {
            let fit = superpose(&frames[left], &frames[right])
                .map_err(|_| EnsembleGeometryError::DegenerateFit)?;
            set_pair(&mut matrix, frames.len(), left, right, fit.rmsd);
        }
    }
...
pub fn generalized_procrustes_mean(
    ...
    for _ in 0..max_iterations {
        ...
        let inverse = f64_from_usize(frames.len())
            .ok_or(EnsembleGeometryError::InvalidParameter)?
            .recip();
        let next: Vec<_> = sum
            .into_iter()
...
pub(crate) fn validate_frames(frames: &[Vec<[f32; 3]>]) -> Result<usize, EnsembleGeometryError> {
    let Some(first) = frames.first() else {
        return Err(EnsembleGeometryError::Empty);
    };
    if first.len() < 3 || frames.iter().any(|frame| frame.len() != first.len()) {
        return Err(EnsembleGeometryError::DimensionMismatch);
    }
    Ok(first.len())
}
