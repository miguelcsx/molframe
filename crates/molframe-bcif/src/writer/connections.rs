...
fn endpoint(
    ...
    bond: usize,
    side: u8,
) -> Result<Endpoint<'_>, molframe_cif::CifWriteError> {
    let Some(source) = structure.atom(molframe_core::AtomIndex::new(index)) else {
        return Err(molframe_cif::CifWriteError::InvalidBondAtom { bond, atom: index });
    };
    ...
    let Some(chain) = chain
        .and_then(molframe_core::structure::ChainRef::label)
...
const fn missing(bond: usize, endpoint: u8, field: &'static str) -> molframe_cif::CifWriteError {
    molframe_cif::CifWriteError::MissingBondField {
        bond,
        endpoint,
        field,
    }
