...
pub struct ReexecutionEnvironment<'a> {
    /// Installed molframe version.
    pub molframe_version: &'a str,
...
pub enum ReexecutionError {
    ...
    #[error("molframe version mismatch: expected {expected}, found {observed}")]
...
/// access. Their fingerprint, the full policy, molframe version and both optional
...
fn validate(
    ...
    if provenance.molframe_version != environment.molframe_version {
        return Err(ReexecutionError::SoftwareVersionMismatch {
            expected: provenance.molframe_version.into(),
            observed: environment.molframe_version.into(),
        });
    }
    check_dictionary(
