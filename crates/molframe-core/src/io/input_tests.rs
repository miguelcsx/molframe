...
fn a_large_plain_local_file_is_parsed_directly_from_a_mapping() {
    ...
    let path = std::env::temp_dir().join(format!("molframe-input-{}", std::process::id()));
...
fn a_small_local_file_avoids_mapping_overhead() {
    ...
    let path = std::env::temp_dir().join(format!("molframe-small-input-{}", std::process::id()));
    let mut file = match File::create(&path) {
        Ok(file) => file,
        Err(error) => panic!("create failed: {error}"),
    };
...
fn a_gzip_file_expands_into_private_mapped_storage() {
    ...
    let path = std::env::temp_dir().join(format!("molframe-gzip-input-{}", std::process::id()));
...
fn a_zstd_file_expands_into_private_mapped_storage() {
    let path = std::env::temp_dir().join(format!("molframe-zstd-input-{}", std::process::id()));
