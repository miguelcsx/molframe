...
fn the_blank_altloc_is_distinct_from_every_label() -> Result<(), DictionaryFull> {
    ...
    let Some(a) = AltId::labelled(interner.intern("A")?) else {
        panic!("interner identifier must fit alternate-label encoding");
    };
...
fn an_altloc_label_longer_than_one_character_is_representable() -> Result<(), DictionaryFull> {
    let mut interner = Interner::new();
    let Some(long) = AltId::labelled(interner.intern("ALT2")?) else {
        panic!("interner identifier must fit alternate-label encoding");
    };
