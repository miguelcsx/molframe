//! Benchmarks for `pdbiox-core` hot paths.
//!
...
use pdbiox_bench::{Sample, input, input_gzip, large_cif_gz, structure};
...
use pdbiox_core::io::{Format, InputBuffer, Limits};
    ...
    group.bench_function("uncompressed", |b| {
        b.iter(|| {
            let _ = black_box(InputBuffer::open(
                "crates/pdbiox-bench/data/4hhb.cif",
                Limits::default(),
            ));
        });
    });
