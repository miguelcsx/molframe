...
pub enum Kind {
    /// An error: rendered `PDBIOX-Ennnn`.
    Error,
    /// A warning: rendered `PDBIOX-Wnnnn`.
    Warning,
}
...
impl Severity {
    ...
    ///
    /// ```
    /// use pdbiox_core::{Severity, Strictness};
    ///
...
/// A stable diagnostic code, rendered as `PDBIOX-E1103` or `PDBIOX-W3011`.
///
/// # Examples
///
/// ```
...
///
/// assert_eq!(Code::E1103.to_string(), "PDBIOX-E1103");
...
impl Code {
    ...
    pub fn remedy(self) -> &'static str {
        self.entry()
            .map_or("report this as a pdbiox bug", |entry| entry.remedy)
    }
...
impl fmt::Display for Code {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "PDBIOX-{}{:04}", self.kind.letter(), self.number)
    }
}
...
pub enum Class {
    ...
    Resource,
    /// An internal invariant was violated. These are bugs in pdbiox.
    Internal,
}
