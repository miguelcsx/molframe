...
fn annotation_names_iterate_deterministically() {
    ...
    let _ = annotations.insert(
        "zeta",
        AtomAnnotation::Boolean(AnnotationColumn::from_values(vec![true]).expect("small column")),
    );
    let _ = annotations.insert(
        "alpha",
        AtomAnnotation::Real(AnnotationColumn::from_values(vec![1.0]).expect("small column")),
    );
    assert_eq!(
