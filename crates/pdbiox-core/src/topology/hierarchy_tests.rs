...
fn sample() -> Topology {
    ...
    let entity = topology.entities.push(
        SymbolId::from_raw(1),
        EntityKind::Polymer,
        OptionalSymbol::NONE,
        &[
...
fn a_deposited_model_number_is_kept_rather_than_replaced_by_its_position() {
    let mut topology = Topology::default();
    topology.models.push(5, 0..0);
    topology.models.push(7, 0..0);
...
fn chains_that_are_copies_of_one_species_are_found_by_reading_a_column() {
    let mut topology = Topology::default();
    let first = topology.entities.push(
        SymbolId::from_raw(1),
        EntityKind::Polymer,
        OptionalSymbol::NONE,
        &[],
    );
    let second = topology.entities.push(
        SymbolId::from_raw(2),
        EntityKind::Water,
        OptionalSymbol::NONE,
        &[],
