...
pub(crate) fn squared_deviation_sum(left: &[[f32; 3]], right: &[[f32; 3]]) -> f64 {
    let mut total = 0.0;
    let (left_blocks, left_tail) = left.as_chunks::<LANES>();
    let (right_blocks, right_tail) = right.as_chunks::<LANES>();
...
pub(crate) fn distances_into(left: &[[f32; 3]], right: &[[f32; 3]], output: &mut [f64]) {
    let (left_blocks, left_tail) = left.as_chunks::<LANES>();
    let (right_blocks, right_tail) = right.as_chunks::<LANES>();
    let (output_blocks, output_tail) = output.as_chunks_mut::<LANES>();
    ...
    for ((left, right), output) in left_tail.iter().zip(right_tail).zip(output_tail) {
...
pub(crate) fn transform_all(
    ...
    let (blocks, tail) = positions.as_chunks_mut::<LANES>();
    for block in blocks {
...
pub(crate) fn sum_positions(positions: &[[f32; 3]]) -> [f64; 3] {
    let mut sums = [0.0; 3];
    let (blocks, tail) = positions.as_chunks::<LANES>();
    for block in blocks {
...
pub(crate) fn weighted_position_sum(positions: &[[f32; 3]], weights: &[f64]) -> ([f64; 3], f64) {
    ...
    let (position_blocks, position_tail) = positions[..paired].as_chunks::<LANES>();
    let (weight_blocks, weight_tail) = weights[..paired].as_chunks::<LANES>();
    for (positions, weights) in position_blocks.iter().zip(weight_blocks) {
    ...
    for (position, weight) in position_tail.iter().zip(weight_tail) {
...
pub(crate) fn squared_radius_sum(
    ...
    let mut sum = 0.0;
    let (blocks, tail) = positions[..paired].as_chunks::<LANES>();
    let mut offset = 0;
    for block in blocks {
    ...
    for (index, position) in tail.iter().enumerate() {
        ...
        sum += match weights {
            Some(values) => squared * values[offset + index],
            None => squared,
        };
    }
    sum
