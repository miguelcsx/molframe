...
pyo3::create_exception!(molframe_adapters, DownloadError, PyRuntimeError);
pyo3::create_exception!(molframe_adapters, TopologyBatchError, PyValueError);

#[pyclass(name = "DownloadOptions", frozen, skip_from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyDownloadOptions(molframe::adapters::DownloadOptions);
...
impl PyDownloadOptions {
    ...
    fn new(max_bytes: u64, timeout_seconds: f64, redirect_limit: usize) -> PyResult<Self> {
        ...
        Ok(Self(molframe::adapters::DownloadOptions {
            max_bytes,
            timeout,
            redirect_limit,
        }))
...
#[derive(Clone, Debug)]
pub(crate) struct PyVerifiedDownload(molframe::adapters::VerifiedDownload);
...
pub(crate) fn fetch_verified(
    ...
    py.detach(move || {
        molframe::adapters::fetch_verified(&url, &expected_sha256, options.0)
...
#[derive(Clone, Debug)]
pub(crate) struct PyTopologyBatch(Option<molframe::adapters::TopologyBatch>);
...
impl PyTopologyBatch {
    ...
    fn from_model(
        ...
        molframe::adapters::TopologyBatch::from_model(
            structure.structure(),
            model,
            namespace.into(),
        )
...
impl PyTopologyBatch {
    fn batch(&self) -> PyResult<&molframe::adapters::TopologyBatch> {
        self.0.as_ref().ok_or_else(consumed_batch)
    }
}

fn validity_flags(validity: &molframe::core::ValidityMask) -> Vec<bool> {
    (0..validity.len())
        .map(|index| validity.get(index).is_present())
        .collect()
}

fn download_error(error: molframe::adapters::DownloadError) -> PyErr {
    DownloadError::new_err(error.to_string())
}

fn topology_error(error: molframe::adapters::TopologyBatchError) -> PyErr {
    TopologyBatchError::new_err(error.to_string())
}

fn topology_import_error(error: molframe::adapters::TopologyImportError) -> PyErr {
    TopologyBatchError::new_err(error.to_string())
}
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add("MISSING_STRING", molframe::adapters::MISSING_STRING)?;
