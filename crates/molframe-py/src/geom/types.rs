...
enum DecompositionValue {
    Three(molframe::Decomposition<3>),
    Four(molframe::Decomposition<4>),
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyTorusMetric(pub(crate) molframe::TorusMetric);

#[pymethods]
impl PyTorusMetric {
    #[new]
    fn new(weights: Vec<f64>) -> PyResult<Self> {
        molframe::TorusMetric::new(weights.into_boxed_slice())
            .map(Self)
    ...
    fn uniform(dimension: usize) -> PyResult<Self> {
        molframe::TorusMetric::uniform(dimension)
            .map(Self)
            .map_err(periodic_error)
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBackboneResidue(pub(crate) molframe::BackboneResidue);
...
impl PyBackboneResidue {
    ...
    fn new(
        ...
        Ok(Self(molframe::BackboneResidue {
            nitrogen,
            alpha_carbon,
            carbonyl_carbon,
            connected_to_next,
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyRotationOptions(pub(crate) molframe::RotationOptions);

#[pymethods]
impl PyRotationOptions {
    #[classattr]
    const STANDARD_MATRIX_TOLERANCE: f64 = molframe::RotationOptions::STANDARD_MATRIX_TOLERANCE;

    #[classattr]
    const STANDARD_SMALL_ANGLE_TOLERANCE: f64 =
        molframe::RotationOptions::STANDARD_SMALL_ANGLE_TOLERANCE;

    #[classattr]
    const STANDARD_HALF_TURN_TOLERANCE: f64 =
        molframe::RotationOptions::STANDARD_HALF_TURN_TOLERANCE;

    #[new]
    fn new(matrix_tolerance: f64, small_angle_tolerance: f64, half_turn_tolerance: f64) -> Self {
        Self(molframe::RotationOptions {
            matrix_tolerance,
    ...
    #[staticmethod]
    fn standard() -> Self {
        Self(molframe::RotationOptions::standard())
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyRotationMeanOptions(pub(crate) molframe::RotationMeanOptions);
...
impl PyRotationMeanOptions {
    ...
    fn new(
        ...
        Self(molframe::RotationMeanOptions {
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PySuperposeOptions(pub(crate) molframe::SuperposeOptions);

#[pymethods]
impl PySuperposeOptions {
    #[classattr]
    const STANDARD_COLLINEAR_RELATIVE_TOLERANCE: f64 =
        molframe::SuperposeOptions::STANDARD_COLLINEAR_RELATIVE_TOLERANCE;

    #[new]
    fn new(collinear_relative_tolerance: f64, eigen: &PyEigenOptions) -> Self {
        Self(molframe::SuperposeOptions {
    ...
    #[staticmethod]
    fn standard() -> Self {
        Self(molframe::SuperposeOptions::standard())
    }
...
fn symmetric_impl(
    py: Python<'_>,
    matrix: PyReadonlyArray2<'_, f64>,
    options: Option<molframe::EigenOptions>,
) -> PyResult<PyDecomposition> {
    ...
    match values.len() {
        9 => {
            ...
            py.detach(move || match options {
                Some(options) => molframe::geom::symmetric_with_options(matrix, options),
                None => molframe::geom::symmetric(matrix),
            })
...
pub(crate) fn backbone_torsions(
    ...
    py.detach(move || {
        molframe::backbone_torsions(&residues.iter().map(|value| value.0).collect::<Vec<_>>())
    })
    .into_iter()
...
fn periodic_values(values: &[f64]) -> PyResult<Vec<molframe::PeriodicAngle>> {
    values
        .iter()
        .copied()
        .map(|value| molframe::PeriodicAngle::from_radians(value).map_err(periodic_error))
        .collect()
}
