...
pub(crate) struct PyRigid(pub(crate) molframe::Rigid);

#[pymethods]
impl PyRigid {
    #[classattr]
    const IDENTITY: Self = Self(molframe::Rigid::IDENTITY);

    #[new]
    fn new(rotation: [[f64; 3]; 3], translation: [f64; 3]) -> Self {
        Self(molframe::Rigid::new(rotation, translation))
    }

    #[staticmethod]
    fn identity() -> Self {
        Self(molframe::Rigid::IDENTITY)
    }

    #[staticmethod]
    fn translation(offset: [f64; 3]) -> Self {
        Self(molframe::Rigid::translation(offset))
    }
...
impl PyStructure {
    ...
    fn transformed(
        ...
        let selection = selection.map_or_else(
            || molframe::AtomSelection::All(structure.atom_count()),
            |selection| selection.inner.clone(),
        );
        let transform = transform.0;
        py.detach(|| molframe::transform(&structure, &selection, &transform))
            .map(Self::new)
