...
use molframe::traj::{DiffusionMap, PcaResult};
...
) -> PyResult<molframe::FrameInput<'py>> {
...
pub(crate) fn borrowed_frames<'py>(
    values: &'py PyReadonlyArray3<'py, f32>,
) -> PyResult<molframe::traj::FrameView<'py>> {
    let input = borrowed_frame_input(values)?;
    molframe::traj::FrameView::new(input.positions, input.frame_count, input.atom_count)
