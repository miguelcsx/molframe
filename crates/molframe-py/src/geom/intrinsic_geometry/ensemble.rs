...
use molframe::{PeriodicAngle, TorusMetric};
...
pub(crate) struct PyHarmonicSimilarityOptions(molframe::traj::HarmonicSimilarityOptions);

#[pymethods]
impl PyHarmonicSimilarityOptions {
    #[new]
    fn new(covariance_regularization: f64, memory_limit_bytes: usize) -> Self {
        Self(molframe::traj::HarmonicSimilarityOptions {
...
pub(crate) fn analyse_rmsd_to_reference(
    ...
    let result = py
        .detach(|| {
            molframe::traj::analyse_rmsd_to_reference_view(
                frames,
                reference,
                alignment.into(),
                &policy,
...
pub(crate) fn analyse_pairwise_fitted_rmsd(
    ...
    let policy = policy.inner.clone();
    let result = py
        .detach(|| molframe::traj::analyse_pairwise_fitted_rmsd_view(frames, memory_limit, &policy))
        .map_err(value_error)?;
...
pub(crate) fn analyse_pairwise_torus_distance(
    ...
    let result = py
        .detach(|| {
            molframe::traj::analyse_pairwise_torus_distance(
                &point_values,
                &metric,
                &metric_name,
                memory_limit,
...
pub(crate) fn analyse_generalized_procrustes_mean(
    ...
    let result = py
        .detach(|| {
            molframe::traj::analyse_generalized_procrustes_mean_view(
                frames,
                tolerance,
                maximum_iterations,
                &policy,
...
pub(crate) fn analyse_agglomerative_clustering(
    ...
    let result = py
        .detach(|| {
            molframe::traj::analyse_agglomerative_clustering(
                &distances,
                cluster_count,
                linkage.into(),
                &policy,
...
pub(crate) fn analyse_dbscan_clustering(
    ...
    let result = py
        .detach(|| {
            molframe::traj::analyse_dbscan_clustering(&distances, epsilon, minimum_points, &policy)
        })
        .map_err(value_error)?;
    analysis_with_value(py, result, clustering_value)
}
...
pub(crate) fn analyse_medoid(
    ...
    let policy = policy.inner.clone();
    let result = py
        .detach(move || molframe::traj::analyse_medoid(&distances, &members, &policy))
        .map_err(value_error)?;
...
pub(crate) fn analyse_kmeans(
    ...
    let result = py
        .detach(move || {
            molframe::traj::analyse_kmeans_view(
                observations,
                rows,
                columns,
                options.native(),
...
pub(crate) fn analyse_harmonic_ensemble_similarity(
    ...
    let result = py
        .detach(move || {
            molframe::traj::analyse_harmonic_ensemble_similarity(
                &first, &second, options.0, &policy,
            )
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_cluster_population_similarity(
    ...
    let result = py
        .detach(move || {
            molframe::traj::analyse_cluster_population_similarity(
                &first,
                &second,
                cluster_count,
                &policy,
...
pub(crate) fn analyse_group_coordinate_variance(
    ...
    let result = py
        .detach(move || {
            molframe::traj::analyse_group_coordinate_variance_view(frames, &groups, &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_block_convergence(
    ...
    let result = py
        .detach(move || {
            molframe::traj::analyse_block_convergence(
                &values,
                block_size,
                remainder.into(),
                &policy,
