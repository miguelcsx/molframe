...
use super::{
    PyCurvature, PySurfaceGridOptions, PySurfaceMesh, array2, borrowed_frames, diffusion_result,
    pca_result, scalar_values, triples_owned, value_error,
};
use crate::contract::{PyAnalysis, analysis_with_value};
use crate::query::PyAnalysisPolicy;
use molframe::PeriodicAngle;
use molframe::surface::{SurfaceWorkflowOptions, SurfaceWorkflowResult, governed_surface_geometry};
use molframe::traj::{
    CartesianFit, EnsembleDistanceMatrix, analyse_cartesian_pca_view, analyse_diffusion_map,
...
pub(crate) fn analyse_pca(
    ...
    let result = py
        .detach(|| {
            analyse_cartesian_pca_view(frames, fit.as_rust(), components, memory_limit, &policy)
        })
        .map_err(value_error)?;
