...
pub(crate) fn distance_matrix<'py>(
    ...
    let matrix = py
        .detach(move || molframe::distance_matrix(positions))
...
pub(crate) fn distance_matrix_value(
    py: Python<'_>,
    matrix: molframe::DistanceMatrix,
...
pub(super) fn matrix_error(error: molframe::MatrixError) -> PyErr {
...
fn rmsd_kernel(
    ...
    py.detach(|| molframe::rmsd_flat(mobile, reference))
...
pub(crate) fn superpose(
    ...
    py.detach(|| molframe::superpose(mobile, reference))
...
pub(crate) fn superpose_with_options(
    ...
    let mobile = borrowed_coordinates(&mobile)?;
    let reference = borrowed_coordinates(&reference)?;
    py.detach(|| molframe::superpose_with_options(mobile, reference, options.0))
        .map(PySuperposition::from)
        .map_err(superpose_error)
}
...
fn superpose_error(error: molframe::SuperposeError) -> PyErr {
    let message = match error {
        molframe::SuperposeError::LengthMismatch => "coordinate sets differ in length",
        molframe::SuperposeError::TooFewPoints => "superposition requires at least three points",
        molframe::SuperposeError::Degenerate => "superposition is undefined for collinear points",
        molframe::SuperposeError::InvalidOptions => "superposition options are invalid",
        molframe::SuperposeError::TooManyPoints => "coordinate set exceeds the supported size",
        molframe::SuperposeError::Eigen(_) => "superposition eigendecomposition failed",
    };
    PyValueError::new_err(message)
}

impl From<molframe::Superposition> for PySuperposition {
    fn from(value: molframe::Superposition) -> Self {
        Self {
