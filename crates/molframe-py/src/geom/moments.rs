...
pub(crate) struct PyEigenOptions {
    pub(crate) inner: molframe::EigenOptions,
}

#[pymethods]
impl PyEigenOptions {
    #[classattr]
    const STANDARD_RELATIVE_TOLERANCE: f64 = molframe::EigenOptions::STANDARD_RELATIVE_TOLERANCE;

    #[classattr]
    const STANDARD_MAXIMUM_SWEEPS: usize = molframe::EigenOptions::STANDARD_MAXIMUM_SWEEPS;

    #[new]
    const fn new(relative_tolerance: f64, maximum_sweeps: usize) -> Self {
        Self {
            inner: molframe::EigenOptions {
                relative_tolerance,
                maximum_sweeps,
            },
        }
    ...
    fn standard() -> Self {
        Self {
            inner: molframe::EigenOptions::standard(),
        }
    }
...
pub(crate) fn centroid(
    ...
    Ok(py.detach(|| molframe::centroid(positions)))
}
...
pub(crate) fn centre_of_mass(
    ...
    Ok(py.detach(|| molframe::centre_of_mass(positions, mass_values)))
}
...
pub(crate) fn radius_of_gyration(
    ...
    let mass_values = mass_values(masses, positions.len())?;
    let mass_values = mass_slice(mass_values.as_ref())?;
    Ok(py.detach(|| molframe::radius_of_gyration(positions, mass_values)))
}
...
pub(crate) fn inertia_tensor(
    ...
    let mass_values = mass_values(masses, positions.len())?;
    let mass_values = mass_slice(mass_values.as_ref())?;
    Ok(py.detach(|| molframe::inertia_tensor(positions, mass_values)))
}
...
pub(crate) fn principal_axes(
    ...
    let mass_values = mass_values(masses, positions.len())?;
    let mass_values = mass_slice(mass_values.as_ref())?;
    py.detach(|| molframe::principal_axes_with_options(positions, mass_values, options.inner))
...
pub(crate) fn asphericity(
    ...
    let positions = borrowed_coordinates(&positions)?;
    py.detach(|| molframe::asphericity_with_options(positions, options.inner))
        .map_err(eigen_error)
}
...
pub(crate) fn gyration_axes(
    ...
    let positions = borrowed_coordinates(&positions)?;
    py.detach(|| molframe::gyration_axes_with_options(positions, options.inner))
        .map(|value| value.map(PyAxes::from))
        .map_err(eigen_error)
}
...
pub(crate) fn best_fit_plane(
    ...
    let positions = borrowed_coordinates(&positions)?;
    py.detach(|| molframe::best_fit_plane_with_options(positions, options.inner))
...
pub(crate) fn plane_deviation(
    ...
    let positions = borrowed_coordinates(&positions)?;
    py.detach(|| molframe::plane_deviation_with_options(positions, options.inner))
        .map_err(eigen_error)
}
...
impl From<molframe::Decomposition<3>> for PyAxes {
    fn from(value: molframe::Decomposition<3>) -> Self {
        Self {
            values: value.values,
...
impl From<molframe::Plane> for PyPlane {
    fn from(value: molframe::Plane) -> Self {
        Self {
            centre: value.centre,
            normal: value.normal,
        }
