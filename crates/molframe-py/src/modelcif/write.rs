...
pub(crate) fn attach(
    ...
    py.detach(move || {
        Ok(PyStructure::new(
            structure.with_extension(molframe::MODEL_CIF_EXTENSION, model_cif),
        ))
    })
}
...
fn write_native(
    ...
    py.detach(move || {
        molframe::modelcif::write_canonical_with_options(&structure, &model_cif, &options.native())
    })
    .map_err(write_error)
}
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add("MODEL_CIF_EXTENSION", molframe::MODEL_CIF_EXTENSION)?;
    Ok(())
}
