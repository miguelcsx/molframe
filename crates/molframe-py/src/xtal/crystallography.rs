...
pub(crate) struct PyUnitCell {
    pub(crate) cell: molframe::UnitCell,
    transform: molframe::CellTransform,
}
...
impl PyUnitCell {
    ...
    fn new(lengths: [f64; 3], angles: [f64; 3]) -> PyResult<Self> {
        Self::build(molframe::UnitCell { lengths, angles })
    }
...
impl PyUnitCell {
    pub(crate) fn from_native(cell: molframe::UnitCell) -> PyResult<Self> {
        Self::build(cell)
    }

    fn build(cell: molframe::UnitCell) -> PyResult<Self> {
        molframe::CellTransform::new(&cell)
...
pub(crate) struct PyCellTransform(molframe::CellTransform);

#[pymethods]
impl PyCellTransform {
    #[new]
    fn new(cell: &PyUnitCell) -> PyResult<Self> {
        molframe::CellTransform::new(&cell.cell)
            .map(Self)
            .map_err(value_error)
    }
...
pub(crate) fn space_group_by_number(py: Python<'_>, hall_number: u16) -> PyResult<PySpaceGroup> {
    py.detach(move || -> PyResult<PySpaceGroup> {
        molframe::space_group_setting(hall_number)
...
pub(crate) fn space_group_by_symbol(py: Python<'_>, hall_symbol: &str) -> PyResult<PySpaceGroup> {
    py.detach(move || -> PyResult<PySpaceGroup> {
        molframe::space_group_by_hall(hall_symbol)
            .map(PySpaceGroup::from)
            .map_err(value_error)
    })
}
...
pub(crate) fn space_group_settings(
    ...
    py.detach(move || -> PyResult<Vec<PySpaceGroup>> {
        molframe::space_group_settings(international_number)
...
impl From<&molframe::SpaceGroupSetting> for PySpaceGroup {
    fn from(value: &molframe::SpaceGroupSetting) -> Self {
        Self {
...
impl From<&molframe::SymmetryOperation> for PySymmetryOperation {
    fn from(value: &molframe::SymmetryOperation) -> Self {
        Self {
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add("ASSEMBLIES_EXTENSION", molframe::xtal::ASSEMBLIES_EXTENSION)?;
    module.add(
        "INSTANCE_ID_ANNOTATION",
        molframe::xtal::INSTANCE_ID_ANNOTATION,
    )?;
    module.add("NCS_EXTENSION", molframe::xtal::NCS_EXTENSION)?;
    module.add("SYMMETRY_EXTENSION", molframe::xtal::SYMMETRY_EXTENSION)?;
