...
pub(crate) fn lower_assemblies(
    ...
    py.detach(|| molframe::xtal::lower_assemblies(&document.inner))
        .map(PyAssemblySet)
...
pub(crate) fn lower_symmetry(py: Python<'_>, document: &PyCifDocument) -> PyResult<PySymmetrySet> {
    ...
    py.detach(|| molframe::xtal::lower_symmetry(&document.inner))
        .map(PySymmetrySet)
        .map_err(|findings| read_error(py, &findings))
}
...
fn neighbors_of(
    ...
    let candidate_limit = match limit {
        Some(limit) => limit,
        None => molframe::xtal::DEFAULT_CRYSTAL_IMAGE_LIMIT,
    };
    let options = molframe::xtal::CrystalNeighborOptions {
        backend: backend.into(),
        candidate_limit,
        ..molframe::xtal::CrystalNeighborOptions::default()
    };
    let result = molframe::xtal::collect_crystal_neighbors(
        structure.structure(),
        &symmetry.0,
        model,
        cutoff,
