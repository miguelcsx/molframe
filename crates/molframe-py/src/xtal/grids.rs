...
impl From<molframe::xtal::CubeAtom> for PyCubeAtom {
    fn from(value: molframe::xtal::CubeAtom) -> Self {
        Self {
            number: value.number,
            charge: value.charge,
...
impl From<molframe::xtal::CubeGrid> for PyCubeGrid {
    fn from(value: molframe::xtal::CubeGrid) -> Self {
        Self {
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMrcBlockOptions(molframe::xtal::MrcBlockOptions);

#[pymethods]
impl PyMrcBlockOptions {
    #[new]
    #[pyo3(signature = (*, memory_limit_bytes=molframe::xtal::DEFAULT_MRC_BLOCK_MEMORY_LIMIT_BYTES))]
    fn new(memory_limit_bytes: usize) -> Self {
        Self(molframe::xtal::MrcBlockOptions::default().with_memory_limit(memory_limit_bytes))
    }
    #[getter]
...
#[derive(Clone, Debug)]
pub(crate) struct PyMrcMapDescriptor(molframe::xtal::MrcMapDescriptor);
...
pub(crate) struct PyMrcBlockReader {
    inner: molframe::xtal::MrcBlockReader<std::fs::File>,
    values: Vec<f32>,
}
...
impl PyMrcBlockReader {
    ...
    fn new(path: PathBuf, options: Option<PyMrcBlockOptions>) -> PyResult<Self> {
        let options = match options {
            Some(value) => value.0,
            None => molframe::xtal::MrcBlockOptions::default(),
        };
        molframe::xtal::MrcBlockReader::open(path, options)
            .map(|inner| Self {
                inner,
                values: Vec::new(),
            })
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add(
        ...
        molframe::xtal::DEFAULT_MRC_BLOCK_MEMORY_LIMIT_BYTES,
    )?;
