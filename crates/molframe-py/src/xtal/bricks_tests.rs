...
fn payload() -> molframe::xtal::ScalarBrickPayload {
    let map = molframe::xtal::DensityMap {
        dimensions: [4, 4, 4],
        starts: [0; 3],
        sampling: [4; 3],
        cell: UnitCell {
    ...
    let reader = molframe::xtal::MrcBlockReader::new(
        Cursor::new(bytes),
        molframe::xtal::MrcBlockOptions::default(),
    )
    .expect("test reader opens");
    let mut provider = molframe::xtal::MrcBrickProvider::new(
        reader,
        molframe::DatasetId::new(1),
        molframe::ChunkId::new(2),
        molframe::xtal::MapBrickId::new(3),
        molframe::xtal::MrcBrickOptions {
            interior: [2; 3],
            halo: 1,
            generation: 4,
            budget: molframe::xtal::MrcBrickBudget::default(),
        },
    )
    .expect("test provider builds");
    provider
        .read_brick(molframe::xtal::MapBrickId::new(3))
