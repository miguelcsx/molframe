...
pub(crate) fn cad_score(
    ...
    py.detach(move || molframe::compare::cad_score(&reference, &model))
        .map(Into::into)
...
pub(crate) fn contact_map_similarity(
    ...
    py.detach(move || molframe::compare::contact_map_similarity(&first, &second))
        .into()
}
...
pub(crate) fn equivalent_atom_mappings(
    ...
    py.detach(move || molframe::compare::equivalent_atom_mappings(&component, limit))
        .map(|values| values.into_iter().map(Into::into).collect())
        .map_err(value_error)
}
...
pub(crate) fn ligand_symmetry_rmsd(
    ...
    let component = component(dictionary, component_id)?;
    py.detach(move || molframe::compare::ligand_symmetry_rmsd(reference, model, &component, limit))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn analyse_cad_score(
    ...
    let analysis = py
        .detach(move || molframe::compare::governed_cad_score(&reference, &model, &policy))
        .map_err(value_error)?;
...
pub(crate) fn analyse_cad_contact_areas(
    ...
    let analysis = py
        .detach(move || {
            molframe::compare::governed_cad_contact_areas(
                positions,
                radius_values,
                residue_values,
                probe,
...
pub(crate) fn analyse_contact_map_similarity(
    ...
    let analysis = py
        .detach(move || {
            molframe::compare::governed_contact_map_similarity(&first, &second, &policy)
        })
        .map_err(value_error)?;
    analysis_with_value(py, analysis, |py, value| {
...
pub(crate) fn analyse_equivalent_atom_mappings(
    ...
    let analysis = py
        .detach(move || {
            molframe::compare::governed_equivalent_atom_mappings(&component, limit, &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_ligand_symmetry_rmsd(
    ...
    let analysis = py
        .detach(move || {
            molframe::compare::governed_ligand_symmetry_rmsd(
...
fn component(
    dictionary: &PyComponentDictionary,
    component_id: &str,
) -> PyResult<std::sync::Arc<molframe::Component>> {
    molframe::ComponentProvider::get(dictionary.0.as_ref(), component_id)
        .map_err(value_error)?
        .ok_or_else(|| PyKeyError::new_err(component_id.to_owned()))
}

fn native_areas(values: Vec<PyContactArea>) -> Vec<molframe::compare::ContactArea> {
    values
        .into_iter()
        .map(|value| molframe::compare::ContactArea {
            first: value.first,
            second: value.second,
            area: value.area,
        })
...
impl From<molframe::compare::ContactArea> for PyContactArea {
    fn from(value: molframe::compare::ContactArea) -> Self {
...
fn mapping_list(
    py: Python<'_>,
    values: Vec<molframe::compare::EquivalentAtomMapping>,
) -> PyResult<Py<PyAny>> {
    let items = values
        .into_iter()
...
impl From<molframe::compare::CadContact> for PyCadContact {
    fn from(value: molframe::compare::CadContact) -> Self {
        Self {
            first: value.first,
            second: value.second,
...
impl From<molframe::compare::LocalCad> for PyLocalCad {
    fn from(value: molframe::compare::LocalCad) -> Self {
        Self {
...
impl From<molframe::compare::CadScore> for PyCadScore {
    fn from(value: molframe::compare::CadScore) -> Self {
        Self {
            score: value.score,
            reference_area: value.reference_area,
            lost_area: value.lost_area,
...
impl From<molframe::compare::ContactSimilarity> for PyContactSimilarity {
    fn from(value: molframe::compare::ContactSimilarity) -> Self {
        Self {
            shared: value.shared,
            union: value.union,
...
impl From<molframe::compare::EquivalentAtomMapping> for PyEquivalentAtomMapping {
    fn from(value: molframe::compare::EquivalentAtomMapping) -> Self {
        Self {
...
impl From<molframe::compare::LigandRmsd> for PyLigandRmsd {
    fn from(value: molframe::compare::LigandRmsd) -> Self {
        Self {
            rmsd: value.rmsd,
