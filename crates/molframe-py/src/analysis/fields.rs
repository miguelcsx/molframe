...
impl PyLinearDensityOptions {
    pub(crate) fn native(&self) -> molframe::analysis::LinearDensityOptions {
        molframe::analysis::LinearDensityOptions {
            axis: self.axis.into(),
    ...
    pub(crate) fn from_native(options: molframe::analysis::LinearDensityOptions) -> Self {
        Self {
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyDensityGridSpec(molframe::analysis::DensityGridSpec);

#[pymethods]
impl PyDensityGridSpec {
    #[new]
    fn new(origin: [f32; 3], spacing: [f32; 3], shape: [usize; 3]) -> Self {
        Self(molframe::analysis::DensityGridSpec {
            origin,
            spacing,
            shape,
        })
...
impl PyDensityGridSpec {
    pub(crate) const fn native(&self) -> molframe::analysis::DensityGridSpec {
        self.0
    }

    pub(crate) const fn from_native(spec: molframe::analysis::DensityGridSpec) -> Self {
        Self(spec)
    }
}
...
pub(crate) struct PyDensityGrid {
    spec: molframe::analysis::DensityGridSpec,
    values: Vec<f64>,
    #[pyo3(get)]
    excluded_weight: f64,
}
...
pub(crate) fn linear_density(
    ...
    py.detach(move || {
        molframe::analysis::linear_density(
            positions,
            weight_values,
            molframe::analysis::LinearDensityOptions {
                axis: axis.into(),
...
pub(crate) fn density_map(
    ...
    let weight_values = weights.as_slice()?;
    let spec = spec.0;
    py.detach(move || molframe::analysis::density_map(positions, weight_values, spec))
...
impl From<PyCartesianAxis> for molframe::analysis::CartesianAxis {
...
impl From<molframe::analysis::CartesianAxis> for PyCartesianAxis {
    fn from(value: molframe::analysis::CartesianAxis) -> Self {
        match value {
            molframe::analysis::CartesianAxis::X => Self::X,
            molframe::analysis::CartesianAxis::Y => Self::Y,
            molframe::analysis::CartesianAxis::Z => Self::Z,
        }
    }
}

impl From<molframe::analysis::LinearDensityBin> for PyLinearDensityBin {
    fn from(value: molframe::analysis::LinearDensityBin) -> Self {
        Self {
            lower: value.lower,
            upper: value.upper,
            weight: value.weight,
...
pub(crate) fn linear_density_analysis(
    py: Python<'_>,
    analysis: molframe::Analysis<Vec<molframe::analysis::LinearDensityBin>>,
) -> PyResult<PyAnalysis> {
...
pub(crate) fn density_map_analysis(
    py: Python<'_>,
    analysis: molframe::Analysis<molframe::analysis::DensityGrid>,
) -> PyResult<PyAnalysis> {
