...
pub(crate) struct PyBaseFrame(pub(crate) molframe::analysis::BaseFrame);

#[pymethods]
impl PyBaseFrame {
    #[new]
    fn new(origin: [f64; 3], x: [f64; 3], y: [f64; 3], z: [f64; 3]) -> Self {
        Self(molframe::analysis::BaseFrame { origin, x, y, z })
    }
...
impl PyHelicalOptions {
    const fn native(self) -> molframe::analysis::HelicalOptions {
        molframe::analysis::HelicalOptions {
...
pub(crate) fn helical_parameters(
    ...
    py.detach(move || molframe::analysis::helical_parameters(first.0, second.0, options.native()))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn helical_steps(
    ...
    py.detach(move || molframe::analysis::helical_steps(&frames, options.native()))
...
pub(crate) fn analyse_helical_parameters(
    ...
    let analysis = py
        .detach(move || {
            molframe::analysis::governed_helical_parameters(
                first.0,
                second.0,
                options.native(),
                &policy,
...
pub(crate) fn analyse_helical_steps(
    ...
    let analysis = py
        .detach(move || {
            molframe::analysis::governed_helical_steps(&frames, options.native(), &policy)
        })
        .map_err(value_error)?;
    analysis_with_value(py, analysis, parameter_list)
}

impl From<molframe::analysis::HelicalParameters> for PyHelicalParameters {
    fn from(value: molframe::analysis::HelicalParameters) -> Self {
        Self {
...
fn parameter_value(
    py: Python<'_>,
    value: molframe::analysis::HelicalParameters,
) -> PyResult<Py<PyAny>> {
...
fn parameter_list(
    py: Python<'_>,
    values: Vec<molframe::analysis::HelicalParameters>,
) -> PyResult<Py<PyAny>> {
    let values = values
        .into_iter()
