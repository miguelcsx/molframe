...
pub(crate) fn parse_dssp_output(py: Python<'_>, output: &[u8]) -> PyResult<Vec<PyDsspSegment>> {
    py.detach(move || molframe::analysis::parse_dssp_output(output))
...
pub(crate) fn run_dssp(
    ...
    py.detach(move || molframe::analysis::run_dssp(executable, input))
        .map(|values| values.into_iter().map(PyDsspSegment::from).collect())
        .map_err(|error| PyValueError::new_err(error.to_string()))
}

impl From<molframe::analysis::DsspSegment> for PyDsspSegment {
    fn from(value: molframe::analysis::DsspSegment) -> Self {
        Self {
