...
pub(crate) struct PyBasePairOptions(molframe::analysis::BasePairOptions);
...
impl PyBasePairOptions {
    ...
    fn new(
        ...
        Self(molframe::analysis::BasePairOptions {
            hydrogen_bonds: molframe::analysis::HydrogenBondOptions {
...
impl PyBasePairOptions {
    pub(crate) const fn native(&self) -> molframe::analysis::BasePairOptions {
        self.0
    }

    pub(crate) const fn from_native(value: molframe::analysis::BasePairOptions) -> Self {
        Self(value)
    }
}
...
impl PySurfaceContactOptions {
    pub(crate) fn native_parts(&self) -> (f32, f32, f32, f32, molframe::SpatialBackend) {
        (
            self.tolerance,
            self.probe,
...
impl PyStructure {
    ...
    fn base_pairs(
        ...
        py.detach(move || {
            molframe::analysis::base_pairs(&structure, dictionary.as_ref(), options.0, &context)
        })
    ...
    fn residue_contact_map(
        ...
        let context = context.map_or_else(default_context, PyExecutionContext::native);
        py.detach(move || {
            molframe::analysis::residue_contact_map(
                &structure,
                cutoff,
                minimum_separation,
                backend.into(),
    ...
    fn half_sphere_exposure(
        ...
        let context = context.map_or_else(default_context, PyExecutionContext::native);
        py.detach(move || {
            molframe::analysis::half_sphere_exposure(&structure, radius, backend.into(), &context)
        })
    ...
    fn chain_interface(
        ...
        let context = context.map_or_else(default_context, PyExecutionContext::native);
        py.detach(move || {
            molframe::analysis::chain_interface(
                &structure,
                &first,
                &second,
                cutoff,
    ...
    fn native_contact_fraction(
        ...
        let context = context.map_or_else(default_context, PyExecutionContext::native);
        py.detach(move || {
            molframe::analysis::native_contact_fraction(
                &reference,
                &target,
                cutoff,
                tolerance,
