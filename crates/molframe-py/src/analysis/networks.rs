...
#[derive(Clone, Debug)]
pub(crate) struct PyFragmentReference(molframe::analysis::FragmentReference);

#[pymethods]
impl PyFragmentReference {
    #[new]
    fn new(id: &str, coordinates_array: PyReadonlyArray2<'_, f32>) -> PyResult<Self> {
        Ok(Self(molframe::analysis::FragmentReference {
            id: id.into(),
...
pub(crate) struct PyGnmOptions(molframe::analysis::GnmOptions);
...
impl PyGnmOptions {
    ...
    fn new(
        ...
        Self(molframe::analysis::GnmOptions {
            contact_distance,
            mode_count,
            zero_mode_tolerance,
            memory_limit_bytes,
...
impl PyGnmOptions {
    pub(crate) const fn from_native(value: molframe::analysis::GnmOptions) -> Self {
        Self(value)
    }

    pub(crate) const fn native(self) -> molframe::analysis::GnmOptions {
        self.0
    }
}
...
pub(crate) fn map_fragments(
    ...
    py.detach(move || molframe::analysis::map_fragments(&trace, &library, maximum_rmsd))
...
pub(crate) fn gaussian_network_model(
    ...
    let value = py
        .detach(move || {
            ...
            molframe::analysis::gaussian_network_model(
                structure.positions(),
                &sites,
                options.0,
                periodic_box.as_ref(),
...
impl PyGaussianNetworkModel {
    pub(crate) fn new(
        py: Python<'_>,
        value: molframe::analysis::GaussianNetworkModel,
    ) -> PyResult<Self> {
...
impl From<molframe::analysis::FragmentMatch> for PyFragmentMatch {
    fn from(value: molframe::analysis::FragmentMatch) -> Self {
        Self {
            start: value.start,
