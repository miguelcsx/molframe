...
impl From<PyStreamlineDirection> for molframe::analysis::StreamlineDirection {
...
pub(crate) struct PyStreamlineOptions {
    native: molframe::analysis::StreamlineOptions,
}
...
pub(crate) struct PyVectorFieldGrid {
    native: molframe::analysis::VectorFieldGrid,
}
...
impl PyVectorFieldGrid {
    ...
    fn new(
        ...
        let native = molframe::analysis::VectorFieldGrid::new(origin, spacing, shape, vectors)
            .map_err(value_error)?;
        Ok(Self { native })
    }
...
fn integrate_streamlines(
    ...
    py.detach(move || -> PyResult<Vec<Vec<[f32; 3]>>> {
        molframe::analysis::integrate_streamlines(&field.native, &seeds, options.native)
            .map_err(value_error)
    })
}

fn value_error(error: molframe::analysis::VectorFieldError) -> PyErr {
