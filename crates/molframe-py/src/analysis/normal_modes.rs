...
pub(crate) struct PyAnmOptions(pub(crate) molframe::analysis::AnmOptions);
...
impl PyAnmOptions {
    ...
    fn new(
        ...
        Self(molframe::analysis::AnmOptions {
            contact_distance,
            mode_count,
            zero_mode_tolerance,
            memory_limit_bytes,
...
pub(crate) struct PyAnisotropicNetworkModel {
    ...
    native: molframe::analysis::AnisotropicNetworkModel,
}
...
impl PyAnisotropicNetworkModel {
    fn new(py: Python<'_>, native: molframe::analysis::AnisotropicNetworkModel) -> PyResult<Self> {
...
#[derive(Clone, Debug)]
pub(crate) struct PyNormalMode(pub(crate) molframe::analysis::NormalMode);

#[pymethods]
impl PyNormalMode {
    #[new]
    fn new(index: i32, scale: f64, displacements: PyReadonlyArray2<'_, f64>) -> PyResult<Self> {
        Ok(Self(molframe::analysis::NormalMode {
            index,
            scale,
...
#[derive(Clone, Debug)]
pub(crate) struct PyNormalModeSet(pub(crate) molframe::analysis::NormalModeSet);
...
pub(crate) fn anisotropic_network_model(
    ...
    let model = py
        .detach(move || {
            ...
            molframe::analysis::anisotropic_network_model(
                structure.positions(),
                &sites,
                options.0,
                cell.as_ref(),
...
pub(crate) fn read_nmd(py: Python<'_>, data: &[u8]) -> PyResult<PyNormalModeSet> {
    py.detach(move || -> PyResult<PyNormalModeSet> {
        molframe::analysis::read_nmd(data)
            .map(PyNormalModeSet)
            .map_err(value_error)
    })
}
...
pub(crate) fn write_nmd(py: Python<'_>, set: &PyNormalModeSet) -> String {
    py.detach(move || -> String { molframe::analysis::write_nmd(&set.0) })
}
