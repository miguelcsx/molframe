...
pub(crate) struct PySelectQuery {
    ...
    pub(crate) request: molframe::SelectionRequest,
}
...
impl PySelectQuery {
    ...
    fn new(py: Python<'_>, query: Py<PyAny>, policy: Option<Py<PyAny>>) -> PyResult<Self> {
        ...
        Ok(Self {
            query,
            policy,
            request: molframe::SelectionRequest::from_query(
                query_ref.inner.clone(),
                request_policy,
            ),
        })
    }
    ...
    fn execute(&self, py: Python<'_>, structure: &PyStructure) -> PyResult<PyEvaluation> {
        ...
        match entry.value {
            molframe::PlanValue::Selection(value) => Ok(PyEvaluation::from(*value)),
