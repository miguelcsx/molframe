...
impl PyMeanSquaredDisplacementSeries {
    fn from_native(py: Python<'_>, values: Vec<molframe::traj::MeanSquaredDisplacement>) -> Self {
...
pub(crate) fn execute_operation(
    ...
    match entry.value {
        molframe::PlanValue::Trajectory(value) => analysis_to_python(py, *value),
...
pub(crate) fn value_to_python(
    py: Python<'_>,
    value: molframe::TrajectoryValue,
) -> PyResult<Py<PyAny>> {
    Py::new(py, analysis_to_python(py, value)?).map(Py::into_any)
}

fn analysis_to_python(py: Python<'_>, value: molframe::TrajectoryValue) -> PyResult<PyAnalysis> {
    match value {
        molframe::TrajectoryValue::RmsdToReference(analysis) => {
            analysis_with_value(py, analysis, |py, values| Ok(float_vector(py, values)))
        }
        molframe::TrajectoryValue::MeanSquaredDisplacement(analysis) => {
            analysis_with_value(py, analysis, msd_series)
        }
        molframe::TrajectoryValue::PairwiseFittedRmsd(analysis) => {
            analysis_with_value(py, analysis, dense_matrix)
        }
        molframe::TrajectoryValue::GeneralizedProcrustesMean(analysis) => {
...
fn msd_series(
    py: Python<'_>,
    values: Vec<molframe::traj::MeanSquaredDisplacement>,
) -> PyResult<Py<PyAny>> {
...
fn dense_matrix(
    py: Python<'_>,
    values: molframe::traj::EnsembleDistanceMatrix,
) -> PyResult<Py<PyAny>> {
