...
) -> PyResult<molframe::TrajectoryRequest> {
    match operation {
        PyTrajectoryOperation::RmsdToReference {
            ...
            alignment,
            policy,
        } => Ok(molframe::TrajectoryRequest::RmsdToReference {
        ...
        PyTrajectoryOperation::MeanSquaredDisplacement {
            ...
            maximum_lag,
            policy,
        } => Ok(molframe::TrajectoryRequest::MeanSquaredDisplacement {
            frames: super::super::lowering::retain_frame(py, frames, frame_slots, value)?,
            atoms: atoms
                .as_ref()
                .map(|value| {
        ...
        PyTrajectoryOperation::PairwiseFittedRmsd {
            ...
            memory_limit,
            policy,
        } => Ok(molframe::TrajectoryRequest::PairwiseFittedRmsd {
            frames: super::super::lowering::retain_frame(py, frames, frame_slots, value)?,
        ...
        PyTrajectoryOperation::GeneralizedProcrustesMean {
            ...
            maximum_iterations,
            policy,
        } => Ok(molframe::TrajectoryRequest::GeneralizedProcrustesMean {
            frames: super::super::lowering::retain_frame(py, frames, frame_slots, value)?,
            tolerance: *tolerance,
...
pub(crate) fn policy_or_default(policy: Option<PyAnalysisPolicy>) -> molframe::AnalysisPolicy {
...
pub(crate) fn from_dict(config: &Bound<'_, PyDict>) -> PyResult<PyTrajectoryOperation> {
    ...
    let policy = match config.get_item("policy")? {
        ...
        _ => molframe::AnalysisPolicy::default(),
    };
    let result = match tag.as_str() {
        ...
        "pairwise_fitted_rmsd" => Ok(PyTrajectoryOperation::PairwiseFittedRmsd {
            ...
            memory_limit: config
                ...
                .map_or(Ok(molframe::traj::DEFAULT_PAIRWISE_MEMORY_LIMIT), |value| {
                    value.extract()
                })?,
            policy,
        }),
        "generalized_procrustes_mean" => Ok(PyTrajectoryOperation::GeneralizedProcrustesMean {
            ...
            tolerance: config
                .get_item("tolerance")?
                .map_or(Ok(molframe::traj::DEFAULT_PROCRUSTES_TOLERANCE), |value| {
                    value.extract()
                })?,
            maximum_iterations: config.get_item("maximum_iterations")?.map_or(
                Ok(molframe::traj::DEFAULT_PROCRUSTES_MAXIMUM_ITERATIONS),
                |value| value.extract(),
            )?,
            policy,
        }),
