...
enum Operation {
    ...
    Structure(molframe::StructureRequest),
...
fn execute_native(
    ...
    context: Option<&crate::core::execution::PyExecutionContext>,
) -> PyResult<molframe::PlanResult> {
    let mut native = molframe::Plan::new();
...
fn execute_lowered<'py>(
    ...
    let inputs = coordinates
        .iter()
        .map(|positions| molframe::CoordinateInput { positions })
        .collect::<Vec<_>>();
    ...
    let scalars = scalar_values
        .iter()
        .map(|values| molframe::ScalarInput { values })
        .collect::<Vec<_>>();
    ...
    let floats = float_values
        .iter()
        .map(|values| molframe::FloatInput { values })
        .collect::<Vec<_>>();
    ...
    let masks = mask_values
        .iter()
        .map(|values| molframe::MaskInput { values })
        .collect::<Vec<_>>();
    ...
    let indices = index_values
        .iter()
        .map(|indices| molframe::IndexInput { indices })
        .collect::<Vec<_>>();
...
) -> PyResult<Vec<molframe::FrameInput<'py>>> {
    arrays
        .iter()
...
fn execute_comparison(
    ...
    match entry.value {
        molframe::PlanValue::Comparison(value) => Ok(value.value),
...
impl PyAnalysisPolicy {
    fn new_default() -> Self {
        Self {
            inner: molframe::AnalysisPolicy::default(),
        }
    }
}
