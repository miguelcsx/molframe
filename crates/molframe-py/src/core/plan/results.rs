...
fn plan_value(
    ...
    match value {
        ...
        molframe::PlanValue::Rmsd(value) => Ok(PyFloat::new(py, value).unbind().into_any()),
        molframe::PlanValue::Comparison(value) => {
            Ok(PyFloat::new(py, value.value).unbind().into_any())
        }
        molframe::PlanValue::Selection(value) => {
        ...
        molframe::PlanValue::BondInference(report) => Ok(Py::new(
            py,
            crate::facade::PyBondInferenceReport::from_native(*report),
        )?
        .into_any()),
        molframe::PlanValue::Geometry(value) => geometry::value_to_python(py, *value),
        molframe::PlanValue::Spatial(value) => match *value {
            molframe::SpatialValue::NeighborPairs(values) => {
                Ok(Py::new(py, spatial::neighbor_table_from_pairs(py, values)?)?.into_any())
            }
            molframe::SpatialValue::AtomsWithin(value) => {
                Ok(spatial::atom_indices_to_python(py, value)
                    .unbind()
                    .into_any())
            }
        },
        molframe::PlanValue::Physical(value) => physical::value_to_python_public(py, *value)
            .map(|value| Py::new(py, value).map(pyo3::Py::into_any))?,
        molframe::PlanValue::Surface(value) => match *value {
            ...
            molframe::SurfaceValue::BuriedSurface(value) => {
                Ok(Py::new(py, crate::surface_types::PyBuriedSurface::from(value))?.into_any())
            }
        },
        molframe::PlanValue::Trajectory(value) => trajectory::value_to_python(py, *value),
    }
}
