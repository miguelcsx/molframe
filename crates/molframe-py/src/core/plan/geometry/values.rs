...
pub(crate) fn execute_operation(
    ...
    match entry.value {
        molframe::PlanValue::Geometry(value) => value_to_python(py, *value),
...
pub(crate) fn value_to_python(
    py: Python<'_>,
    value: molframe::GeometryValue,
) -> PyResult<Py<PyAny>> {
    match value {
        molframe::GeometryValue::Centroid(value) | molframe::GeometryValue::CentreOfMass(value) => {
            Ok(value.into_pyobject(py)?.unbind().into_any())
        }
        molframe::GeometryValue::RadiusOfGyration(value)
        | molframe::GeometryValue::Asphericity(value) => {
            Ok(value.into_pyobject(py)?.unbind().into_any())
        }
        molframe::GeometryValue::InertiaTensor(value) => {
            Ok(value.into_pyobject(py)?.unbind().into_any())
        }
        molframe::GeometryValue::PrincipalAxes(value)
        | molframe::GeometryValue::GyrationAxes(value) => match value {
            Some(value) => Ok(Py::new(py, PyAxes::from(value))?.into_any()),
            None => Ok(py.None()),
        },
        molframe::GeometryValue::DistanceMatrix(value) => {
            Ok(Py::new(py, PyDistanceMatrix::from_native(value))?.into_any())
        }
        molframe::GeometryValue::Rmsf(value) => {
