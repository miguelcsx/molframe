...
pub(crate) fn to_native<'py>(
...
) -> PyResult<molframe::GeometryRequest> {
    ...
    Ok(match operation {
        PyGeometryOperation::Centroid { positions: value } => molframe::GeometryRequest::Centroid {
            positions: positions(value)?,
        },
        PyGeometryOperation::CentreOfMass {
            positions: value,
            masses: weights,
        } => molframe::GeometryRequest::CentreOfMass {
            positions: positions(value)?,
        ...
        PyGeometryOperation::RadiusOfGyration {
            positions: value,
            masses: weights,
        } => molframe::GeometryRequest::RadiusOfGyration {
            positions: positions(value)?,
            masses: masses(weights)?,
        },
        PyGeometryOperation::InertiaTensor {
            positions: value,
            masses: weights,
        } => molframe::GeometryRequest::InertiaTensor {
            positions: positions(value)?,
            masses: masses(weights)?,
        },
        PyGeometryOperation::PrincipalAxes {
            ...
            masses: weights,
            options,
        } => molframe::GeometryRequest::PrincipalAxes {
            positions: positions(value)?,
            masses: masses(weights)?,
            options: *options,
        },
        PyGeometryOperation::Asphericity {
            positions: value,
            options,
        } => molframe::GeometryRequest::Asphericity {
            positions: positions(value)?,
            options: *options,
        },
        PyGeometryOperation::GyrationAxes {
            positions: value,
            options,
        } => molframe::GeometryRequest::GyrationAxes {
            positions: positions(value)?,
            options: *options,
        },
        PyGeometryOperation::DistanceMatrix { positions: value } => {
            molframe::GeometryRequest::DistanceMatrix {
                positions: positions(value)?,
            }
        }
        PyGeometryOperation::DistanceMatrixBetween { left, right } => {
            molframe::GeometryRequest::DistanceMatrixBetween {
                left: positions(left)?,
                right: positions(right)?,
            }
        }
        PyGeometryOperation::Rmsf { frames: value } => molframe::GeometryRequest::Rmsf {
...
pub(crate) fn eigen_from_dict(
    config: &Bound<'_, PyDict>,
    tag: &str,
    make: fn(Py<PyAny>, molframe::EigenOptions) -> PyGeometryOperation,
) -> PyResult<PyGeometryOperation> {
    require_operation_tag(config, tag)?;
    let operation = make(
        super::super::required(config, "positions")?.unbind(),
...
fn eigen_options(config: &Bound<'_, PyDict>) -> PyResult<molframe::EigenOptions> {
    match config.get_item("options")? {
        Some(value) => Ok(value.extract::<crate::geometry::PyEigenOptions>()?.inner),
        None => Ok(molframe::EigenOptions::standard()),
    }
}
