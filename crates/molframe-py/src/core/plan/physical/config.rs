...
fn policy_from_dict(config: &Bound<'_, PyDict>) -> PyResult<molframe::AnalysisPolicy> {
    match config.get_item("policy")? {
        Some(value) if !value.is_none() => Ok(value.extract::<PyAnalysisPolicy>()?.inner),
        _ => Ok(molframe::AnalysisPolicy::default()),
    }
}
...
fn selection_object(py: Python<'_>, selection: &molframe::AtomSelection) -> PyResult<Py<PyAny>> {
    Ok(Py::new(
        py,
        PySelection {
