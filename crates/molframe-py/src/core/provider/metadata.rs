...
impl From<molframe::PayloadKind> for PyPayloadKind {
    fn from(value: molframe::PayloadKind) -> Self {
        match value {
            molframe::PayloadKind::Structure => Self::Structure,
            molframe::PayloadKind::BondTopology => Self::BondTopology,
            molframe::PayloadKind::Property => Self::Property,
            molframe::PayloadKind::Frame => Self::Frame,
        }
    }
}

impl From<PyPayloadKind> for molframe::PayloadKind {
...
pub(crate) struct PyChunkLayout(pub(crate) molframe::ChunkLayout);

#[pymethods]
impl PyChunkLayout {
    #[staticmethod]
    fn regular(rows_per_chunk: u32) -> Self {
        Self(molframe::ChunkLayout::Regular { rows_per_chunk })
    }

    #[staticmethod]
    fn source_defined(target_rows: u32) -> Self {
        Self(molframe::ChunkLayout::SourceDefined { target_rows })
    }
    ...
    fn is_regular(&self) -> bool {
        matches!(self.0, molframe::ChunkLayout::Regular { .. })
    }
}

#[pyclass(name = "ChunkDescriptor", frozen, from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyChunkDescriptor(pub(crate) molframe::ChunkDescriptor);
...
impl PyChunkDescriptor {
    ...
    fn new(
        ...
        molframe::ChunkDescriptor::new(dataset.0, chunk.0, logical_start.0, rows)
            .map(Self)
            .map_err(value_error)
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyDatasetDescriptor(pub(crate) molframe::DatasetDescriptor);
...
impl PyDatasetDescriptor {
    ...
    fn regular(
        ...
        molframe::DatasetDescriptor::regular(
            dataset.0,
            payload.into(),
            logical_rows,
            rows_per_chunk,
    ...
    fn source_defined(
        ...
        molframe::DatasetDescriptor::source_defined(
            dataset.0,
            payload.into(),
            logical_rows,
            chunk_count,
...
#[derive(Clone, Debug)]
pub(crate) struct PyDatasetCatalog(pub(crate) molframe::DatasetCatalog);

#[pymethods]
impl PyDatasetCatalog {
    #[new]
    fn new(datasets: Vec<PyDatasetDescriptor>) -> PyResult<Self> {
        molframe::DatasetCatalog::new(datasets.into_iter().map(|item| item.0).collect())
            .map(Self)
            .map_err(value_error)
    }
