...
macro_rules! static_alphabet {
    ($rust:ident, $python:ident, $name:literal) => {
        ...
        pub(crate) struct $python(pub(crate) molframe::seq::$rust);

        #[pymethods]
        impl $python {
            #[new]
            fn new() -> Self {
                Self(molframe::seq::$rust)
            }

            fn symbols<'py>(&self, py: Python<'py>) -> Bound<'py, PyBytes> {
                bytes(
                    py,
                    <molframe::seq::$rust as molframe::seq::Alphabet>::symbols(&self.0),
                )
            }

            fn encode_symbol(&self, symbol: u8) -> Option<u8> {
                <molframe::seq::$rust as molframe::seq::Alphabet>::encode_symbol(&self.0, symbol)
            }

            fn decode_symbol(&self, code: u8) -> Option<u8> {
                <molframe::seq::$rust as molframe::seq::Alphabet>::decode_symbol(&self.0, code)
            }
            ...
            fn encode<'py>(
                ...
                <molframe::seq::$rust as molframe::seq::Alphabet>::encode(&self.0, &symbols)
                    .as_deref()
                    .map(|value| bytes(py, value))
            }

            fn decode<'py>(&self, py: Python<'py>, codes: Vec<u8>) -> Option<Bound<'py, PyBytes>> {
                <molframe::seq::$rust as molframe::seq::Alphabet>::decode(&self.0, &codes)
                    .as_deref()
                    .map(|value| bytes(py, value))
            }

            fn len(&self) -> usize {
                <molframe::seq::$rust as molframe::seq::Alphabet>::len(&self.0)
            }

            fn is_empty(&self) -> bool {
                <molframe::seq::$rust as molframe::seq::Alphabet>::is_empty(&self.0)
            }
...
#[derive(Clone, Debug)]
pub(crate) struct PyCustomAlphabet(pub(crate) molframe::seq::CustomAlphabet);

#[pymethods]
impl PyCustomAlphabet {
    #[new]
    fn new(symbols: Vec<u8>) -> PyResult<Self> {
        molframe::seq::CustomAlphabet::new(symbols.into_boxed_slice())
            .map(Self)
    ...
    fn symbols<'py>(&self, py: Python<'py>) -> Bound<'py, PyBytes> {
        bytes(
            py,
            <molframe::seq::CustomAlphabet as molframe::seq::Alphabet>::symbols(&self.0),
        )
    }

    fn encode_symbol(&self, symbol: u8) -> Option<u8> {
        molframe::seq::Alphabet::encode_symbol(&self.0, symbol)
    }

    fn decode_symbol(&self, code: u8) -> Option<u8> {
        molframe::seq::Alphabet::decode_symbol(&self.0, code)
    }

    fn encode<'py>(&self, py: Python<'py>, symbols: Vec<u8>) -> Option<Bound<'py, PyBytes>> {
        molframe::seq::Alphabet::encode(&self.0, &symbols)
            .as_deref()
            .map(|value| bytes(py, value))
    }

    fn decode<'py>(&self, py: Python<'py>, codes: Vec<u8>) -> Option<Bound<'py, PyBytes>> {
        molframe::seq::Alphabet::decode(&self.0, &codes)
            .as_deref()
            .map(|value| bytes(py, value))
    }

    fn len(&self) -> usize {
        molframe::seq::Alphabet::len(&self.0)
    }

    fn is_empty(&self) -> bool {
        molframe::seq::Alphabet::is_empty(&self.0)
    }
}

#[derive(Clone, Debug)]
enum AlphabetValue {
    Protein(molframe::seq::ProteinAlphabet),
    Dna(molframe::seq::DnaAlphabet),
    Rna(molframe::seq::RnaAlphabet),
    Nucleotide(molframe::seq::NucleotideAlphabet),
    Custom(molframe::seq::CustomAlphabet),
}
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add(
        ...
        Py::new(
            module.py(),
            PyProteinAlphabet(molframe::seq::ProteinAlphabet),
        )?,
    )?;
    module.add(
        "DNA",
        Py::new(module.py(), PyDnaAlphabet(molframe::seq::DnaAlphabet))?,
    )?;
    module.add(
        "RNA",
        Py::new(module.py(), PyRnaAlphabet(molframe::seq::RnaAlphabet))?,
    )?;
    module.add(
        "NUCLEOTIDE",
        Py::new(
            module.py(),
            PyNucleotideAlphabet(molframe::seq::NucleotideAlphabet),
        )?,
    )?;
    Ok(())
}
