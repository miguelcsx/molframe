...
pub(crate) struct PyKmerStorage(pub(crate) molframe::seq::KmerStorage);

#[pymethods]
impl PyKmerStorage {
    #[staticmethod]
    fn exact() -> Self {
        Self(molframe::seq::KmerStorage::Exact)
    }
    ...
    fn bucketed(buckets: usize) -> PyResult<Self> {
        ...
        Ok(Self(molframe::seq::KmerStorage::Bucketed { buckets }))
    }

    #[getter]
    fn buckets(&self) -> Option<usize> {
        match self.0 {
            molframe::seq::KmerStorage::Exact => None,
            molframe::seq::KmerStorage::Bucketed { buckets } => Some(buckets),
        }
    }

    #[getter]
    fn is_exact(&self) -> bool {
        matches!(self.0, molframe::seq::KmerStorage::Exact)
    }
}

#[pyclass(name = "SeedPattern", frozen, from_py_object)]
#[derive(Clone, Debug)]
pub(crate) struct PySeedPattern(pub(crate) molframe::seq::SeedPattern);

#[pymethods]
impl PySeedPattern {
    #[new]
    fn new(mask: Vec<bool>) -> PyResult<Self> {
        molframe::seq::SeedPattern::new(&mask)
            .map(Self)
...
#[derive(Clone, Debug)]
pub(crate) struct PyKmerTableOptions(pub(crate) molframe::seq::KmerTableOptions);
...
impl PyKmerTableOptions {
    ...
    fn new(k: usize, pattern: Option<PySeedPattern>, storage: Option<PyKmerStorage>) -> Self {
        Self(molframe::seq::KmerTableOptions {
            k,
            pattern: pattern.map(|value| value.0),
            storage: storage.map_or(molframe::seq::KmerStorage::Exact, |value| value.0),
        })
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyKmerTable(pub(crate) molframe::seq::KmerTable);
...
impl PyKmerTable {
    ...
    fn build(sequences: Vec<Vec<u8>>, options: &PyKmerTableOptions) -> PyResult<Self> {
        ...
        molframe::seq::KmerTable::build(&borrowed, &options.0)
            .map(Self)
...
impl From<molframe::seq::KmerHit> for PyKmerHit {
    fn from(value: molframe::seq::KmerHit) -> Self {
        Self {
