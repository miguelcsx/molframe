...
use super::sequence_errors::MsaError;
...
create_exception!(_native, NewickError, PyValueError);
create_exception!(_native, RerootError, PyValueError);

#[pyclass(name = "MsaOptions", frozen, from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMsaOptions(molframe::seq::MsaOptions);
...
impl PyMsaOptions {
    ...
    fn new(scoring: PyScoring, refinement_passes: usize, memory_limit_bytes: usize) -> Self {
        Self(
            molframe::seq::MsaOptions::progressive(scoring.inner())
    ...
    #[staticmethod]
    fn progressive(scoring: PyScoring) -> Self {
        Self(molframe::seq::MsaOptions::progressive(scoring.inner()))
    }
...
parser!(parse_fasta, molframe::seq::parse_fasta);
parser!(parse_clustal, molframe::seq::parse_clustal);
parser!(parse_phylip, molframe::seq::parse_phylip);
parser!(parse_stockholm, molframe::seq::parse_stockholm);
writer!(write_fasta, molframe::seq::write_fasta);
writer!(write_clustal, molframe::seq::write_clustal);
writer!(write_phylip, molframe::seq::write_phylip);
writer!(write_stockholm, molframe::seq::write_stockholm);

#[pyfunction]
pub(crate) fn parse_a3m(py: Python<'_>, text: String) -> PyResult<Vec<PyFastaRecord>> {
    py.detach(move || molframe::seq::parse_a3m(&text))
...
pub(crate) fn write_a3m(py: Python<'_>, records: Vec<PyFastaRecord>) -> PyResult<String> {
    ...
    py.detach(|| molframe::seq::write_a3m(&records))
        .map_err(|error| PyValueError::new_err(error.to_string()))
}
...
pub(crate) fn a3m_match_columns(py: Python<'_>, sequence: Vec<u8>) -> Vec<u8> {
    ...
    py.detach(|| molframe::seq::a3m_match_columns(&sequence))
}

#[pyfunction]
pub(crate) fn a2m_match_columns(py: Python<'_>, sequence: Vec<u8>) -> Vec<u8> {
    let sequence = sequence.into_boxed_slice();
    py.detach(|| molframe::seq::a2m_match_columns(&sequence))
}

#[pyfunction]
pub(crate) fn kmer_counts(py: Python<'_>, sequence: Vec<u8>, k: usize) -> Vec<(Vec<u8>, u32)> {
    let sequence = sequence.into_boxed_slice();
    py.detach(|| molframe::seq::kmer_counts(&sequence, k))
}
...
pub(crate) fn minimizers(
    ...
    let sequence = sequence.into_boxed_slice();
    py.detach(|| molframe::seq::minimizers(&sequence, k, window))
}
...
pub(crate) fn multiple_sequence_alignment(
    ...
    py.detach(|| {
        ...
        molframe::seq::msa(&borrowed, scoring.inner())
    })
    .map_err(msa_error)
}
...
pub(crate) fn progressive_msa(
    ...
    py.detach(|| {
        let borrowed: Vec<&[u8]> = sequences.iter().map(Vec::as_slice).collect();
        molframe::seq::progressive_msa(&borrowed, options.0)
    })
    .map_err(msa_error)
}

fn msa_error(error: molframe::seq::MsaError) -> PyErr {
    MsaError::new_err(error.to_string())
}
...
pub(crate) fn align_global_banded(
    ...
    py.detach(move || molframe::seq::global_banded(&left, &right, scoring.inner(), band))
        .map(Into::into)
...
pub(crate) fn global_score(
    ...
    let left = left.into_boxed_slice();
    let right = right.into_boxed_slice();
    py.detach(move || molframe::seq::global_score(&left, &right, scoring.inner()))
        .map_err(super::sequence::align_error)
}
...
pub(crate) fn seed_and_extend(
    ...
    py.detach(move || molframe::seq::seed_and_extend(&query, &reference, k, scoring.inner()))
        .map(|alignment| alignment.map(Into::into))
        .map_err(super::sequence::align_error)
}

#[pyclass(name = "MatrixIdentity", frozen, skip_from_py_object)]
#[derive(Clone, Debug)]
pub(crate) struct PyMatrixIdentity {
    #[pyo3(get)]
    name: String,
    #[pyo3(get)]
    version: String,
...
#[derive(Clone, Debug)]
pub(crate) struct PySubstitutionMatrix(pub(crate) molframe::seq::SubstitutionMatrix);
...
impl PySubstitutionMatrix {
    ...
    fn get(&self, left: u8, right: u8) -> i32 {
    ...
    fn identity(&self) -> PyMatrixIdentity {
        let value = self.0.identity();
        PyMatrixIdentity {
            name: value.name().to_owned(),
            version: value.version().to_owned(),
            source: value.source().to_owned(),
            retrieved: value.retrieved().to_owned(),
        }
    }
}
...
pub(crate) fn blosum62(py: Python<'_>) -> PySubstitutionMatrix {
    py.detach(move || -> PySubstitutionMatrix { PySubstitutionMatrix(molframe::seq::blosum62()) })
}
...
matrix_aligner!(align_global_matrix, molframe::seq::global_matrix);
matrix_aligner!(align_local_matrix, molframe::seq::local_matrix);
matrix_aligner!(align_semi_global_matrix, molframe::seq::semi_global_matrix);

#[pyclass(name = "Tree", frozen, skip_from_py_object)]
#[derive(Clone, Debug)]
pub(crate) struct PyTree(pub(crate) molframe::seq::Tree);
...
#[pyclass(name = "TraversalOrder", frozen, eq, eq_int, from_py_object)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum PyTraversalOrder {
    Preorder,
    Postorder,
}
...
impl PyTree {
    ...
    fn from_newick(text: &str) -> PyResult<Self> {
        molframe::seq::Tree::from_newick(text)
            .map(Self)
            .map_err(|error| NewickError::new_err(format!("invalid Newick tree: {error:?}")))
    }
    ...
    fn ladderize(&self, direction: PyLadderDirection) -> Self {
        self.ladderized(direction)
    }

    fn traverse(&self, order: PyTraversalOrder) -> Vec<Self> {
        self.0
            .traverse(order.into())
            .into_iter()
            .cloned()
    ...
    fn rerooted_at_leaf(&self, leaf: &str, fraction_from_leaf: f64) -> PyResult<Self> {
        self.0
            ...
            .map(Self)
            .map_err(|error| RerootError::new_err(error.to_string()))
    }

    fn reroot_at_leaf(&self, leaf: &str, fraction_from_leaf: f64) -> PyResult<Self> {
        self.rerooted_at_leaf(leaf, fraction_from_leaf)
    }
}

impl From<PyLadderDirection> for molframe::seq::LadderDirection {
...
impl From<PyTraversalOrder> for molframe::seq::TraversalOrder {
    fn from(value: PyTraversalOrder) -> Self {
        match value {
            PyTraversalOrder::Preorder => Self::Preorder,
            PyTraversalOrder::Postorder => Self::Postorder,
        }
    }
}

pub(crate) fn register_tree_types(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add("NewickError", module.py().get_type::<NewickError>())?;
    module.add("RerootError", module.py().get_type::<RerootError>())?;
    module.add_class::<PyTraversalOrder>()?;
    Ok(())
}
...
pub(crate) fn neighbor_joining(
    ...
    tree_from_distances(py, labels, distances, molframe::seq::neighbor_joining)
}
...
pub(crate) fn upgma(
    ...
    tree_from_distances(py, labels, distances, molframe::seq::upgma)
}
...
fn tree_from_distances(
    ...
    labels: Vec<String>,
    distances: PyReadonlyArray2<'_, f64>,
    kernel: fn(&[&str], &[Vec<f64>]) -> Option<molframe::seq::Tree>,
) -> PyResult<Option<PyTree>> {
...
impl From<molframe::seq::FastaRecord> for PyFastaRecord {
    fn from(value: molframe::seq::FastaRecord) -> Self {
        Self {
            id: value.id,
...
impl From<PyFastaRecord> for molframe::seq::FastaRecord {
