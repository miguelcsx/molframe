...
pub(crate) struct PyScoring(molframe::seq::Scoring);

impl PyScoring {
    pub(crate) const fn inner(self) -> molframe::seq::Scoring {
        self.0
    }
}
...
impl PyScoring {
    ...
    #[staticmethod]
    fn simple() -> Self {
        Self(molframe::seq::Scoring::simple())
    }
...
macro_rules! aligner {
    ($name:ident, $python:literal, $kernel:path) => {
        ...
        pub(crate) fn $name(
            ...
            let scoring = scoring.map_or_else(molframe::seq::Scoring::simple, |value| value.0);
...
aligner!(global, "align_global", molframe::seq::global);
aligner!(local, "align_local", molframe::seq::local);
aligner!(semi_global, "align_semi_global", molframe::seq::semi_global);

pub(super) fn align_error(error: molframe::seq::AlignError) -> PyErr {
    AlignError::new_err(error.to_string())
}

impl From<molframe::seq::Alignment> for PyAlignment {
    fn from(value: molframe::seq::Alignment) -> Self {
        Self {
            score: value.score,
            columns: value
                .columns
