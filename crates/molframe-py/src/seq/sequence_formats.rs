...
use super::sequence_errors::{
    A2mError, FastqError, MatrixError, RegionAlignError, SimilarKmerError,
};
...
pub(crate) fn parse_fastq(py: Python<'_>, text: String) -> PyResult<Vec<PyFastqRecord>> {
    py.detach(move || molframe::seq::parse_fastq(&text))
        .map(|records| records.into_iter().map(Into::into).collect())
        .map_err(|error| FastqError::new_err(error.to_string()))
}
...
pub(crate) fn write_fastq(py: Python<'_>, records: Vec<PyFastqRecord>) -> PyResult<String> {
    ...
    py.detach(|| molframe::seq::write_fastq(&records))
        .map_err(|error| FastqError::new_err(error.to_string()))
}
    ...
    ($parse:ident, $write:ident, $rust_parse:path, $rust_write:path) => {
        ...
        pub(crate) fn $parse(py: Python<'_>, text: String) -> PyResult<Vec<super::PyFastaRecord>> {
            py.detach(|| $rust_parse(&text))
                .map(|records| records.into_iter().map(Into::into).collect())
                .map_err(|error| A2mError::new_err(error.to_string()))
        }
...
strict_fasta_format!(
    parse_a2m,
    write_a2m,
    molframe::seq::parse_a2m,
    molframe::seq::write_a2m
);

#[pyclass(name = "MatrixProfile", frozen, from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMatrixProfile(molframe::seq::MatrixProfile);

#[pymethods]
impl PyMatrixProfile {
    #[staticmethod]
    fn blosum(level: u8) -> Self {
        Self(molframe::seq::MatrixProfile::Blosum(level))
    }

    #[staticmethod]
    fn pam(distance: u16) -> Self {
        Self(molframe::seq::MatrixProfile::Pam(distance))
    }

    #[staticmethod]
    fn identity() -> Self {
        Self(molframe::seq::MatrixProfile::Identity)
    }

    #[staticmethod]
    fn nuc44() -> Self {
        Self(molframe::seq::MatrixProfile::Nuc44)
    }
}
...
pub(crate) fn load_matrix(
    ...
    py.detach(|| molframe::seq::load_matrix(profile.0))
        .map(PySubstitutionMatrix)
        .map_err(|error| MatrixError::new_err(error.to_string()))
}

#[pyclass(name = "SimilarKmerOptions", frozen, from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PySimilarKmerOptions(molframe::seq::SimilarKmerOptions);

#[pymethods]
impl PySimilarKmerOptions {
    #[new]
    fn new(minimum_score: i32, max_results: usize, max_candidates: usize) -> Self {
        Self(molframe::seq::SimilarKmerOptions {
            minimum_score,
            max_results,
            max_candidates,
        })
...
pub(crate) fn similar_kmers(
    ...
    py.detach(move || molframe::seq::similar_kmers(&query, &alphabet, &matrix, options.0))
        .map(|values| {
            values
                .into_iter()
        ...
        .map_err(|error| SimilarKmerError::new_err(error.to_string()))
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyRegionOptions(molframe::seq::RegionOptions);
...
impl PyRegionOptions {
    ...
    fn new(
        ...
        Self(molframe::seq::RegionOptions {
            left: left.0..left.1,
...
pub(crate) fn align_region(
    ...
    py.detach(move || molframe::seq::align_region(&left, &right, scoring.inner(), &options))
        .map(Into::into)
        .map_err(|error| RegionAlignError::new_err(error.to_string()))
}

impl From<PyAlignmentMode> for molframe::seq::AlignmentMode {
...
impl From<molframe::seq::FastqRecord> for PyFastqRecord {
    fn from(value: molframe::seq::FastqRecord) -> Self {
        Self {
            id: value.id,
...
impl From<PyFastqRecord> for molframe::seq::FastqRecord {
