...
impl PyHedron {
    ...
    fn from_points(i: [f32; 3], j: [f32; 3], k: [f32; 3]) -> Option<Self> {
        molframe::Hedron::from_points(i, j, k).map(Self::from)
    }
}
...
impl PyDihedron {
    ...
    fn from_points(i: [f32; 3], j: [f32; 3], k: [f32; 3], l: [f32; 3]) -> Option<Self> {
        molframe::Dihedron::from_points(i, j, k, l).map(Self::from)
    }
}
...
pub(crate) struct PyInternalCoordinates {
    inner: molframe::InternalCoordinates,
}
...
pub(crate) fn internal_coordinates(
    ...
    py.detach(move || molframe::internal_coordinates(&structure, molframe::ModelIndex::new(model)))
...
pub(crate) fn place_atom(
    ...
    molframe::place_atom(i, j, k, length, angle, torsion)
}

impl From<molframe::Hedron> for PyHedron {
    fn from(value: molframe::Hedron) -> Self {
        Self {
...
impl From<molframe::Dihedron> for PyDihedron {
    fn from(value: molframe::Dihedron) -> Self {
        Self {
...
impl From<molframe::InternalAtom> for PyInternalAtom {
    fn from(value: molframe::InternalAtom) -> Self {
        Self {
            atom: value.atom.get(),
            references: value.references.map(molframe::AtomIndex::get),
