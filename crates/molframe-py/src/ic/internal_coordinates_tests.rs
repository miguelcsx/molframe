...
fn internal_records_convert_typed_indices_at_the_boundary() {
    let value = molframe::InternalAtom {
        atom: molframe::AtomIndex::new(4),
        references: [
            molframe::AtomIndex::new(0),
            molframe::AtomIndex::new(1),
            molframe::AtomIndex::new(2),
        ],
        coordinate: molframe::Dihedron {
            first: molframe::Hedron {
                first_length: 1.0,
                angle: 1.1,
                second_length: 1.2,
            },
...
fn internal_coordinate_wrapper_is_owned() {
    let structure = molframe::Structure::new(molframe::StructureData::empty());
    let result = molframe::internal_coordinates(&structure, molframe::ModelIndex::new(0));
    let inner = result.expect("empty structure has a deterministic empty frame");
    let type_check: fn(molframe::InternalCoordinates) -> PyInternalCoordinates =
