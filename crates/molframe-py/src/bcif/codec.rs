...
impl From<PyDataType> for molframe::bcif::DataType {
...
impl From<molframe::bcif::DataType> for PyDataType {
    fn from(value: molframe::bcif::DataType) -> Self {
        match value {
            molframe::bcif::DataType::Int8 => Self::Int8,
            molframe::bcif::DataType::Int16 => Self::Int16,
            molframe::bcif::DataType::Int32 => Self::Int32,
            molframe::bcif::DataType::Uint8 => Self::Uint8,
            molframe::bcif::DataType::Uint16 => Self::Uint16,
            molframe::bcif::DataType::Uint32 => Self::Uint32,
            molframe::bcif::DataType::Float32 => Self::Float32,
            molframe::bcif::DataType::Float64 => Self::Float64,
        }
    }
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyEncoding(pub(crate) molframe::bcif::Encoding);

#[pymethods]
impl PyEncoding {
    #[staticmethod]
    fn byte_array(data_type: PyDataType) -> Self {
        Self(molframe::bcif::Encoding::ByteArray {
    ...
    fn fixed_point(factor: f64, src_type: PyDataType) -> Self {
        Self(molframe::bcif::Encoding::FixedPoint {
            factor,
    ...
    fn interval_quantization(
        ...
        Self(molframe::bcif::Encoding::IntervalQuantization {
            min: minimum,
            max: maximum,
            num_steps,
            src_type: src_type.into(),
    ...
    fn run_length(src_type: PyDataType, src_size: usize) -> Self {
        Self(molframe::bcif::Encoding::RunLength {
            src_type: src_type.into(),
            src_size,
        })
    }
    ...
    fn delta(origin: i64, src_type: PyDataType) -> Self {
        Self(molframe::bcif::Encoding::Delta {
            origin,
            src_type: src_type.into(),
        })
    }
    ...
    fn integer_packing(byte_count: u8, is_unsigned: bool, src_size: usize) -> Self {
        Self(molframe::bcif::Encoding::IntegerPacking {
            byte_count,
            is_unsigned,
            src_size,
        })
    ...
    fn string_array(
        ...
        Self(molframe::bcif::Encoding::StringArray {
    ...
    fn kind(&self) -> &'static str {
        match self.0 {
            molframe::bcif::Encoding::ByteArray { .. } => "byte_array",
            molframe::bcif::Encoding::FixedPoint { .. } => "fixed_point",
            molframe::bcif::Encoding::IntervalQuantization { .. } => "interval_quantization",
            molframe::bcif::Encoding::RunLength { .. } => "run_length",
            molframe::bcif::Encoding::Delta { .. } => "delta",
            molframe::bcif::Encoding::IntegerPacking { .. } => "integer_packing",
            molframe::bcif::Encoding::StringArray { .. } => "string_array",
        }
    }
    ...
    fn data_type(&self) -> Option<PyDataType> {
        match self.0 {
            molframe::bcif::Encoding::ByteArray { r#type } => Some(r#type.into()),
            _ => None,
        }
    }
    ...
    fn src_type(&self) -> Option<PyDataType> {
        match self.0 {
            molframe::bcif::Encoding::FixedPoint { src_type, .. }
            | molframe::bcif::Encoding::IntervalQuantization { src_type, .. }
            | molframe::bcif::Encoding::RunLength { src_type, .. }
            | molframe::bcif::Encoding::Delta { src_type, .. } => Some(src_type.into()),
            _ => None,
        }
    }
    ...
    fn factor(&self) -> Option<f64> {
        match self.0 {
            molframe::bcif::Encoding::FixedPoint { factor, .. } => Some(factor),
            _ => None,
        }
    }
    ...
    fn interval(&self) -> Option<(f64, f64, u32)> {
        match self.0 {
            molframe::bcif::Encoding::IntervalQuantization {
                min,
                max,
                num_steps,
                ..
    ...
    fn origin(&self) -> Option<i64> {
        match self.0 {
            molframe::bcif::Encoding::Delta { origin, .. } => Some(origin),
            _ => None,
        }
    }
    ...
    fn src_size(&self) -> Option<usize> {
        match self.0 {
            molframe::bcif::Encoding::RunLength { src_size, .. }
            | molframe::bcif::Encoding::IntegerPacking { src_size, .. } => Some(src_size),
            _ => None,
        }
    }
    ...
    fn packing(&self) -> Option<(u8, bool)> {
        match self.0 {
            molframe::bcif::Encoding::IntegerPacking {
                byte_count,
                is_unsigned,
                ..
    ...
    fn string_array_parts(&self) -> Option<StringArrayParts> {
        match &self.0 {
            molframe::bcif::Encoding::StringArray {
                data_encoding,
                string_data,
                offset_encoding,
                offsets,
...
#[derive(Clone, Debug)]
pub(crate) struct PyEncodedData(pub(crate) molframe::bcif::EncodedData);

#[pymethods]
impl PyEncodedData {
    #[new]
    fn new(encoding: Vec<PyEncoding>, data: &Bound<'_, PyBytes>) -> Self {
        Self(molframe::bcif::EncodedData {
...
pub(crate) fn encode_integers(
    ...
    py.detach(|| molframe::bcif::encode_integers(values))
        .map(PyEncodedData)
        .map_err(codec_error)
}
...
pub(crate) fn encode_floats(
    ...
    let values = contiguous(&values)?;
    py.detach(|| molframe::bcif::encode_floats(values))
        .map(PyEncodedData)
        .map_err(codec_error)
}
...
pub(crate) fn encode_interval(
    ...
    let values = contiguous(&values)?;
    py.detach(|| molframe::bcif::encode_interval(values, steps))
        .map(PyEncodedData)
        .map_err(codec_error)
}
...
pub(crate) fn encode_strings(py: Python<'_>, values: Vec<String>) -> PyResult<PyEncodedData> {
    py.detach(move || molframe::bcif::encode_strings(&values))
        .map(PyEncodedData)
        .map_err(codec_error)
}
...
pub(crate) fn decode(py: Python<'_>, encoded: &PyEncodedData) -> PyResult<PyDecoded> {
    ...
    py.detach(move || molframe::bcif::decode(&encoded))
        .map_err(codec_error)
...
fn decoded_into_python(py: Python<'_>, decoded: molframe::bcif::Decoded) -> PyResult<PyDecoded> {
    let (kind, values) = match decoded {
        molframe::bcif::Decoded::Integers(values) => {
            ("integers", values.into_pyarray(py).into_any().unbind())
        }
        molframe::bcif::Decoded::Floats(values) => {
            ("floats", values.into_pyarray(py).into_any().unbind())
        }
        molframe::bcif::Decoded::Strings(values) => ("strings", strings_into_python(py, &values)?),
    };
...
fn strings_into_python(
    py: Python<'_>,
    values: &molframe::bcif::DecodedStringColumn,
) -> PyResult<Py<PyAny>> {
