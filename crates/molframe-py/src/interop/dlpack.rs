    ...
    fn __dlpack__<'py>(
        ...
        if dl_device.is_some_and(|device| device != (CPU_DEVICE_TYPE, CPU_DEVICE_ID)) {
            return Err(PyValueError::new_err(
                "molframe structure coordinates are available on CPU device 0",
            ));
        }
        let tensor = molframe::DlpackTensor::coordinates(self.structure())
