...
pub(crate) struct PySmallCifOptions(molframe::cif::SmallCifOptions);
...
impl PySmallCifOptions {
    ...
    fn new(dialect: PySmallCifDialect) -> Self {
        match dialect {
            PySmallCifDialect::Ddl2 => Self(molframe::cif::SmallCifOptions::ddl2()),
            PySmallCifDialect::Ddl1 => Self(molframe::cif::SmallCifOptions::ddl1()),
        }
    }
...
pub(crate) fn lower_small_cif(document: &PyCifDocument) -> PyResult<PySmallCifStructure> {
    let value = molframe::cif::lower_small_cif(&document.inner)
...
pub(crate) fn lower_small_cif_with_options(
    ...
    let value = molframe::cif::lower_small_cif_with_options(&document.inner, options.0)
        .map_err(|error| SmallCifError::new_err(error.to_string()))?;
    PySmallCifStructure::try_from(value).map_err(|error| SmallCifError::new_err(error.to_string()))
}

impl From<molframe::cif::SmallCifAtom> for PySmallCifAtom {
    fn from(value: molframe::cif::SmallCifAtom) -> Self {
        Self {
...
impl From<molframe::cif::SmallCifBond> for PySmallCifBond {
    fn from(value: molframe::cif::SmallCifBond) -> Self {
        Self {
            first: value.first,
            second: value.second,
...
impl TryFrom<molframe::cif::SmallCifStructure> for PySmallCifStructure {
    type Error = PyErr;

    fn try_from(value: molframe::cif::SmallCifStructure) -> Result<Self, Self::Error> {
