...
impl PyCifLexer {
    ...
    fn new(data: Vec<u8>) -> PyResult<Self> {
        let mut lexer = molframe::cif::lexer::Lexer::new(&data).map_err(lex_error)?;
...
fn token(value: molframe::cif::lexer::Token<'_>) -> PyCifToken {
    match value {
        molframe::cif::lexer::Token::Block(text) => value_with("block", text, None),
        molframe::cif::lexer::Token::FrameStart(text) => value_with("frame_start", text, None),
        molframe::cif::lexer::Token::FrameEnd => value_with("frame_end", "", None),
        molframe::cif::lexer::Token::Loop => value_with("loop", "", None),
        molframe::cif::lexer::Token::Tag(text) => value_with("tag", text, None),
        molframe::cif::lexer::Token::Value(text, quoting) => {
...
fn position(value: molframe::Position) -> PositionTuple {
    (value.byte_offset, value.line, value.column)
}

fn span(value: molframe::ByteSpan) -> SpanTuple {
    (
...
fn lex_error(error: molframe::cif::lexer::LexError) -> PyErr {
