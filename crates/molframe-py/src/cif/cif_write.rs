...
impl PyCifWriteOptions {
    pub(crate) fn native(&self) -> molframe::CifWriteOptions {
...
pub(crate) fn quote_text(py: Python<'_>, text: &str) -> String {
    py.detach(move || -> String { molframe::cif::quote_text(text) })
}

#[pyfunction]
pub(crate) fn render_value(py: Python<'_>, value: &PyCifValue) -> String {
    py.detach(move || -> String { molframe::cif::render_value(&value.inner) })
}
...
fn write_native(
    ...
    py.detach(move || molframe::write_mmcif_with_options(&structure, &options.native()))
