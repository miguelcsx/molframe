...
impl PyDataset {
    ...
    fn load(&self, py: Python<'_>, index: isize) -> PyResult<PyStructure> {
        ...
        match self
            .inner
            .load_with(index, |entry| molframe::read(&entry.path))
        {
            Ok(structure) => Ok(PyStructure::new(structure)),
            Err(molframe::LoadError::Dataset(error)) => Err(dataset_error(error)),
            Err(molframe::LoadError::Loader(findings)) => Err(read_error(py, &findings)),
        }
    }
...
fn dataset_error(error: molframe::DatasetError) -> PyErr {
