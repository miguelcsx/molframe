...
pub(crate) struct PyTrajectory {
    ...
    data: Vec<std::collections::BTreeMap<Box<str>, molframe::traj::FrameValue>>,
...
impl PyTrajectory {
    ...
    pub(crate) fn native_trajectory(&self, py: Python<'_>) -> PyResult<molframe::traj::Trajectory> {
        let frames = (0..self.frames)
            .map(|frame| {
                Ok(molframe::traj::Frame {
                    positions: self.positions.frame(py, frame, self.frames, self.atoms)?,
                })
            })
            .collect::<PyResult<_>>()?;
        molframe::traj::Trajectory::from_frames(frames)
