...
impl PyGsdOptions {
    ...
    fn new(length_to_angstrom: f64) -> PyResult<Self> {
        molframe::traj::GsdOptions::new(length_to_angstrom)
            .map(|_| Self { length_to_angstrom })
            .map_err(|error| GsdError::new_err(error.to_string()))
    }
}

impl From<PyGsdOptions> for molframe::traj::GsdOptions {
...
impl TryFrom<molframe::traj::GsdTrajectory> for PyGsdTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::GsdTrajectory) -> Result<Self, Self::Error> {
        let frames = value
            .frames
            .into_iter()
...
pub(crate) fn parse_gsd(
    ...
    py.detach(move || -> PyResult<PyGsdTrajectory> {
        molframe::traj::parse_gsd(&path, options.into())
            .map_err(|error| GsdError::new_err(error.to_string()))?
            .try_into()
    })
}
...
pub(crate) fn write_gsd(
    ...
    py.detach(move || -> PyResult<()> {
        let frames = frames
            ...
            .map(TryInto::try_into)
            .collect::<PyResult<Vec<_>>>()?;
        molframe::traj::write_gsd(&path, &frames, options.into())
            .map_err(|error| GsdError::new_err(error.to_string()))
    })
}
...
impl From<PyTngCompression> for molframe::traj::TngCompression {
...
impl From<molframe::traj::TngCompression> for PyTngCompression {
    fn from(value: molframe::traj::TngCompression) -> Self {
        match value {
            molframe::traj::TngCompression::Uncompressed => Self::uncompressed(),
            molframe::traj::TngCompression::Lossless => Self::lossless(),
            molframe::traj::TngCompression::Lossy { precision } => Self {
                kind: TngCompressionKind::Lossy,
                precision,
            },
        }
...
impl PyTngWriteOptions {
    ...
    fn new(
        ...
        let defaults = molframe::traj::TngWriteOptions::default();
        Self {
...
impl From<PyTngWriteOptions> for molframe::traj::TngWriteOptions {
...
impl TryFrom<molframe::traj::TngTrajectory> for PyTngTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::TngTrajectory) -> Result<Self, Self::Error> {
        let frames = value
            .frames
            .into_iter()
            .map(TryInto::try_into)
...
pub(crate) fn write_tng(
    ...
    py.detach(move || -> PyResult<()> {
        let frames = frames
            ...
            .map(TryInto::try_into)
            .collect::<PyResult<Vec<_>>>()?;
        molframe::traj::write_tng(&path, &frames, options.into())
            .map_err(|error| TngError::new_err(error.to_string()))
    })
}
