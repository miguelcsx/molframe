...
impl From<PyAmberNetcdfPrecision> for molframe::traj::AmberNetcdfPrecision {
...
impl From<molframe::traj::AmberNetcdfPrecision> for PyAmberNetcdfPrecision {
    fn from(value: molframe::traj::AmberNetcdfPrecision) -> Self {
        match value {
            molframe::traj::AmberNetcdfPrecision::Single => Self::Single,
            molframe::traj::AmberNetcdfPrecision::Double => Self::Double,
        }
    }
}
...
impl From<molframe::traj::AmberNetcdfMetadata> for PyAmberNetcdfMetadata {
    fn from(value: molframe::traj::AmberNetcdfMetadata) -> Self {
        Self {
...
impl PyAmberNetcdfWriteOptions {
    ...
    fn new(
        ...
        let defaults = molframe::traj::AmberNetcdfWriteOptions::default();
        Self {
...
impl From<PyAmberNetcdfWriteOptions> for molframe::traj::AmberNetcdfWriteOptions {
...
impl TryFrom<molframe::traj::AmberNetcdfTrajectory> for PyAmberNetcdfTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::AmberNetcdfTrajectory) -> Result<Self, Self::Error> {
        let frames = value
            .frames
            .into_iter()
...
pub(crate) fn parse_amber_netcdf(py: Python<'_>, bytes: Vec<u8>) -> PyResult<Vec<PyTimestep>> {
    py.detach(move || -> PyResult<Vec<PyTimestep>> {
        molframe::traj::parse_amber_netcdf(&bytes)
...
pub(crate) fn parse_amber_netcdf_record(
    ...
    py.detach(move || -> PyResult<PyAmberNetcdfTrajectory> {
        molframe::traj::parse_amber_netcdf_record(&bytes)
            .map_err(|error| AmberNetcdfError::new_err(error.to_string()))?
            .try_into()
    })
}
...
pub(crate) fn write_amber_netcdf(
    ...
    py.detach(move || -> PyResult<Vec<u8>> {
        let frames = frames
            ...
            .map(TryInto::try_into)
            .collect::<PyResult<Vec<_>>>()?;
        molframe::traj::write_amber_netcdf(&frames, options.into())
            .map_err(|error| AmberNetcdfError::new_err(error.to_string()))
    })
}
