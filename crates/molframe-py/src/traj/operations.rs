...
enum NativeFrameAnalysisError {
    Trajectory(molframe::traj::TrajectoryError),
    Geometry(String),
    Invalid(String),
}

impl From<molframe::traj::TrajectoryError> for NativeFrameAnalysisError {
    fn from(value: molframe::traj::TrajectoryError) -> Self {
...
struct NativeRmsdAnalysis {
    reference: usize,
    alignment: molframe::traj::FrameAlignment,
...
impl molframe::traj::FrameAnalysis for NativeRmsdAnalysis {
    type Output = Vec<f64>;
    ...
    fn prepare(
        &self,
        trajectory: &molframe::traj::Trajectory,
    ...
    fn single_frame(
        &self,
        timestep: &molframe::traj::Timestep,
        partial: &mut Self::Partial,
        _context: &molframe::core::ExecutionContext,
    ) -> Result<(), Self::Error> {
        if timestep.positions.len() != partial.reference.len() {
            return Err(molframe::traj::TrajectoryError::AtomCountMismatch {
        ...
        let value = match self.alignment {
            molframe::traj::FrameAlignment::None => {
                { molframe::rmsd(&timestep.positions, &partial.reference) }
                    .map_err(|error| NativeFrameAnalysisError::Geometry(format!("{error:?}")))?
            }
            molframe::traj::FrameAlignment::Rigid => {
                molframe::superpose(&timestep.positions, &partial.reference)
                    .map(|fit| fit.rmsd)
                    .map_err(|error| NativeFrameAnalysisError::Geometry(format!("{error:?}")))?
            }
        };
    ...
    fn conclude(
    ...
    ) -> Result<molframe::Analysis<Self::Output>, Self::Error> {
        ...
        let mut result = molframe::Analysis::complete(
            partial.values,
            molframe::Coverage::complete(count),
            &self.policy,
        );
        result.provenance = result
            .provenance
            .with_algorithm(molframe::AlgorithmId::new(Self::NAME, "1"))
            .with_parameter(
                "reference",
                molframe::ParameterValue::Integer(i64::try_from(self.reference).map_err(|_| {
                    NativeFrameAnalysisError::Invalid("reference exceeds parameter limits".into())
                })?),
            )
            .with_parameter(
                "alignment",
                molframe::ParameterValue::Text(format!("{:?}", self.alignment).into()),
            );
        Ok(result)
    }
}
...
impl PyFrameAnalysis {
    fn native(&self, policy: molframe::AnalysisPolicy) -> NativeRmsdAnalysis {
        match self.kind {
...
pub(crate) fn run_analysis(
    ...
    let result = py
        .detach(move || {
            molframe::traj::run_analysis(&native_trajectory, &native_analysis, &context)
        })
...
pub(crate) fn agglomerative_clustering(
    ...
    py.detach(move || -> PyResult<PyClustering> {
        molframe::traj::agglomerative_clustering(&distances.native(), cluster_count, linkage.into())
...
pub(crate) fn dbscan_clustering(
    ...
    py.detach(move || -> PyResult<PyClustering> {
        molframe::traj::dbscan_clustering(&distances.native(), epsilon, minimum_points)
            .map(PyClustering::from)
            .map_err(kernel_error)
    })
}
...
pub(crate) fn medoid(
    ...
    py.detach(move || -> PyResult<usize> {
        molframe::traj::medoid(&distances.native(), &members).map_err(kernel_error)
    })
}
...
pub(crate) fn harmonic_ensemble_similarity(
    ...
    py.detach(move || -> PyResult<PyHarmonicSimilarity> {
        molframe::traj::harmonic_ensemble_similarity(&first, &second, options.native())
...
pub(crate) fn cluster_population_similarity(
    ...
    py.detach(move || -> PyResult<f64> {
        molframe::traj::cluster_population_similarity(&first, &second, cluster_count)
            .map_err(kernel_error)
    })
}
...
pub(crate) fn group_coordinate_variance(
    ...
    py.detach(move || molframe::traj::group_coordinate_variance_view(frames, &groups))
...
pub(crate) fn block_convergence(
    ...
    py.detach(move || -> PyResult<Vec<PyConvergenceBlock>> {
        molframe::traj::block_convergence(&values, block_size, remainder.into())
...
pub(crate) fn path_similarity(
    ...
    let metric = match metric {
        PyPathFrameMetric::CartesianRmsd => molframe::traj::PathFrameMetric::CartesianRmsd,
        PyPathFrameMetric::FittedRmsd => molframe::traj::PathFrameMetric::FittedRmsd,
    };
    py.detach(move || {
        molframe::traj::path_similarity_view(first, second, metric, memory_limit_bytes)
    })
...
pub(crate) fn water_dynamics(
    ...
    let mode = match mode {
        PySurvivalMode::Continuous => molframe::traj::SurvivalMode::Continuous,
        PySurvivalMode::Intermittent => molframe::traj::SurvivalMode::Intermittent,
    };
    molframe::traj::water_dynamics(&occupancy, maximum_lag, frame_duration, mode)
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add(
        ...
        molframe::traj::DEFAULT_PAIRWISE_MEMORY_LIMIT,
    )?;
    module.add(
        "DEFAULT_PROCRUSTES_TOLERANCE",
        molframe::traj::DEFAULT_PROCRUSTES_TOLERANCE,
    )?;
    module.add(
        "DEFAULT_PROCRUSTES_MAXIMUM_ITERATIONS",
        molframe::traj::DEFAULT_PROCRUSTES_MAXIMUM_ITERATIONS,
    )?;
