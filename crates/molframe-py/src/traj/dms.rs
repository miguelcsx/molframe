...
use molframe::traj::{DmsSystem, read_dms as native_read_dms, write_dms as native_write_dms};
...
impl PyDmsSystem {
    ...
    fn new(
        ...
        let topology = match topology {
            Some(value) => value,
            None => PyDmsTopology {
                value: molframe::traj::DmsTopology::default(),
            },
        };
        let frame = match frame {
            Some(value) => value,
            None => PyDmsFrame {
                value: molframe::traj::DmsFrame::default(),
            },
        };
...
fn dms_error(error: molframe::traj::DmsError) -> PyErr {
    match error {
        molframe::traj::DmsError::Io(error) => PyOSError::new_err(error.to_string()),
