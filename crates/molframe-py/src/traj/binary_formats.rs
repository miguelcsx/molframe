...
impl From<PyDcdEndian> for molframe::traj::DcdEndian {
...
impl From<molframe::traj::DcdEndian> for PyDcdEndian {
    fn from(value: molframe::traj::DcdEndian) -> Self {
        match value {
            molframe::traj::DcdEndian::Little => Self::Little,
            molframe::traj::DcdEndian::Big => Self::Big,
        }
    }
}
...
impl From<molframe::traj::DcdHeader> for PyDcdHeader {
    fn from(value: molframe::traj::DcdHeader) -> Self {
        Self {
...
impl From<PyDcdHeader> for molframe::traj::DcdHeader {
...
impl PyDcdWriteOptions {
    ...
    fn new(
        ...
        let defaults = molframe::traj::DcdWriteOptions::default();
        Self {
            endian: match endian {
                Some(value) => value,
...
impl From<PyDcdWriteOptions> for molframe::traj::DcdWriteOptions {
...
impl From<PyTrrPrecision> for molframe::traj::TrrPrecision {
...
impl From<molframe::traj::TrrPrecision> for PyTrrPrecision {
    fn from(value: molframe::traj::TrrPrecision) -> Self {
        match value {
            molframe::traj::TrrPrecision::Single => Self::Single,
            molframe::traj::TrrPrecision::Double => Self::Double,
        }
    }
}
...
impl From<PyTrrWriteOptions> for molframe::traj::TrrWriteOptions {
...
impl TryFrom<molframe::traj::DcdTrajectory> for PyDcdTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::DcdTrajectory) -> Result<Self, Self::Error> {
        Ok(Self {
...
impl TryFrom<molframe::traj::TrrTrajectory> for PyTrrTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::TrrTrajectory) -> Result<Self, Self::Error> {
        Ok(Self {
            frames: convert_frames(value.frames)?,
            steps: value.steps,
...
impl TryFrom<molframe::traj::XtcTrajectory> for PyXtcTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::XtcTrajectory) -> Result<Self, Self::Error> {
        Ok(Self {
            frames: convert_frames(value.frames)?,
            steps: value.steps,
...
pub(crate) fn write_dcd(
    ...
    py.detach(move || -> PyResult<Vec<u8>> {
        ...
        molframe::traj::write_dcd(&frames, &options.into())
            .map_err(|error| DcdError::new_err(error.to_string()))
    })
}
...
pub(crate) fn write_trr(
    ...
    py.detach(move || -> PyResult<Vec<u8>> {
        let frames = native_frames(frames)?;
        molframe::traj::write_trr(&frames, options.into())
            .map_err(|error| TrrError::new_err(error.to_string()))
    })
}
...
pub(crate) fn write_trr_with_precisions(
    ...
    py.detach(move || -> PyResult<Vec<u8>> {
        ...
        molframe::traj::write_trr_with_precisions(&frames, &precisions)
            .map_err(|error| TrrError::new_err(error.to_string()))
    })
}
...
pub(crate) fn write_xtc(
    ...
    py.detach(move || -> PyResult<Vec<u8>> {
        let frames = native_frames(frames)?;
        molframe::traj::write_xtc(
            &frames,
            molframe::traj::XtcWriteOptions {
...
pub(crate) fn write_xtc_with_precisions(
    ...
    py.detach(move || -> PyResult<Vec<u8>> {
        let frames = native_frames(frames)?;
        molframe::traj::write_xtc_with_precisions(&frames, &precisions)
            .map_err(|error| XtcError::new_err(error.to_string()))
    })
}

fn native_frames(frames: Vec<PyTimestep>) -> PyResult<Vec<molframe::traj::Timestep>> {
    frames.into_iter().map(TryInto::try_into).collect()
}

fn convert_frames(frames: Vec<molframe::traj::Timestep>) -> PyResult<Vec<PyTimestep>> {
    frames.into_iter().map(TryInto::try_into).collect()
}
