...
impl PyTrajectoryStreamReader {
    ...
    fn random_access(&self) -> PyResult<super::generic::PyRandomAccess> {
        Ok(match self.lock()?.reader.random_access() {
            molframe::traj::RandomAccess::Full => super::generic::PyRandomAccess::Full,
            molframe::traj::RandomAccess::ViaIndex => super::generic::PyRandomAccess::ViaIndex,
            molframe::traj::RandomAccess::None => super::generic::PyRandomAccess::None,
        })
    }
    ...
    native_read_trajectory(&path, &options)
        .map(|reader| PyTrajectoryStreamReader {
            state: Mutex::new(TrajectoryStreamState {
                reader,
                timestep: molframe::traj::Timestep::default(),
            }),
        })
        .map_err(io_error)
}
    ...
    fn new(
        ...
        let mut inner = TrajectoryReadOptions {
            ...
            amber_restart_layout: match amber_restart_layout {
                ...
                None => molframe::traj::AmberRestartLayout::Auto,
            },
...
fn apply_h5md_options(
    ...
    particle_group: Option<String>,
    units: Option<molframe::traj::H5mdUnitSystem>,
) {
...
fn io_error(error: molframe::traj::TrajectoryIoError) -> PyErr {
    match error {
        molframe::traj::TrajectoryIoError::Io(error) => PyOSError::new_err(error.to_string()),
