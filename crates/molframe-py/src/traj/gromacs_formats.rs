...
impl From<molframe::traj::GromacsMoleculeType> for PyGromacsMoleculeType {
    fn from(value: molframe::traj::GromacsMoleculeType) -> Self {
        Self {
...
impl From<PyGromacsMoleculeType> for molframe::traj::GromacsMoleculeType {
...
impl From<molframe::traj::GromacsItpAtom> for PyGromacsItpAtom {
    fn from(value: molframe::traj::GromacsItpAtom) -> Self {
        Self {
            number: value.number,
...
impl From<PyGromacsItpAtom> for molframe::traj::GromacsItpAtom {
...
impl<const N: usize> From<molframe::traj::GromacsInteraction<N>> for PyGromacsInteraction {
    fn from(value: molframe::traj::GromacsInteraction<N>) -> Self {
        Self {
...
impl TryFrom<PyGromacsItp> for molframe::traj::GromacsItp {
    type Error = PyErr;
...
impl TryFrom<molframe::traj::GromacsItp> for PyGromacsItp {
    type Error = PyErr;

    fn try_from(value: molframe::traj::GromacsItp) -> Result<Self, Self::Error> {
        Ok(Self {
            directives: value.directives.into_iter().map(Into::into).collect(),
            molecule_type: value.molecule_type.map(Into::into),
            atoms: value.atoms.into_iter().map(Into::into).collect(),
...
pub(crate) fn parse_gromacs_itp(py: Python<'_>, text: &str) -> PyResult<PyGromacsItp> {
    py.detach(move || -> PyResult<PyGromacsItp> {
        molframe::traj::parse_gromacs_itp(text)
