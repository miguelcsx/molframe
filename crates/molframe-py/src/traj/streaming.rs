...
use molframe::core::ExecutionContext;
use molframe::traj::{FrameAlignment, TrajectoryReader, TrajectoryReaderOptions};
...
fn open(
    ...
    molframe::traj::read_trajectory_in(
        path,
...
fn rmsf_stream<'py>(
    ...
    let result = py.detach(move || {
        ...
        molframe::traj::rmsf_stream(&mut *reader, &context, frame_workspace_bytes)
...
fn rmsd_stream(
    ...
    py.detach(move || {
        ...
        let result = molframe::traj::rmsd_stream(
            &mut *reader,
            positions,
            if fit {
            ...
            |index, time, rmsd| {
                Python::attach(|py| emit.call1(py, (index, time, rmsd)))
                    ...
                    .map_err(|error| {
                        ...
                        molframe::traj::TrajectoryError::InvalidSource {
...
fn contact_counts_stream(
    ...
    let context = context.native();
    let options = options.map_or_else(molframe::SpatialSearchOptions::default, |options| {
        options.inner()
    });
    py.detach(move || {
        let mut reader = open(&path, &context, frame_workspace_bytes)?;
...
fn run_analysis_stream(
    ...
    use molframe::core::{Backpressure, BatchDemand, BatchSource, ChunkId, DatasetId, LogicalRow};
    let context = context.native();
    py.detach(move || {
        let reader = open(&path, &context, frame_workspace_bytes)?;
        let mut source = molframe::traj::TrajectoryBatchSource::new(
            reader,
            DatasetId::new(0),
            ChunkId::new(0),
            LogicalRow::new(0),
...
fn trajectory_error(error: molframe::traj::TrajectoryError) -> PyErr {
    match error {
        molframe::traj::TrajectoryError::MemoryLimit { .. }
        | molframe::traj::TrajectoryError::Spatial(molframe::SpatialError::Memory(_)) => {
            PyMemoryError::new_err(error.to_string())
        }
        _ => value_error(error),
    }
