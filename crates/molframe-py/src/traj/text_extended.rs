...
impl TryFrom<molframe::traj::TrzTrajectory> for PyTrzTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::TrzTrajectory) -> Result<Self, Self::Error> {
        let frames = value
            .frames
            .into_iter()
...
impl TryFrom<PyTrzTrajectory> for molframe::traj::TrzTrajectory {
    type Error = PyErr;
...
pub(crate) fn write_trz(py: Python<'_>, trajectory: PyTrzTrajectory) -> PyResult<Vec<u8>> {
    py.detach(move || -> PyResult<Vec<u8>> {
        molframe::traj::write_trz(&trajectory.try_into()?)
            .map_err(|error| TrzError::new_err(error.to_string()))
    })
}
...
impl From<molframe::traj::GromosBoundary> for PyGromosBoundary {
    fn from(value: molframe::traj::GromosBoundary) -> Self {
        match value {
            molframe::traj::GromosBoundary::Vacuum => Self::Vacuum,
            molframe::traj::GromosBoundary::Rectangular => Self::Rectangular,
            molframe::traj::GromosBoundary::Triclinic => Self::Triclinic,
            molframe::traj::GromosBoundary::TruncatedOctahedron => Self::TruncatedOctahedron,
        }
    }
}
...
impl TryFrom<molframe::traj::GromosTrajectory> for PyGromosTrajectory {
    type Error = PyErr;

    fn try_from(value: molframe::traj::GromosTrajectory) -> Result<Self, Self::Error> {
        let frames = value
            .frames
            .into_iter()
            .map(TryInto::try_into)
...
pub(crate) fn parse_gromos11_trc(py: Python<'_>, source: &str) -> PyResult<PyGromosTrajectory> {
    py.detach(move || -> PyResult<PyGromosTrajectory> {
        molframe::traj::parse_gromos11_trc(source)
