...
#[derive(Clone, Debug)]
pub(crate) struct PySideChainDefinition(pub(crate) molframe::SideChainDefinition);
...
pub(crate) fn side_chain_definition(
    ...
    py.detach(move || -> Option<PySideChainDefinition> {
        ...
        molframe::side_chain_definition(&component.0, &roles).map(PySideChainDefinition)
    })
}
