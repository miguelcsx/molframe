...
impl From<molframe::surface::SurfaceComponent> for PySurfaceComponent {
    fn from(value: molframe::surface::SurfaceComponent) -> Self {
        Self {
            faces: value.faces,
            area: value.area,
        }
...
pub(crate) struct PySurfaceComponentFilter {
    native: molframe::surface::SurfaceComponentFilter,
}
...
fn surface_components(py: Python<'_>, mesh: &PyIndexedSurfaceMesh) -> Vec<PySurfaceComponent> {
    py.detach(move || -> Vec<PySurfaceComponent> {
        molframe::surface::surface_components(&mesh.native)
            .into_iter()
            .map(Into::into)
            .collect()
    })
...
fn filter_surface_components(
    ...
    py.detach(move || -> PyResult<PyIndexedSurfaceMesh> {
        molframe::surface::filter_surface_components(&mesh.native, filter.native)
