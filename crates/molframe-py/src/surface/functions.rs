...
pub(crate) fn fibonacci_sphere(py: Python<'_>, count: u16) -> Vec<[f64; 3]> {
    py.detach(move || -> Vec<[f64; 3]> { molframe::surface::fibonacci_sphere(count) })
}
...
pub(crate) fn buried_surface(
    ...
    py.detach(move || {
        molframe::surface::buried_surface(
            &positions,
            &radii,
            probe_radius,
            sample_points,
...
pub(crate) fn atom_depths<'py>(
    ...
    py.detach(move || molframe::surface::atom_depths(atoms, surface, options.0))
...
pub(crate) fn cavities(
    ...
    let positions = borrowed_coordinates(&positions)?;
    let radii = radii.as_slice()?;
    py.detach(move || molframe::surface::cavities_with_options(positions, radii, probe, options.0))
...
pub(crate) fn surface_points(
    ...
    let radii = radii.as_slice()?.to_vec();
    py.detach(move || {
        molframe::surface::surface_points(
            &positions,
            &radii,
            probe,
            samples,
...
pub(crate) fn surface_points_at_density(
    ...
    let radii = radii.as_slice()?.to_vec();
    py.detach(move || {
        molframe::surface::surface_points_at_density(
            &positions,
            &radii,
            probe,
            density,
...
pub(crate) fn atom_contact_areas(
    ...
    let radii = radii.as_slice()?.to_vec();
    py.detach(move || {
        molframe::surface::atom_contact_areas(
            &positions,
            &radii,
            probe,
            density,
...
pub(crate) fn surface_points_excluding_pairs(
    ...
    let pairs = pairs
        ...
        .collect::<Vec<_>>();
    py.detach(move || {
        molframe::surface::surface_points_excluding_pairs(
            &positions,
            &radii,
            probe,
            density,
...
fn solvent_excluded_surface_with_options_impl(
    ...
    let radii = radii.as_slice()?;
    py.detach(move || {
        molframe::surface::solvent_excluded_surface_with_options(positions, radii, probe, options.0)
    })
    .map(Into::into)
    .map_err(value_error)
}
...
pub(crate) fn edge_geodesic_distances(
    ...
    py.detach(move || -> PyResult<PySurfaceDistances> {
        molframe::surface::edge_geodesic_distances(&mesh.native, source)
            .map(Into::into)
            .map_err(value_error)
    })
}
...
pub(crate) fn surface_patch(
    ...
    py.detach(move || -> PyResult<Vec<u32>> {
        molframe::surface::surface_patch(&mesh.native, source, radius)
            .map(<[u32]>::into_vec)
            .map_err(value_error)
    })
}
...
pub(crate) fn surface_curvatures(
    ...
    py.detach(move || -> PyResult<Vec<PySurfaceCurvature>> {
        molframe::surface::surface_curvatures(&mesh.native)
            .map(|values| values.into_iter().map(Into::into).collect())
            .map_err(value_error)
    })
}
...
pub(crate) fn write_obj(py: Python<'_>, path: &str, mesh: &PyIndexedSurfaceMesh) -> PyResult<()> {
    py.detach(move || -> PyResult<()> {
        molframe::surface::write_obj(path, &mesh.native).map_err(value_error)
    })
}
