...
pub(crate) fn motif_error(error: molframe::fx::MotifError) -> PyErr {
    MotifError::new_err(error.to_string())
}

pub(crate) fn mapping_error(error: molframe::fx::MappingError) -> PyErr {
    MappingError::new_err(error.to_string())
}

pub(crate) fn measurement_error(error: molframe::fx::MeasurementError) -> PyErr {
    MeasurementError::new_err(error.to_string())
}

pub(crate) fn evaluation_error(error: molframe::fx::EvaluationError) -> PyErr {
    EvaluationError::new_err(error.to_string())
}

pub(crate) fn specification_error(error: molframe::fx::SpecificationError) -> PyErr {
    SpecificationError::new_err(error.to_string())
}

pub(crate) fn ame_measurement_error(error: molframe::fx::AmeMeasurementError) -> PyErr {
