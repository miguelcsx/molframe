...
#[derive(Clone, Debug)]
pub(crate) struct PyAtomSite(pub(crate) molframe::fx::AtomSite);
...
impl PyAtomSite {
    ...
    fn new(component: String, atom: String) -> PyResult<Self> {
        ...
        Ok(Self(molframe::fx::AtomSite::new(component, atom)))
    }
...
impl From<PyComponentRole> for molframe::fx::ComponentRole {
...
impl From<molframe::fx::ComponentRole> for PyComponentRole {
    fn from(value: molframe::fx::ComponentRole) -> Self {
        match value {
            molframe::fx::ComponentRole::Residue => Self::Residue,
            molframe::fx::ComponentRole::Ligand => Self::Ligand,
            molframe::fx::ComponentRole::Cofactor => Self::Cofactor,
            molframe::fx::ComponentRole::Metal => Self::Metal,
            _ => Self::Unknown,
        }
    }
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyComponentSpec(pub(crate) molframe::fx::ComponentSpec);

#[pymethods]
impl PyComponentSpec {
    #[new]
    fn new(role: PyComponentRole) -> Self {
        Self(molframe::fx::ComponentSpec::new(role.into()))
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyConstraint(pub(crate) molframe::fx::Constraint);
...
impl PyConstraint {
    ...
    fn distance(
        ...
        Ok(Self(molframe::fx::Constraint::Distance {
            first: first.0.clone(),
    ...
    fn angle(atoms: Vec<PyAtomSite>, target: f64, tolerance: f64) -> PyResult<Self> {
        ...
        Ok(Self(molframe::fx::Constraint::Angle {
            atoms,
            target,
            tolerance,
        }))
    ...
    fn dihedral(atoms: Vec<PyAtomSite>, target: f64, tolerance: f64) -> PyResult<Self> {
        ...
        Ok(Self(molframe::fx::Constraint::Dihedral {
            atoms,
            target,
            tolerance,
        }))
    ...
    fn chirality(atoms: Vec<PyAtomSite>, positive: bool) -> PyResult<Self> {
        ...
        Ok(Self(molframe::fx::Constraint::Chirality {
            atoms,
            positive,
        }))
    }
    ...
    fn planarity(atoms: Vec<PyAtomSite>, tolerance: f64) -> PyResult<Self> {
        ...
        Ok(Self(molframe::fx::Constraint::Planarity {
    ...
    fn coordination(
        ...
        Ok(Self(molframe::fx::Constraint::Coordination {
    ...
    fn steric_exclusion(
        ...
        Ok(Self(molframe::fx::Constraint::StericExclusion {
            first: first.0.clone(),
            second: second.0.clone(),
            min_distance,
        }))
    ...
    fn kind(&self) -> &'static str {
        match &self.0 {
            molframe::fx::Constraint::Distance { .. } => "distance",
            molframe::fx::Constraint::Angle { .. } => "angle",
            molframe::fx::Constraint::Dihedral { .. } => "dihedral",
            molframe::fx::Constraint::Chirality { .. } => "chirality",
            molframe::fx::Constraint::Planarity { .. } => "planarity",
            molframe::fx::Constraint::Coordination { .. } => "coordination",
            molframe::fx::Constraint::StericExclusion { .. } => "steric-exclusion",
            _ => "unknown",
        }
    }
    ...
    fn atoms(&self) -> Vec<PyAtomSite> {
        if let molframe::fx::Constraint::Distance { first, second, .. }
        | molframe::fx::Constraint::StericExclusion { first, second, .. } = &self.0
        {
            return vec![PyAtomSite(first.clone()), PyAtomSite(second.clone())];
        }
        if let molframe::fx::Constraint::Angle { atoms, .. } = &self.0 {
            return atoms.iter().cloned().map(PyAtomSite).collect();
        }
        if let molframe::fx::Constraint::Dihedral { atoms, .. }
        | molframe::fx::Constraint::Chirality { atoms, .. } = &self.0
        {
            return atoms.iter().cloned().map(PyAtomSite).collect();
        }
        if let molframe::fx::Constraint::Planarity { atoms, .. } = &self.0 {
            return atoms.iter().cloned().map(PyAtomSite).collect();
        }
        if let molframe::fx::Constraint::Coordination {
            centre, partners, ..
        } = &self.0
        {
    ...
    fn target(&self) -> Option<f64> {
        match &self.0 {
            molframe::fx::Constraint::Distance { target, .. }
            | molframe::fx::Constraint::Angle { target, .. }
            | molframe::fx::Constraint::Dihedral { target, .. } => Some(*target),
            _ => None,
        }
    }
    ...
    fn tolerance(&self) -> Option<f64> {
        match &self.0 {
            molframe::fx::Constraint::Distance { tolerance, .. }
            | molframe::fx::Constraint::Angle { tolerance, .. }
            | molframe::fx::Constraint::Dihedral { tolerance, .. }
            | molframe::fx::Constraint::Planarity { tolerance, .. } => Some(*tolerance),
            _ => None,
        }
    }
    ...
    fn positive(&self) -> Option<bool> {
        match &self.0 {
            molframe::fx::Constraint::Chirality { positive, .. } => Some(*positive),
            _ => None,
        }
    }
    ...
    fn count(&self) -> Option<usize> {
        match &self.0 {
            molframe::fx::Constraint::Coordination { count, .. } => Some(*count),
            _ => None,
        }
    }
    ...
    fn max_distance(&self) -> Option<f64> {
        match &self.0 {
            molframe::fx::Constraint::Coordination { max_distance, .. } => Some(*max_distance),
            _ => None,
        }
    }
    ...
    fn min_distance(&self) -> Option<f64> {
        match &self.0 {
            molframe::fx::Constraint::StericExclusion { min_distance, .. } => Some(*min_distance),
            _ => None,
        }
    }
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyNamedConstraint(pub(crate) molframe::fx::NamedConstraint);

#[pymethods]
impl PyNamedConstraint {
    #[new]
    fn new(name: String, constraint: &PyConstraint) -> Self {
        Self(molframe::fx::NamedConstraint {
...
#[derive(Clone, Debug)]
pub(crate) struct PyMotif(pub(crate) molframe::fx::Motif);
...
impl PyMotif {
    ...
    fn new(
        ...
        let constraints = constraints
            ...
            .map(|item| item.0)
            .collect::<Vec<_>>();
        molframe::fx::Motif::new(components, constraints)
            .map(Self)
            .map_err(motif_error)
    }
...
fn fixed_sites<const N: usize>(
    ...
    kind: &str,
) -> PyResult<[molframe::fx::AtomSite; N]> {
    if values.len() != N {
