...
fn count_pairs_within(
    ...
    py.detach(move || {
        ...
        let selection = |indices: Option<&[u32]>| {
            indices.map_or(molframe::AtomSelection::All(atoms), |indices| {
                molframe::AtomSelection::Sparse(indices.to_vec())
            })
        };
        let left = selection(left);
        let right = selection(right);
        molframe::spatial::count_pairs_within(&molframe::spatial::PairQuery {
            positions,
            left: &left,
            right: &right,
            cutoff,
        ...
        .map_err(|error| match error {
            molframe::SpatialError::Memory(_) => PyMemoryError::new_err(error.to_string()),
            _ => value_error(error),
        })
    })
}
