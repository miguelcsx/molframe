...
pyo3::create_exception!(molframe_spatial, SpatialError, PyValueError);

#[pyclass(name = "NeighborList", frozen, skip_from_py_object)]
#[derive(Debug)]
pub(crate) struct PyNeighborList(molframe::NeighborList);
...
impl PyNeighborList {
    ...
    fn build(
        ...
        let periodic = cell
            .map(|value| molframe::PeriodicBox::from_cell(value.cell))
            .transpose()
        ...
        py.detach(|| {
            ...
            molframe::NeighborList::build_with_options(
                positions,
                left,
                right,
                cutoff,
                periodic.as_ref(),
                molframe::CoordinateGeneration::INITIAL,
                options.0,
            )
