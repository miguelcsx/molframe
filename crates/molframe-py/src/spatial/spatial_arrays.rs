...
impl PyNeighborTable {
    ...
    pub(crate) fn from_pairs(pairs: Vec<molframe::NeighborPair>) -> Result<Self, String> {
...
enum SearchProfile {
    ...
    Options(molframe::SpatialSearchOptions),
}

pub(crate) enum SpatialBindingError {
    Input(String),
    Spatial(molframe::SpatialError),
    Allocation(String),
}
...
fn nearest_neighbors(
    ...
    let periodic_options = match periodic_options {
        ...
        None => molframe::KdPeriodicOptions::default(),
    };
    py.detach(move || {
...
fn run_neighbor_pairs(
    ...
    periodic: Option<&molframe::PeriodicBox>,
    ...
    let pairs = match profile {
        SearchProfile::Backend(backend) => molframe::pairs_within(
            positions,
            &left,
            &right,
            cutoff,
        ...
        SearchProfile::Options(options) => molframe::pairs_within_with_options(
            positions,
            &left,
            &right,
            cutoff,
...
fn run_atoms_within(
    ...
    let selected = match profile {
        SearchProfile::Backend(backend) => molframe::within(
            positions,
            &query,
            &targets,
            cutoff,
        ...
        SearchProfile::Options(options) => molframe::within_with_options(
            positions,
            &query,
            &targets,
            cutoff,
...
fn periodic_box(cell: Option<&PyUnitCell>) -> PyResult<Option<molframe::PeriodicBox>> {
    cell.map(|value| molframe::PeriodicBox::from_cell(value.cell))
        .transpose()
