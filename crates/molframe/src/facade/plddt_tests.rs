...
fn modelcif_plddt_becomes_a_shared_atom_property_without_changing_b_factors() {
    ...
    let source_pointer = match structure
        .engine()
        .annotations()
        .get(crate::PLDDT_ANNOTATION)
    {
        Some(crate::AtomAnnotation::Real(column)) => column.values().as_ptr(),
        _ => panic!("lowered real pLDDT expected"),
    };
    let dataset = crate::engine::core::DatasetId::new(u64::from(u32::MAX) + 9);
    let first_chunk = crate::engine::core::ChunkId::new(u64::from(u32::MAX) + 17);
    let provider =
        crate::engine::core::PropertyChunkProvider::plddt(dataset, first_chunk, structure.into())
    ...
    assert_eq!(
        chunk
            .value(crate::engine::core::LocalRow::new(4))
            .expect("missing residue atom"),
        crate::engine::core::PropertyValue::Real(0.0, crate::engine::core::Presence::Unknown)
    );
    assert_eq!(
        chunk
            .value(crate::engine::core::LocalRow::new(9))
            .expect("non-polymer atom"),
        crate::engine::core::PropertyValue::Real(0.0, crate::engine::core::Presence::Inapplicable)
    );
    drop(provider);
