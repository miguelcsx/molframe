...
use pdbiox_geom::{Superposition, superpose};
...
pub fn tm_score(model: &[[f32; 3]], reference: &[[f32; 3]]) -> Result<f64, CompareError> {
    ...
    for (moved, &target) in model
        ...
        .zip(reference)
    {
        let distance = pdbiox_geom::distance(moved, target);
...
pub fn weighted_rmsd(
    ...
    for ((model, reference), weight) in model.iter().zip(reference).zip(weights) {
        ...
        weighted_squared += weight * pdbiox_geom::distance_squared(*model, *reference);
        total_weight += weight;
    }
