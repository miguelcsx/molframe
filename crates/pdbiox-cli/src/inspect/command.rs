...
pub(crate) fn assemblies(input: &Path, context: Context) -> Exit {
    ...
    let rows = structure
        .assembly_set()
        .into_iter()
        .flat_map(pdbiox::AssemblySet::assemblies)
        .map(|assembly| {
            vec![
...
pub(crate) fn sequence(input: &Path, ccd: &Path, ccd_version: &str, context: Context) -> Exit {
    ...
    let chains =
        match pdbiox::compare::chain_sequences(&structure, &provider, context.policy.identifiers) {
            Ok(chains) => chains,
            Err(finding) => {
...
pub(super) fn value_text(value: Option<&pdbiox::CifValue>) -> String {
    match value {
        Some(pdbiox::CifValue::Inapplicable) => ".".to_owned(),
        Some(pdbiox::CifValue::Unknown) | None => "?".to_owned(),
        Some(pdbiox::CifValue::Text(value)) => value.to_string(),
        Some(pdbiox::CifValue::Integer(value)) => value.to_string(),
        Some(pdbiox::CifValue::Float(value)) => value.to_string(),
    }
}
