...
use pdbiox::core::{Backpressure, BatchDemand, BatchSource, execution::Retained};
use std::path::Path;
    ...
    let result = pdbiox::analysis::sasa_stream(
        ...
        |frame, time, area| {
            ...
            output.map_err(|error| {
                pdbiox::traj::TrajectoryError::SourceIo {
                    format: "SASA output",
                    kind: error.kind(),
                }
                .into()
...
fn read_radii(
    ...
    let mut source =
        pdbiox::open_structure_batches(path, &options, context.execution).map_err(|error| {
        ...
        match source
            ...
            Backpressure::Ready(lease) => {
                ...
                for &element in batch.elements() {
                    ...
                    let element = pdbiox::Element::from_atomic_number(element);
                    let Some(radius) = pdbiox::vdw_radius(element, set) else {
