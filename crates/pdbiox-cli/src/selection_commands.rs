...
use pdbiox::{AtomIndex, ChainIndex, QueryStructure as _};
use std::path::Path;
...
fn write_selection(output: &Path, selected: &pdbiox::Structure, context: Context) -> Exit {
    let table_result = match context.format {
        OutputKind::Arrow => {
            pdbiox::write_atom_ipc_with_metadata(output, selected, context.provenance_metadata())
        }
        OutputKind::Parquet => pdbiox::write_atom_parquet_with_metadata(
            output,
            selected,
...
fn write_structure(output: &Path, selected: &pdbiox::Structure, context: Context) -> Exit {
    match pdbiox::write(output, selected) {
        Ok(()) => {
            emit_written(output, selected, context);
            Exit::Success
        }
...
fn emit_written(output: &Path, selected: &pdbiox::Structure, context: Context) {
    if context.is_json() {
...
fn emit_atoms(structure: &pdbiox::Structure, context: Context) {
...
fn atom_row(structure: &pdbiox::Structure, index: AtomIndex) -> Option<AtomRow> {
