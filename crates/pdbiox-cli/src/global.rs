...
impl GlobalOptions {
    pub(crate) fn context(self) -> Result<Context, pdbiox::PolicyConfigError> {
        let mut policy = pdbiox::AnalysisPolicy::default();
        let mut configured_format = None;
        let mut chemistry = pdbiox::ChemistryConfiguration::default();
        ...
        let local = Path::new("pdbiox.toml");
        if local.is_file() {
        ...
        Ok(Context {
            ...
            missing_element_policy: if self.read_behavior.infer_elements {
                ...
                pdbiox::MissingElementPolicy::PreserveUnknown
            },
            residue_boundary_policy: if self.read_behavior.infer_residue_boundaries {
                ...
                pdbiox::AmbiguousResidueBoundaryPolicy::Reject
            },
        })
    }
}
...
fn apply_configuration(
    path: &Path,
    policy: &mut pdbiox::AnalysisPolicy,
    format: &mut Option<OutputFormat>,
    chemistry: &mut pdbiox::ChemistryConfiguration,
) -> Result<(), pdbiox::PolicyConfigError> {
    let configuration = pdbiox::read_configuration(path)?;
...
fn parse_output_format(value: &str) -> Result<OutputFormat, pdbiox::PolicyConfigError> {
    match value {
        ...
        _ => Err(pdbiox::PolicyConfigError::InvalidValue {
            field: "output.format",
...
fn xdg_policy_path() -> Option<PathBuf> {
    std::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .map(|root| root.join("pdbiox").join("config.toml"))
}
