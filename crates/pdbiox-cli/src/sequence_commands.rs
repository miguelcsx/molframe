//! Standalone sequence file workflows backed by `pdbiox-seq`.
...
use pdbiox::seq::{Alignment, FastaRecord};
...
fn convert(input: &str, from: SequenceFormat, to: SequenceFormat) -> Result<String, String> {
    ...
    let document = pdbiox::seq::read_sequence(&text, sequence_format(from)).map_err(display)?;
    pdbiox::seq::write_sequence(&document, sequence_format(to)).map_err(display)
}
...
fn align(
    ...
    let alignment = match mode {
        PairwiseMode::Global => {
            pdbiox::seq::global_matrix(left, right, &matrix, gap_open, gap_extend)
        }
        PairwiseMode::Local => {
            pdbiox::seq::local_matrix(left, right, &matrix, gap_open, gap_extend)
        }
        PairwiseMode::SemiGlobal => {
            pdbiox::seq::semi_global_matrix(left, right, &matrix, gap_open, gap_extend)
        }
    }
    .map_err(display)?;
    ...
    Ok(pdbiox::seq::write_fasta(&output))
}

fn msa(input: &str, scoring: pdbiox::seq::Scoring, passes: usize) -> Result<String, String> {
    let text = read_text(input)?;
    ...
    Ok(pdbiox::seq::write_fasta(&records))
}
...
fn kmer(
    ...
    match operation {
        KmerOperation::Counts => Ok(pdbiox::seq::kmer_counts(&record.sequence, k)
            .into_iter()
        ...
        KmerOperation::Minimizers => {
            ...
            Ok(pdbiox::seq::minimizers(&record.sequence, k, window)
                .into_iter()
...
fn tree(input: &str, method: TreeMethod) -> Result<String, String> {
    ...
    let tree = match method {
        TreeMethod::NeighborJoining => pdbiox::seq::neighbor_joining(&label_refs, &distances),
        TreeMethod::Upgma => pdbiox::seq::upgma(&label_refs, &distances),
    };
    tree.map(|value| value.to_newick())
        .ok_or_else(|| "distance matrix cannot produce a tree".to_owned())
}

const fn sequence_format(format: SequenceFormat) -> pdbiox::seq::SequenceFormat {
    match format {
        SequenceFormat::Fasta => pdbiox::seq::SequenceFormat::Fasta,
        SequenceFormat::Fastq => pdbiox::seq::SequenceFormat::Fastq,
        SequenceFormat::A2m => pdbiox::seq::SequenceFormat::A2m,
        SequenceFormat::A3m => pdbiox::seq::SequenceFormat::A3m,
        SequenceFormat::Clustal => pdbiox::seq::SequenceFormat::Clustal,
        SequenceFormat::Stockholm => pdbiox::seq::SequenceFormat::Stockholm,
        SequenceFormat::Phylip => pdbiox::seq::SequenceFormat::Phylip,
    }
}

fn fasta_records(text: &str) -> Result<Vec<FastaRecord>, String> {
    match pdbiox::seq::read_sequence(text, pdbiox::seq::SequenceFormat::Fasta).map_err(display)? {
        pdbiox::seq::SequenceDocument::Records(records) => Ok(records),
...
fn substitution_matrix(choice: MatrixChoice) -> Result<pdbiox::seq::SubstitutionMatrix, String> {
    match choice {
        MatrixChoice::Blosum62 => Ok(pdbiox::seq::blosum62()),
        MatrixChoice::Identity => {
            pdbiox::seq::load_matrix(pdbiox::seq::MatrixProfile::Identity).map_err(display)
        }
        MatrixChoice::Nuc44 => {
            pdbiox::seq::load_matrix(pdbiox::seq::MatrixProfile::Nuc44).map_err(display)
        }
    }
}
