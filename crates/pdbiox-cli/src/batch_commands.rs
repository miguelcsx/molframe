...
fn convert(patterns: &[String], format: &str, outdir: &Path, context: Context) -> Exit {
    ...
    let results = run(&jobs, context.execution, |(input, output)| {
        ...
        let (structure, _findings) = pdbiox::read_with_options(
            input,
            &pdbiox::ReadOptions::new()
                .mode(context.mode)
                .missing_element_policy(context.missing_element_policy)
        ...
        .map_err(|findings| findings_message(&findings))?;
        pdbiox::write(output, &structure).map_err(|findings| findings_message(&findings))?;
        Ok(vec![
...
fn findings_message(findings: &[pdbiox::Diagnostic]) -> String {
    findings
        .iter()
