...
pub(crate) fn analyse_gdt(
    ...
    let analysis = py
        .detach(|| pdbiox::compare::governed_gdt_with_cutoffs(model, reference, &cutoffs, &policy))
        .map_err(value_error)?;
...
pub(crate) fn analyse_weighted_rmsd(
    ...
    let policy = policy.inner.clone();
    let analysis = py
        .detach(|| pdbiox::compare::governed_weighted_rmsd(model, reference, &weights, &policy))
        .map_err(value_error)?;
    analysis_with_value(py, analysis, |py, value| {
        Ok(PyFloat::new(py, value).unbind().into_any())
    })
...
pub(crate) fn analyse_ce_align(
    ...
    let policy = policy.inner.clone();
    let analysis = py
        .detach(|| pdbiox::compare::governed_ce_align(reference, mobile, options.inner(), &policy))
        .map_err(value_error)?;
    analysis_with_value(py, analysis, |py, value| {
