...
macro_rules! score {
    ($name:ident, $kernel:path) => {
        ...
        pub(crate) fn $name(
            ...
            let model = coordinates(model)?;
            let reference = coordinates(reference)?;
            py.detach(|| $kernel(&model, &reference))
                .map_err(value_error)
        }
    };
}

score!(tm_score, pdbiox::compare::tm_score);
score!(gdt_ts, pdbiox::compare::gdt_ts);
score!(gdt_ha, pdbiox::compare::gdt_ha);
...
#[derive(Clone, Debug)]
pub(crate) struct PyLddtOptions(pdbiox::compare::LddtOptions);
...
impl PyLddtOptions {
    ...
    fn new(
        ...
        Self(pdbiox::compare::LddtOptions {
            inclusion_radius,
    ...
    fn standard(inclusion_radius: f64) -> Self {
        Self(pdbiox::compare::LddtOptions::standard(inclusion_radius))
    }
}
...
pub(crate) fn gdt(
    ...
    py.detach(|| pdbiox::compare::gdt_with_cutoffs(&model, &reference, &cutoffs))
        .map_err(value_error)
}
...
pub(crate) struct PyCeOptions(pdbiox::compare::CeOptions);

impl PyCeOptions {
    pub(crate) const fn inner(self) -> pdbiox::compare::CeOptions {
        self.0
    }
}
...
impl PyCeOptions {
    ...
    #[staticmethod]
    fn original() -> Self {
        Self(pdbiox::compare::CeOptions::original())
    }
}
...
pub(crate) fn ce_align(
    ...
    py.detach(|| pdbiox::compare::ce_align(&reference, &mobile, options.0))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn ce_alignments(
    ...
    let reference = coordinates(reference)?;
    let mobile = coordinates(mobile)?;
    py.detach(|| pdbiox::compare::ce_alignments(&reference, &mobile, options.0))
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyQsOptions(pdbiox::compare::QsOptions);

#[pymethods]
impl PyQsOptions {
    #[new]
    fn new(contact_distance: f32, empty_policy: PyEmptyQsPolicy) -> Self {
        Self(pdbiox::compare::QsOptions {
            contact_distance,
            empty_policy: empty_policy.into(),
        })
    }
    ...
    fn standard(contact_distance: f32) -> Self {
        Self(pdbiox::compare::QsOptions::standard(contact_distance))
    }
}

#[pyclass(name = "DockQOptions", frozen, from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyDockQOptions(pdbiox::compare::DockQOptions);

#[pymethods]
impl PyDockQOptions {
    #[new]
    fn new(contact_distance: f32, ligand_scale: f64, interface_scale: f64) -> Self {
        Self(pdbiox::compare::DockQOptions {
            contact_distance,
            ligand_scale,
            interface_scale,
        })
...
pub(crate) fn qs_score(
    ...
    py.detach(|| {
        pdbiox::compare::qs_score_in_namespace(
            &model,
            &native,
            &first_chain,
            &second_chain,
...
pub(crate) fn dockq(
    ...
    py.detach(|| {
        pdbiox::compare::dockq_in_namespace(
            &model,
            &native,
            &receptor,
            &ligand,
...
pub(crate) fn weighted_rmsd(
    ...
    let model = coordinates(model)?;
    let reference = coordinates(reference)?;
    py.detach(move || pdbiox::compare::weighted_rmsd(&model, &reference, &weights))
        .map_err(value_error)
}
...
governed_score!(analyse_tm_score, pdbiox::compare::governed_tm_score);
governed_score!(analyse_gdt_ts, pdbiox::compare::governed_gdt_ts);
governed_score!(analyse_gdt_ha, pdbiox::compare::governed_gdt_ha);
...
pub(crate) fn analyse_dockq(
    ...
    let analysis = py
        .detach(|| {
            pdbiox::compare::governed_dockq(&model, &native, receptor, ligand, options.0, &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_qs_score(
    ...
    let analysis = py
        .detach(|| {
            pdbiox::compare::governed_qs_score(
                &model,
                &native,
                first_chain,
                second_chain,
...
fn scalar_analysis(py: Python<'_>, analysis: pdbiox::Analysis<f64>) -> PyResult<PyAnalysis> {
    analysis_with_value(py, analysis, |py, value| {
        Ok(PyFloat::new(py, value).unbind().into_any())
    })
}

impl From<PyEmptyLddtPolicy> for pdbiox::compare::EmptyLddtPolicy {
...
impl From<PyCeSignificanceProfile> for pdbiox::compare::CeSignificanceProfile {
...
impl From<PyEmptyQsPolicy> for pdbiox::compare::EmptyQsPolicy {
...
impl From<pdbiox::compare::CeAlignment> for PyCeAlignment {
    fn from(value: pdbiox::compare::CeAlignment) -> Self {
        Self {
...
impl From<pdbiox::compare::DockQ> for PyDockQ {
    fn from(value: pdbiox::compare::DockQ) -> Self {
        Self {
            fnat: value.fnat,
