...
pub(crate) fn chain_sequences(
    ...
    py.detach(move || {
        pdbiox::compare::chain_sequences(&structure, dictionary.as_ref(), namespace.into())
    })
...
pub(crate) fn analyse_assign_chains(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_assign_chains(
                &reference,
                &target,
                dictionary.as_ref(),
                scoring.inner(),
...
pub(crate) fn map_sequence_to_structure(
    ...
    let dictionary = dictionary.0.clone();
    py.detach(move || {
        pdbiox::compare::map_sequence_to_structure(
            &query,
            &structure,
            dictionary.as_ref(),
            namespace.into(),
...
pub(crate) fn analyse_map_sequence_to_structure(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_map_sequence_to_structure(
                &query,
                &structure,
                dictionary.as_ref(),
                scoring.inner(),
...
impl From<pdbiox::compare::ChainSequence> for PyChainSequence {
    fn from(value: pdbiox::compare::ChainSequence) -> Self {
        Self {
            label: value.label,
...
impl From<pdbiox::compare::ChainMapping> for PyChainMapping {
    fn from(value: pdbiox::compare::ChainMapping) -> Self {
        Self {
...
impl From<pdbiox::compare::ChainAlternative> for PyChainAlternative {
    fn from(value: pdbiox::compare::ChainAlternative) -> Self {
        Self {
            reference: value.reference,
            target: value.target,
            identity: value.identity,
...
impl From<pdbiox::compare::ChainAssignment> for PyChainAssignment {
    fn from(value: pdbiox::compare::ChainAssignment) -> Self {
        Self {
...
fn assignment_value(
    py: Python<'_>,
    value: pdbiox::compare::ChainAssignment,
) -> PyResult<Py<PyAny>> {
    Ok(Py::new(py, PyChainAssignment::from(value))?.into_any())
}

impl From<pdbiox::compare::ResidueMatch> for PyResidueMatch {
    fn from(value: pdbiox::compare::ResidueMatch) -> Self {
        Self {
...
fn residue_match_list(
    py: Python<'_>,
    values: Vec<pdbiox::compare::ResidueMatch>,
) -> PyResult<Py<PyAny>> {
    let items = values
        .into_iter()
