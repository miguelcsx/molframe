//! Common typed sequence-format dispatch delegated to `pdbiox-seq`.
...
#[derive(Clone, Debug)]
pub(crate) struct PySequenceDocument(pdbiox::seq::SequenceDocument);

#[pymethods]
impl PySequenceDocument {
    #[staticmethod]
    fn records(records: Vec<PyFastaRecord>) -> Self {
        Self(pdbiox::seq::SequenceDocument::Records(
    ...
    fn fastq(records: Vec<PyFastqRecord>) -> Self {
        Self(pdbiox::seq::SequenceDocument::Fastq(
            records.into_iter().map(Into::into).collect(),
        ))
    }
    ...
    fn tree(tree: &PyTree) -> Self {
        Self(pdbiox::seq::SequenceDocument::Tree(tree.0.clone()))
    }

    #[getter]
    const fn kind(&self) -> PySequenceDocumentKind {
        match &self.0 {
            pdbiox::seq::SequenceDocument::Records(_) => PySequenceDocumentKind::Records,
            pdbiox::seq::SequenceDocument::Fastq(_) => PySequenceDocumentKind::Fastq,
            pdbiox::seq::SequenceDocument::Tree(_) => PySequenceDocumentKind::Tree,
        }
    }

    fn record_values(&self) -> PyResult<Vec<PyFastaRecord>> {
        match &self.0 {
            pdbiox::seq::SequenceDocument::Records(records) => {
    ...
    fn fastq_values(&self) -> PyResult<Vec<PyFastqRecord>> {
        match &self.0 {
            pdbiox::seq::SequenceDocument::Fastq(records) => {
                Ok(records.iter().cloned().map(Into::into).collect())
            }
    ...
    fn tree_value(&self) -> PyResult<PyTree> {
        match &self.0 {
            pdbiox::seq::SequenceDocument::Tree(tree) => Ok(PyTree(tree.clone())),
...
pub(crate) fn read_sequence(
    ...
    py.detach(move || pdbiox::seq::read_sequence(&text, format.into()))
...
pub(crate) fn write_sequence(
    ...
    py.detach(move || pdbiox::seq::write_sequence(&document, format.into()))
        .map_err(value_error)
}

impl From<PySequenceFormat> for pdbiox::seq::SequenceFormat {
