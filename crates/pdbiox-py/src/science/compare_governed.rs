...
pub(crate) fn analyse_gdt(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_gdt_with_cutoffs(&model, &reference, &cutoffs, &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_weighted_rmsd(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_weighted_rmsd(&model, &reference, &weights, &policy)
        })
        .map_err(value_error)?;
    analysis_with_value(py, analysis, |py, value| {
        Ok(PyFloat::new(py, value).unbind().into_any())
...
pub(crate) fn analyse_ce_align(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_ce_align(&reference, &mobile, options.inner(), &policy)
        })
        .map_err(value_error)?;
    analysis_with_value(py, analysis, |py, value| {
...
pub(crate) fn analyse_ce_alignments(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_ce_alignments(&reference, &mobile, options.inner(), &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_map_chains(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::compare::governed_map_chains(
                &reference,
                &target,
                dictionary.as_ref(),
                scoring.inner(),
...
fn ce_alignment_list(
    py: Python<'_>,
    values: Vec<pdbiox::compare::CeAlignment>,
) -> PyResult<Py<PyAny>> {
    let items = values
        .into_iter()
...
fn chain_mapping_list(
    py: Python<'_>,
    values: Vec<pdbiox::compare::ChainMapping>,
) -> PyResult<Py<PyAny>> {
    let items = values
        .into_iter()
