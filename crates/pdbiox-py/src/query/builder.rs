...
#[derive(Clone, Debug)]
pub(crate) struct PyQueryBuilder(pub(crate) pdbiox::query::Builder);

#[pymethods]
impl PyQueryBuilder {
    fn query(&self) -> PyQuery {
        PyQuery {
            inner: pdbiox::Query::from_builder(self.0.clone()),
        }
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyColumnBuilder(pdbiox::query::ColumnBuilder);
...
impl PyColumns {
    ...
    fn all() -> PyQueryBuilder {
        PyQueryBuilder(pdbiox::query::col::all())
    }

    #[staticmethod]
    fn none() -> PyQueryBuilder {
        PyQueryBuilder(pdbiox::query::col::none())
    }

    #[staticmethod]
    fn is_protein() -> PyQueryBuilder {
        PyQueryBuilder(pdbiox::query::col::is_protein())
    }

    #[staticmethod]
    fn name() -> PyColumnBuilder {
        PyColumnBuilder(pdbiox::query::col::name())
    }

    #[staticmethod]
    fn resname() -> PyColumnBuilder {
        PyColumnBuilder(pdbiox::query::col::resname())
    }

    #[staticmethod]
    fn chain() -> PyColumnBuilder {
        PyColumnBuilder(pdbiox::query::col::chain())
    }

    #[staticmethod]
    fn bfactor() -> PyColumnBuilder {
        PyColumnBuilder(pdbiox::query::col::bfactor())
    }

    #[staticmethod]
    fn occupancy() -> PyColumnBuilder {
        PyColumnBuilder(pdbiox::query::col::occupancy())
    }

    #[staticmethod]
    fn x() -> PyColumnBuilder {
        PyColumnBuilder(pdbiox::query::col::x())
    }

    #[staticmethod]
    fn within(radius: f32, target: &PyQueryBuilder) -> PyQueryBuilder {
        PyQueryBuilder(pdbiox::query::col::within(radius, target.0.clone()))
    }

    #[staticmethod]
    fn by_residue(target: &PyQueryBuilder) -> PyQueryBuilder {
        PyQueryBuilder(pdbiox::query::col::by_residue(target.0.clone()))
    }
}
