...
pub(crate) fn circular_summary(
    ...
    py.detach(move || {
        ...
        pdbiox::circular_summary(&angles, indeterminate_tolerance)
    })
...
pub(crate) fn torus_summary(
    ...
    py.detach(move || {
        ...
        pdbiox::torus_summary(&points, indeterminate_tolerance)
    })
    .map(|values| {
        values
            .iter()
...
pub(crate) fn rotation_mean(
    ...
    py.detach(move || {
        ...
        pdbiox::rotation_mean_with_options(&rotations, options)
    })
...
pub(crate) fn path_torsions(
    ...
    Ok(py.detach(move || {
        ...
        pdbiox::path_torsions(&positions, maximum)
    }))
}
...
pub(crate) fn backbone_frames(
    ...
    Ok(py
        .detach(move || {
            let positions = materialize_optional_positions(positions, present);
            pdbiox::backbone_frames(&positions)
        })
        .into_iter()
...
pub(crate) fn helix_geometry(
    ...
    py.detach(move || {
        let positions = materialize_optional_positions(positions, present);
        pdbiox::helix_geometry_with_options(&positions, options)
    })
...
fn periodic_angles(values: &[f64]) -> Result<Vec<pdbiox::PeriodicAngle>, pdbiox::PeriodicError> {
    values
        .iter()
        .copied()
...
fn rotations_from_matrices(
    matrices: &[f64],
    matrix_tolerance: f64,
) -> Result<Vec<pdbiox::Rotation3>, pdbiox::RotationError> {
    matrices
        .chunks_exact(9)
        .map(|matrix| {
