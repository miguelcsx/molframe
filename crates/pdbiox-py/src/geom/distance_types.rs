...
struct DistanceMatrixOwner {
    matrix: Arc<pdbiox::DistanceMatrix>,
}

/// An immutable, row-major dense matrix of pairwise distances.
#[pyclass(name = "DistanceMatrix", frozen, skip_from_py_object)]
#[derive(Clone, Debug)]
pub(crate) struct PyDistanceMatrix(pub(crate) Arc<pdbiox::DistanceMatrix>);

impl PyDistanceMatrix {
    pub(crate) fn from_native(matrix: pdbiox::DistanceMatrix) -> Self {
        Self(Arc::new(matrix))
    }
}
...
impl PyDistanceMatrix {
    ...
    fn from_positions(py: Python<'_>, positions: PyReadonlyArray2<'_, f32>) -> PyResult<Self> {
        ...
        py.detach(move || pdbiox::distance_matrix(positions))
    ...
    fn from_sets(
        ...
        py.detach(move || pdbiox::distance_matrix_between(left, right))
            .map(Self::from_native)
            .map_err(matrix_error)
    }
