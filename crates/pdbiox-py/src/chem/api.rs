...
pub(crate) fn component_coverage(
    ...
    py.detach(move || pdbiox::component_coverage(&structure, &provider, &policy))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn apply_component_chemistry(
    ...
    let structure = structure.structure().clone();
    let provider = extract_provider(provider)?;
    let policy = policy.map_or(pdbiox::PolymerLinkPolicy::Disabled, |value| value.0);
    py.detach(move || pdbiox::apply_component_chemistry(&structure, &provider, policy))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn apply_polymer_role_profile(
    ...
    py.detach(move || pdbiox::apply_polymer_role_profile(&structure, &provider, &profile))
        .map(Into::into)
        .map_err(value_error)
}
