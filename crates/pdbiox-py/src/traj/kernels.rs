...
impl From<pdbiox::traj::MeanSquaredDisplacement> for PyMeanSquaredDisplacement {
    fn from(value: pdbiox::traj::MeanSquaredDisplacement) -> Self {
        Self {
            lag: value.lag,
...
impl From<pdbiox::traj::DielectricEstimate> for PyDielectricEstimate {
    fn from(value: pdbiox::traj::DielectricEstimate) -> Self {
        Self {
...
pub(crate) fn kmeans(
    ...
    py.detach(move || {
        pdbiox::traj::kmeans_view(observations, rows, columns, options.native())
            .map(PyKMeans::from)
            .map_err(value_error)
    })
}
...
pub(crate) fn mean_squared_displacement(
    ...
    py.detach(move || {
        pdbiox::traj::mean_squared_displacement_view(view, atoms, maximum_lag)
...
pub(crate) fn trajectory_dielectric_from_dipoles(
    ...
    py.detach(move || {
        pdbiox::traj::dielectric_from_dipoles(
            &dipoles,
            pdbiox::traj::DielectricOptions {
...
pub(crate) fn rmsd_to_reference(
    ...
    let view = borrowed_frames(&frames)?;
    py.detach(move || {
        pdbiox::traj::rmsd_to_reference_view(view, reference, alignment.into()).map_err(value_error)
    })
}
...
pub(crate) fn pairwise_fitted_rmsd(
    ...
    let view = borrowed_frames(&frames)?;
    py.detach(move || {
        pdbiox::traj::pairwise_fitted_rmsd_view(view, memory_limit)
            .map(Into::into)
            .map_err(value_error)
    })
}
...
pub(crate) fn pairwise_torus_distance(
    ...
    let weights = weights
        ...
        .map_err(|_| PyValueError::new_err(CONTIGUOUS))?
        .to_vec();
    let metric = pdbiox::TorusMetric::new(weights).map_err(value_error)?;
    py.detach(move || {
        pdbiox::traj::pairwise_torus_distance(&points, &metric, memory_limit)
            .map(Into::into)
            .map_err(value_error)
    })
}
...
pub(crate) fn generalized_procrustes_mean(
    ...
    let result = py
        .detach(move || {
            pdbiox::traj::generalized_procrustes_mean_view(view, tolerance, maximum_iterations)
        })
        .map_err(value_error)?;
...
fn periodic_points(values: PyReadonlyArray2<'_, f64>) -> PyResult<Vec<Vec<pdbiox::PeriodicAngle>>> {
    values
        .as_slice()
        .map_err(|_| PyValueError::new_err(CONTIGUOUS))?;
    values
        ...
        .map(|row| {
            row.iter()
                .copied()
                .map(pdbiox::PeriodicAngle::from_radians)
