...
#[derive(Clone, Debug)]
enum TransformSpec {
    Rigid(pdbiox::traj::RigidTransform),
...
pub(crate) struct PyRigidTransform {
    inner: pdbiox::traj::RigidTransform,
}
...
impl PyRigidTransform {
    ...
    fn new(offset: [f64; 3]) -> Self {
        Self {
            inner: pdbiox::traj::RigidTransform::translation(offset),
        }
    }
...
fn apply_transforms(frame: &mut PyTimestep, transforms: &[TransformSpec]) -> PyResult<()> {
    let mut native: pdbiox::traj::Timestep = frame.clone().try_into()?;
    for transform in transforms {
        use pdbiox::traj::FrameTransform;
        match transform {
            TransformSpec::Rigid(value) => value.apply(&mut native),
            TransformSpec::Center(atoms, target) => {
                pdbiox::traj::Center::geometric(atoms.clone(), *target).apply(&mut native)
            }
            TransformSpec::Fit(atoms, reference) => {
                pdbiox::traj::Fit::new(atoms.clone(), reference.clone()).apply(&mut native)
            }
            TransformSpec::Wrap(groups) => match groups {
                Some(groups) => pdbiox::traj::Wrap::groups(groups.clone()).apply(&mut native),
                None => pdbiox::traj::Wrap::atoms().apply(&mut native),
            },
            TransformSpec::Unwrap(bonds) => {
                pdbiox::traj::Unwrap::molecules(bonds.clone()).apply(&mut native)
            }
        }
