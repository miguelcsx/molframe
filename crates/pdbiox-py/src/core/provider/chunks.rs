...
impl From<pdbiox::PropertyKind> for PyPropertyKind {
    fn from(value: pdbiox::PropertyKind) -> Self {
        match value {
            pdbiox::PropertyKind::Boolean => Self::Boolean,
            pdbiox::PropertyKind::Integer => Self::Integer,
            pdbiox::PropertyKind::Real => Self::Real,
            pdbiox::PropertyKind::Symbol => Self::Symbol,
        }
    }
}
...
pub(crate) struct PyPropertyValue(pdbiox::PropertyValue);

#[pyclass(name = "AtomEndpoint", frozen, from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyAtomEndpoint(pdbiox::AtomEndpoint);

#[pymethods]
impl PyAtomEndpoint {
    #[new]
    fn new(dataset: PyDatasetId, row: PyLogicalRow) -> Self {
        Self(pdbiox::AtomEndpoint::new(dataset.0, row.0))
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBondChunkRecord(pdbiox::BondChunkRecord);
...
#[derive(Clone, Debug)]
pub(crate) struct PyBondChunk(pub(crate) pdbiox::BondChunk);
...
impl PyPropertyValue {
    ...
    fn kind(&self) -> PyPropertyKind {
        match self.0 {
            pdbiox::PropertyValue::Boolean(_, _) => PyPropertyKind::Boolean,
            pdbiox::PropertyValue::Integer(_, _) => PyPropertyKind::Integer,
            pdbiox::PropertyValue::Real(_, _) => PyPropertyKind::Real,
            pdbiox::PropertyValue::Symbol(_, _) => PyPropertyKind::Symbol,
        }
    }
    ...
    fn presence(&self) -> PyPresence {
        match self.0 {
            pdbiox::PropertyValue::Boolean(_, presence)
            | pdbiox::PropertyValue::Integer(_, presence)
            | pdbiox::PropertyValue::Real(_, presence)
            | pdbiox::PropertyValue::Symbol(_, presence) => presence.into(),
        }
    }
    ...
    fn boolean(&self) -> Option<bool> {
        match self.0 {
            pdbiox::PropertyValue::Boolean(value, _) => Some(value),
            _ => None,
        }
    }
    ...
    fn integer(&self) -> Option<i64> {
        match self.0 {
            pdbiox::PropertyValue::Integer(value, _) => Some(value),
            _ => None,
        }
    }
    ...
    fn real(&self) -> Option<f64> {
        match self.0 {
            pdbiox::PropertyValue::Real(value, _) => Some(value),
            _ => None,
        }
    }
    ...
    fn symbol(&self) -> Option<PySymbolId> {
        match self.0 {
            pdbiox::PropertyValue::Symbol(value, _) => Some(PySymbolId(value)),
            _ => None,
        }
    }
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyStructureChunk(pub(crate) pdbiox::StructureChunk);
...
#[derive(Clone, Debug)]
pub(crate) struct PyPropertyChunk(pub(crate) pdbiox::PropertyChunk);
...
#[derive(Clone, Debug)]
pub(crate) struct PyFrameChunk(pub(crate) pdbiox::FrameChunk);
...
impl PyFrameChunk {
    ...
    fn new(
        ...
        pdbiox::FrameChunk::shared(descriptor.0, coordinates.0, start..end)
            .map(Self)
            .map_err(value_error)
    }
