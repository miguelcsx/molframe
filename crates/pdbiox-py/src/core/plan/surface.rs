...
impl PySasa {
    ...
    fn execute<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, numpy::PyArray1<f64>>> {
        match execute_operation(py, &self.operation)? {
            ...
            pdbiox::SurfaceValue::BuriedSurface(_) => Err(incompatible_result()),
        }
    }
...
impl PyBuriedSurfaceOperation {
    ...
    fn execute(&self, py: Python<'_>) -> PyResult<PyBuriedSurface> {
        match execute_operation(py, &self.operation)? {
            pdbiox::SurfaceValue::BuriedSurface(value) => Ok(value.into()),
            pdbiox::SurfaceValue::SolventAccessibleSurface(_) => Err(incompatible_result()),
        }
    }
...
fn execute_operation(
    ...
    match entry.value {
        pdbiox::PlanValue::Surface(value) => Ok(*value),
...
) -> PyResult<pdbiox::SurfaceRequest> {
    match operation {
        PySurfaceOperation::SolventAccessibleSurface {
            ...
            probe,
            sample_points,
        } => Ok(pdbiox::SurfaceRequest::SolventAccessibleSurface {
        ...
        PySurfaceOperation::BuriedSurface {
            ...
            probe,
            sample_points,
        } => Ok(pdbiox::SurfaceRequest::BuriedSurface {
            positions: retain_coordinate(py, arrays, coordinate_slots, positions)?,
            radii: retain_float(py, radii, floats, float_slots)?,
