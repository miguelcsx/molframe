...
impl CoordinateMetric {
    ...
    pub(super) const fn native(self) -> pdbiox::ComparisonMetric {
        match self {
            Self::Lddt { inclusion_radius } => pdbiox::ComparisonMetric::Lddt { inclusion_radius },
            Self::TmScore => pdbiox::ComparisonMetric::TmScore,
            Self::GdtTs => pdbiox::ComparisonMetric::GdtTs,
            Self::GdtHa => pdbiox::ComparisonMetric::GdtHa,
        }
    }
}
...
impl PyRmsd {
    ...
    fn execute(&self, py: Python<'_>) -> PyResult<f64> {
        ...
        match entry.value {
            pdbiox::PlanValue::Rmsd(value) => Ok(value),
            _ => Err(PyValueError::new_err(
