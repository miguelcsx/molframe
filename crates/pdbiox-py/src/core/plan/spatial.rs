...
impl PyNeighborPairs {
    ...
    fn execute(&self, py: Python<'_>) -> PyResult<PyNeighborTable> {
        match execute_operation(py, &self.operation)? {
            pdbiox::SpatialValue::NeighborPairs(values) => neighbor_table_from_pairs(py, values),
            pdbiox::SpatialValue::AtomsWithin(_) => Err(incompatible_result()),
        }
    }
...
impl PyAtomsWithin {
    ...
    fn execute<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyArray1<u32>>> {
        match execute_operation(py, &self.operation)? {
            pdbiox::SpatialValue::AtomsWithin(value) => Ok(atom_indices_to_python(py, value)),
            pdbiox::SpatialValue::NeighborPairs(_) => Err(incompatible_result()),
        }
    }
...
fn execute_operation(
    ...
    match entry.value {
        pdbiox::PlanValue::Spatial(value) => Ok(*value),
...
) -> PyResult<pdbiox::SpatialRequest> {
    match operation {
        PySpatialOperation::NeighborPairs {
            ...
            Ok(pdbiox::SpatialRequest::NeighborPairs {
                positions: slot,
        ...
        PySpatialOperation::AtomsWithin {
            ...
            let slot = retain_positions(py, positions, arrays, coordinate_slots)?;
            let count = arrays[slot].shape()[0];
            Ok(pdbiox::SpatialRequest::AtomsWithin {
                positions: slot,
