...
#[derive(Clone, Debug)]
pub(crate) struct PyDensityMap(pub(crate) pdbiox::xtal::DensityMap);
...
impl PyDensityMap {
    ...
    fn from_mrc_bytes(data: &[u8]) -> PyResult<Self> {
        pdbiox::xtal::DensityMap::from_mrc_bytes(data)
            .map(Self)
            .map_err(value_error)
    }
...
impl From<pdbiox::xtal::MapStatistics> for PyMapStatistics {
    fn from(value: pdbiox::xtal::MapStatistics) -> Self {
        Self {
            count: value.count,
...
impl From<pdbiox::xtal::MapHistogram> for PyMapHistogram {
    fn from(value: pdbiox::xtal::MapHistogram) -> Self {
        Self {
            minimum: value.minimum,
            maximum: value.maximum,
            counts: value.counts,
