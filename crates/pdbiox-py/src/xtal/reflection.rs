...
#[derive(Clone, Debug)]
pub(crate) struct PyReflectionValue(pub(crate) pdbiox::xtal::ReflectionValue);
...
impl PyReflectionValue {
    ...
    fn new(py: Python<'_>, kind: &str, value: Option<&Bound<'_, PyAny>>) -> PyResult<Self> {
        let value = match kind {
            "missing" => pdbiox::xtal::ReflectionValue::Missing,
            "inapplicable" => pdbiox::xtal::ReflectionValue::Inapplicable,
            "integer" => pdbiox::xtal::ReflectionValue::Integer(required(value, kind)?.extract()?),
            "real" => pdbiox::xtal::ReflectionValue::Real(required(value, kind)?.extract()?),
            "text" => pdbiox::xtal::ReflectionValue::Text(
    ...
    #[staticmethod]
    fn missing() -> Self {
        Self(pdbiox::xtal::ReflectionValue::Missing)
    }

    #[staticmethod]
    fn inapplicable() -> Self {
        Self(pdbiox::xtal::ReflectionValue::Inapplicable)
    }

    #[getter]
    fn kind(&self) -> &'static str {
        match self.0 {
            pdbiox::xtal::ReflectionValue::Missing => "missing",
            pdbiox::xtal::ReflectionValue::Inapplicable => "inapplicable",
            pdbiox::xtal::ReflectionValue::Integer(_) => "integer",
            pdbiox::xtal::ReflectionValue::Real(_) => "real",
            pdbiox::xtal::ReflectionValue::Text(_) => "text",
        }
    }
    ...
    fn value(&self, py: Python<'_>) -> Py<PyAny> {
        match &self.0 {
            pdbiox::xtal::ReflectionValue::Missing
            | pdbiox::xtal::ReflectionValue::Inapplicable => py.None(),
            pdbiox::xtal::ReflectionValue::Integer(value) => {
                PyInt::new(py, *value).unbind().into_any()
            }
            pdbiox::xtal::ReflectionValue::Real(value) => {
                PyFloat::new(py, *value).unbind().into_any()
            }
            pdbiox::xtal::ReflectionValue::Text(value) => {
...
#[derive(Clone, Debug)]
pub(crate) struct PyReflectionColumn(pub(crate) pdbiox::xtal::ReflectionColumn);
...
impl PyReflectionColumn {
    ...
    fn new(
        ...
        let mtz_type = mtz_type
            ...
            .transpose()?;
        Ok(Self(pdbiox::xtal::ReflectionColumn {
            label: label.into(),
...
#[derive(Clone, Debug)]
pub(crate) struct PyReflectionDataset(pub(crate) pdbiox::xtal::ReflectionDataset);
...
impl PyReflectionDataset {
    ...
    fn new(
        ...
        Self(pdbiox::xtal::ReflectionDataset {
            id,
...
#[derive(Clone, Debug)]
pub(crate) struct PyReflectionTable(pub(crate) pdbiox::xtal::ReflectionTable);
...
impl PyReflectionTable {
    ...
    fn new(
        ...
        Ok(Self(pdbiox::xtal::ReflectionTable {
...
pub(crate) fn read_mtz(py: Python<'_>, data: &[u8]) -> PyResult<PyReflectionTable> {
    ...
    py.detach(move || pdbiox::xtal::read_mtz(&data))
...
pub(crate) fn write_mtz(py: Python<'_>, table: &PyReflectionTable) -> PyResult<Py<PyBytes>> {
    ...
    py.detach(move || pdbiox::xtal::write_mtz(&table))
...
impl From<PyReflectionColumnType> for pdbiox::xtal::ReflectionColumnType {
...
impl From<pdbiox::xtal::ReflectionColumnType> for PyReflectionColumnType {
    fn from(value: pdbiox::xtal::ReflectionColumnType) -> Self {
        match value {
            pdbiox::xtal::ReflectionColumnType::MillerIndex => Self::MillerIndex,
            pdbiox::xtal::ReflectionColumnType::Amplitude => Self::Amplitude,
            pdbiox::xtal::ReflectionColumnType::Intensity => Self::Intensity,
            pdbiox::xtal::ReflectionColumnType::StandardDeviation => Self::StandardDeviation,
            pdbiox::xtal::ReflectionColumnType::Phase => Self::Phase,
            pdbiox::xtal::ReflectionColumnType::Flag => Self::Flag,
            pdbiox::xtal::ReflectionColumnType::Real => Self::Real,
            pdbiox::xtal::ReflectionColumnType::Text => Self::Text,
        }
    }
}
...
fn reflection_values(values: &Bound<'_, PyAny>) -> PyResult<Vec<pdbiox::xtal::ReflectionValue>> {
...
fn reflection_columns(values: &Bound<'_, PyAny>) -> PyResult<Vec<pdbiox::xtal::ReflectionColumn>> {
    let mut output = Vec::new();
    for value in values.try_iter()? {
        let value = value?;
        let value = value
...
fn unit_cell((lengths, angles): ([f64; 3], [f64; 3])) -> pdbiox::UnitCell {
    pdbiox::UnitCell { lengths, angles }
}

fn cell_arrays(cell: pdbiox::UnitCell) -> ([f64; 3], [f64; 3]) {
