...
pub(crate) struct PyMapBrickId(pub(crate) pdbiox::xtal::MapBrickId);

#[pymethods]
impl PyMapBrickId {
    #[new]
    fn new(value: u64) -> Self {
        Self(pdbiox::xtal::MapBrickId::new(value))
    }
...
pub(crate) struct PyMrcBrickBudget(pub(crate) pdbiox::xtal::MrcBrickBudget);
...
impl PyMrcBrickBudget {
    ...
    #[pyo3(signature = (
        *,
        max_payload_bytes=pdbiox::xtal::DEFAULT_MRC_BRICK_PAYLOAD_BYTES,
        max_working_set_bytes=pdbiox::xtal::DEFAULT_MRC_BRICK_WORKING_SET_BYTES
    ))]
    fn new(max_payload_bytes: usize, max_working_set_bytes: usize) -> Self {
        Self(pdbiox::xtal::MrcBrickBudget {
            max_payload_bytes,
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMrcBrickOptions(pub(crate) pdbiox::xtal::MrcBrickOptions);
...
impl PyMrcBrickOptions {
    ...
    fn new(
        ...
        Self(pdbiox::xtal::MrcBrickOptions {
            interior,
            halo,
            generation,
            budget: match budget {
                Some(value) => value.0,
                None => pdbiox::xtal::MrcBrickBudget::default(),
            },
        })
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMapBrickAddress(pdbiox::xtal::MapBrickAddress);
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMapBrickShape(pdbiox::xtal::MapBrickShape);
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyScalarBrickMetadata(pdbiox::xtal::ScalarBrickMetadata);
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyMrcBrickDescriptor(pdbiox::xtal::MrcBrickDescriptor);
...
#[derive(Debug)]
pub(crate) struct PyScalarBrickPayload(pub(crate) pdbiox::xtal::ScalarBrickPayload);
...
#[derive(Debug)]
pub(crate) struct PyMrcBrickProvider(pdbiox::xtal::MrcBrickProvider<std::fs::File>);
...
impl PyMrcBrickProvider {
    ...
    fn new(
        ...
        let options = match options {
            Some(value) => value.0,
            None => pdbiox::xtal::MrcBrickOptions::default(),
        };
        pdbiox::xtal::MrcBrickProvider::open(path, dataset.0, first_chunk.0, first_brick.0, options)
            .map(Self)
            .map_err(brick_error)
    }
...
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add(
        ...
        pdbiox::xtal::DEFAULT_MRC_BRICK_PAYLOAD_BYTES,
    )?;
    module.add(
        "DEFAULT_MRC_BRICK_WORKING_SET_BYTES",
        pdbiox::xtal::DEFAULT_MRC_BRICK_WORKING_SET_BYTES,
    )?;
