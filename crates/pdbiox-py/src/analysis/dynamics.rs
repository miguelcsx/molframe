...
impl PyWaterDynamicsOptions {
    const fn native(self) -> pdbiox::analysis::WaterDynamicsOptions {
        pdbiox::analysis::WaterDynamicsOptions {
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyDielectricOptions(pub(crate) pdbiox::analysis::DielectricOptions);

#[pymethods]
impl PyDielectricOptions {
    #[new]
    fn new(volume: f64, temperature: f64, fluctuation_prefactor: f64) -> Self {
        Self(pdbiox::analysis::DielectricOptions {
            volume,
            temperature,
...
pub(crate) fn water_dynamics(
    ...
    py.detach(move || pdbiox::analysis::water_dynamics(&occupancy, options.native()))
...
pub(crate) fn analyse_water_dynamics(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::analysis::governed_water_dynamics(&occupancy, options.native(), &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn dielectric_from_dipoles(
    ...
    py.detach(move || pdbiox::analysis::dielectric_from_dipoles(&dipoles, options.0))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn analyse_dielectric_from_dipoles(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::analysis::governed_dielectric_from_dipoles(&dipoles, options.0, &policy)
        })
        .map_err(value_error)?;
...
impl From<pdbiox::analysis::WaterLag> for PyWaterLag {
    fn from(value: pdbiox::analysis::WaterLag) -> Self {
        Self {
            lag: value.lag,
...
impl From<pdbiox::analysis::DielectricResult> for PyDielectricResult {
    fn from(value: pdbiox::analysis::DielectricResult) -> Self {
        Self {
...
fn water_lag_list(py: Python<'_>, values: Vec<pdbiox::analysis::WaterLag>) -> PyResult<Py<PyAny>> {
    let values = values
        .into_iter()
