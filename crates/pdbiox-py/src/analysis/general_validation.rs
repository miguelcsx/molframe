...
use super::{
    PyRamachandranOptions, PyRamachandranRecord, PyRamachandranRegion, PyReferenceLibrary,
};

#[pyclass(name = "BondDeviation", frozen, skip_from_py_object)]
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBondDeviation {
    inner: pdbiox::validate::BondDeviation,
    #[pyo3(get)]
    atom_a: u32,
    #[pyo3(get)]
    atom_b: u32,
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyPlanarityOptions(pub(crate) pdbiox::validate::PlanarityOptions);

#[pymethods]
impl PyPlanarityOptions {
    #[new]
    fn new(maximum_deviation: f64, plane_fit: &PyEigenOptions) -> Self {
        Self(pdbiox::validate::PlanarityOptions {
            maximum_deviation,
...
impl PyStructure {
    ...
    fn completeness(
        ...
        let structure = self.structure().clone();
        py.detach(move || {
            pdbiox::validate::completeness(&structure, namespace.into())
...
pub(crate) fn assess_bond_deviation(
    ...
    py.detach(move || {
        pdbiox::validate::assess_bond_deviation(&deviation, &references, &distribution)
    })
...
pub(crate) fn assess_ramachandran(
    ...
    let references = references.0.clone();
    let distribution = distribution.to_owned();
    py.detach(move || pdbiox::validate::assess_ramachandran(&record, &references, &distribution))
        .map(PyReferenceAssessment::from)
        .map_err(value_error)
}

#[pyfunction]
pub(crate) fn classify_ramachandran(
    py: Python<'_>,
    phi: f64,
    psi: f64,
    options: &PyRamachandranOptions,
) -> PyResult<(PyRamachandranRegion, PyReferenceAssessment)> {
...
project!(
    ...
    PyBondDeviation,
    |value: pdbiox::validate::BondDeviation| {
        Self {
            inner: value,
...
project!(
    pdbiox::validate::Clash,
    PyClash,
    |value: pdbiox::validate::Clash| Self {
...
project!(
    pdbiox::validate::CisPeptide,
    PyCisPeptide,
    |value: pdbiox::validate::CisPeptide| Self {
...
project!(
    ...
    PyPlanarityFlag,
    |value: pdbiox::validate::PlanarityFlag| {
        Self {
            residue: value.residue.get(),
            deviation: value.deviation,
        }
...
project!(
    pdbiox::validate::ValenceError,
    PyValenceError,
    |value: pdbiox::validate::ValenceError| {
        Self {
            atom: value.atom.get(),
            bonds: value.bonds,
...
project!(
    pdbiox::validate::ReferenceAssessment,
    PyReferenceAssessment,
    |value: pdbiox::validate::ReferenceAssessment| Self {
        set: value.set.into(),
...
pub(crate) fn project_completeness(
    ...
    value: pdbiox::validate::ChainCompleteness,
