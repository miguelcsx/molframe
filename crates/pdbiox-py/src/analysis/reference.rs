...
#[derive(Clone, Debug)]
pub(crate) struct PyReferenceDistribution(pub(super) pdbiox::validate::ReferenceDistribution);

#[pymethods]
impl PyReferenceDistribution {
    #[staticmethod]
    fn histogram(name: &str, edges: Vec<f64>, weights: Vec<f64>) -> PyResult<Self> {
        pdbiox::validate::ReferenceDistribution::histogram(name, edges, weights)
            .map(Self)
            .map_err(value_error)
    }
    ...
    fn grid(name: &str, x_edges: Vec<f64>, y_edges: Vec<f64>, weights: Vec<f64>) -> PyResult<Self> {
        pdbiox::validate::ReferenceDistribution::grid(name, x_edges, y_edges, weights)
            .map(Self)
            .map_err(value_error)
    }
}
...
#[derive(Clone, Debug)]
pub(crate) struct PyReferenceLibrary(pub(super) Arc<pdbiox::validate::ReferenceLibrary>);

#[pymethods]
impl PyReferenceLibrary {
    #[new]
    fn new(id: &str, version: &str, distributions: Vec<PyReferenceDistribution>) -> PyResult<Self> {
        pdbiox::validate::ReferenceLibrary::new(
            id,
            version,
...
#[derive(Clone, Debug)]
pub(crate) struct PyRamachandranBasin(pub(super) pdbiox::validate::RamachandranBasin);

impl PyRamachandranBasin {
    fn build(region: pdbiox::validate::RamachandranRegion, distribution: &str) -> PyResult<Self> {
        pdbiox::validate::RamachandranBasin::new(region, distribution)
            .map(Self)
            .map_err(value_error)
    }
}
...
pub(crate) struct PyRamachandranOptions {
    pub(super) references: Arc<pdbiox::validate::ReferenceLibrary>,
    pub(super) basins: Vec<pdbiox::validate::RamachandranBasin>,
...
impl PyRamachandranOptions {
    ...
    fn new(
        ...
        pdbiox::validate::RamachandranOptions::new(
            &references.0,
            basins.clone(),
            minimum_probability,
        )
