...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyTlsModel(pdbiox::validate::TlsModel);
...
pub(crate) fn b_factor_distribution(
    ...
    py.detach(move || {
        pdbiox::validate::b_factor_distribution(&structure, &selection, outlier_standard_deviations)
    })
    .map(Into::into)
    .map_err(value_error)
}
...
pub(crate) fn tls_b_factor_consistency(
    ...
    py.detach(move || {
        pdbiox::validate::tls_b_factor_consistency(
            &structure,
            &groups,
...
pub(crate) fn analyse_b_factor_distribution(
    ...
    let analysis = py
        .detach(move || {
            let kernel = pdbiox::validate::b_factor_distribution_kernel(
                &selection,
...
pub(crate) fn analyse_tls_b_factor_consistency(
    ...
    let analysis = py
        .detach(move || {
            let kernel = pdbiox::validate::tls_b_factor_consistency_kernel(
                &groups,
                maximum_absolute_deviation,
                symmetry_tolerance,
            );
...
fn native_groups(groups: Vec<PyTlsGroup>) -> Vec<pdbiox::validate::TlsGroup> {
    groups
        .into_iter()
        .map(|group| pdbiox::validate::TlsGroup {
            id: group.id,
...
impl From<pdbiox::validate::BFactorOutlier> for PyBFactorOutlier {
    fn from(value: pdbiox::validate::BFactorOutlier) -> Self {
        Self {
            atom: value.atom.get(),
            value: value.value,
...
impl From<pdbiox::validate::BFactorDistribution> for PyBFactorDistribution {
    fn from(value: pdbiox::validate::BFactorDistribution) -> Self {
        Self {
...
impl From<pdbiox::validate::TlsBFactorReport> for PyTlsBFactorReport {
    fn from(value: pdbiox::validate::TlsBFactorReport) -> Self {
        Self {
            intended: value.intended,
            assessed: value.assessed,
            flags: value
