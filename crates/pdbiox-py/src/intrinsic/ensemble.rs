...
use super::{coordinate_frames_owned, value_error};
...
pub(crate) fn analyse_rmsd_to_reference(
    ...
    let frames = coordinate_frames_owned(frames)
        ...
        .into_iter()
        .enumerate()
        .map(|(frame, positions)| pdbiox::traj::Timestep {
            frame,
            positions,
            ..Default::default()
        })
    ...
    let result = py
        .detach(|| {
            pdbiox::traj::analyse_rmsd_to_reference(&frames, reference, alignment.into(), &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn analyse_pairwise_fitted_rmsd(
    ...
    let policy = policy.inner.clone();
    let result = py
        .detach(|| pdbiox::traj::analyse_pairwise_fitted_rmsd(&frames, memory_limit, &policy))
        .map_err(value_error)?;
...
pub(crate) fn analyse_generalized_procrustes_mean(
    ...
    let result = py
        .detach(|| {
            pdbiox::traj::analyse_generalized_procrustes_mean(
                &frames,
                tolerance,
                maximum_iterations,
                &policy,
...
pub(crate) fn analyse_kmeans(
    ...
    let policy = policy.inner.clone();
    let result = py
        .detach(move || pdbiox::traj::analyse_kmeans(&observations, options.native(), &policy))
        .map_err(value_error)?;
...
pub(crate) fn analyse_group_coordinate_variance(
    ...
    let policy = policy.inner.clone();
    let result = py
        .detach(move || pdbiox::traj::analyse_group_coordinate_variance(&frames, &groups, &policy))
        .map_err(value_error)?;
