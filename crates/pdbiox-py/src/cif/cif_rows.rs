...
impl PyCifRows {
    ...
    fn integer(&self, item: &str) -> Option<i64> {
        self.category
            .value(item, self.row)
            .and_then(pdbiox::CifValue::as_integer)
    }

    fn float(&self, item: &str) -> Option<f64> {
        self.category
            .value(item, self.row)
            .and_then(pdbiox::CifValue::as_float)
    }

    fn is_recorded(&self, item: &str) -> bool {
        self.category
            .value(item, self.row)
            .is_some_and(pdbiox::CifValue::is_recorded)
    }
}

impl PyCifRows {
    pub(crate) fn from_category(category: pdbiox::Category) -> Self {
        Self { category, row: 0 }
    }
}
