...
const STRUCTURE_CAPSULE: &std::ffi::CStr = c"pdbiox.Structure";

/// Extracts a `pdbiox::Structure` through the stable Python capsule protocol.
///
/// # Errors
///
/// Returns a Python exception when the object exposes no capsule method or the
/// capsule fails its name check.
pub fn structure_from_python(object: &Bound<'_, PyAny>) -> PyResult<pdbiox::Structure> {
    ...
    Ok(unsafe { pointer.cast::<pdbiox::Structure>().as_ref() }.clone())
}
