...
fn map_boundary(value: PyMapBoundary) -> pdbiox::xtal::MapBoundary {
    match value {
        PyMapBoundary::Missing => pdbiox::xtal::MapBoundary::Missing,
        PyMapBoundary::Periodic => pdbiox::xtal::MapBoundary::Periodic,
    }
}
...
pub(crate) fn real_space_map_correlation(
    ...
    py.detach(move || pdbiox::validate::real_space_map_correlation(&observed, &calculated))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn masked_real_space_correlation(
    ...
    py.detach(move || pdbiox::validate::masked_real_space_correlation(&observed, &calculated, mask))
        .map(Into::into)
        .map_err(value_error)
}
...
pub(crate) fn sampled_real_space_correlation(
    ...
    let calculated = calculated.0.clone();
    py.detach(move || {
        pdbiox::validate::sampled_real_space_correlation(
            &observed,
            &calculated,
            positions,
...
fn correlation_value(
    py: Python<'_>,
    value: pdbiox::validate::RealSpaceCorrelation,
) -> PyResult<Py<PyAny>> {
...
pub(crate) fn governed_real_space_map_correlation(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::validate::governed_real_space_map_correlation(&observed, &calculated, &policy)
        })
        .map_err(value_error)?;
...
pub(crate) fn governed_masked_real_space_correlation(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::validate::governed_masked_real_space_correlation(
                &observed,
                &calculated,
                mask,
                &policy,
...
pub(crate) fn governed_sampled_real_space_correlation(
    ...
    let analysis = py
        .detach(move || {
            pdbiox::validate::governed_sampled_real_space_correlation(
                &observed,
                &calculated,
                positions,
                &policy,
