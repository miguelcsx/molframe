...
impl PyRigid {
    ...
    fn apply<'py>(
        ...
        let mut positions = coordinates(positions)?;
        let transform = self.0;
        py.detach(|| transform.apply_all(&mut positions));
        let values: Vec<f32> = positions.into_iter().flatten().collect();
        Array2::from_shape_vec((values.len() / 3, 3), values)
